...
- Do not treat a new npm workspace package as complete until `package-lock.json`
  has been regenerated and `npm ci` has been re-proven. Local `node_modules`
  can hide a missing lockfile sync that GitHub Actions will catch immediately.
