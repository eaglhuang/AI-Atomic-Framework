...
ATM Captain Dispatch Standard is the only valid worker dispatch format.
Every dispatch card sent or handed off to worker 001 through 007 MUST start
with exactly this first line:

```text
ATM Captain Dispatch Standard
Skill used: atm-dispatch
Delegation mode: <local | internal sidecar | external handoff>
Terminology boundary: ATM is the product, framework, CLI, and governance workflow. AI-Atomic-Framework is only this repository name; do not call ATM AAF or use AAF as shorthand.
Internal sidecar is the default for review, preflight, grep, checklist, planning-only checks, and post-report verification.
External dispatch is opt-in.
External write is forbidden unless explicitly granted.
Captain must apply atm-dispatch before any dispatch, sidecar delegation, review, condition review, or closeout.
```

Do not use a second dispatch format that starts with `Skill used:
atm-dispatch`, `[Dispatch to ...]`, or any custom heading. The header line above
is mandatory even for read-only sidecar, condition-review, and closeout
handoffs.
