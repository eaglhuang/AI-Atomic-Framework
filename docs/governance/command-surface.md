...
`node atm.mjs broker batch execute --surface build|projection --wave <id> --surface-family <family> --manifest-digest <digest> --sealed-source-sha <sha> --payload-digest <source-digest> --receipt-digest <output-digest>` extends the same broker-owned write executor to generated surfaces. It emits `atm.waveGeneratedWriteReceipt.v1`, rejects waiting/serial fallback decisions, and carries the sealed source, manifest digest, source digest, output digest, ticket ids, and covered task ids needed for later atomic checkpoint gating.

`node atm.mjs batch checkpoint-readiness --wave <id> --manifest-digest <digest> --task <id> --delivery-receipt <path> --build-receipt <path> --projection-receipt <path>` evaluates whether every task in the wave has commit, build, and projection receipts before atomic closeback. The result is `atm.atomicWaveCheckpointReadiness.v1`; missing receipts or blocked planning closeback keep the checkpoint in a reconcile-required state.
