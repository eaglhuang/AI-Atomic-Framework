...
The first adapter set lives in separate packages: `integration-claude-code`, `integration-codex`, `integration-copilot`, `integration-cursor`, and `integration-gemini`. Each package emits the same minimum ATM entrypoints and keeps charter injection as a delivery-time placeholder instead of copying host-specific rule text into framework core.
