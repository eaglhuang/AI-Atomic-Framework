...
SAMPLES = {
    "md": """# TASK-RFT-0015 可直接拿去讀的中文文件

這一批的重點是把句子修到可以直接拿去讀的狀態，而不是只留下幾個看起來像中文的碎片。
...
請同時保護 `owner-null`、`fallback-owner`、`payload.trace.severity`、`active-claim`、`closure packet`，不要把「確認」「而不是」「關聯」「可讀」洗掉。
""",
    "ts": """/**
 * tasks scope repair 的中文說明。
 * 這個命令比 tasks scope add 更強，因為它可以在緊急授權下補回遺漏的 paths，
 * 而不是直接跳過 scope repair。
 */
...
export function describeScopeRepair(input: ScopeRepairHint): string {
  ...
  const reason = input.reason || '請先確認 close-gate regressions 與 closure packet，而不是直接收尾';
  return `${input.taskId} :: ${actor} :: ${reason}`;
}
""",
    "json": """{
  "taskId": "TASK-RFT-0015",
  "status": "running",
  "owner": null,
