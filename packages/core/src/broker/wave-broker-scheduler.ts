...
const nonBatchableStates = new Set<WaveBrokerTicketState>(['released', 'failed', 'cancelled']);
const ticketStateTransitions: Readonly<Record<WaveBrokerTicketState, readonly WaveBrokerTicketState[]>> = {
...
export function createEmptyWaveBrokerSchedulerDocument(now = new Date().toISOString()): WaveBrokerSchedulerDocument {
  return { schemaId: 'atm.waveBrokerScheduler.v1', specVersion: '0.1.0', tickets: [], updatedAt: now };
}

export function createWaveBrokerTicket(input: WaveBrokerTicketInput): WaveBrokerTicket {
...
export function enqueueWaveBrokerTicket(
  document: WaveBrokerSchedulerDocument,
  input: WaveBrokerTicketInput
): { readonly document: WaveBrokerSchedulerDocument; readonly ticket: WaveBrokerTicket; readonly replayed: boolean } {
...
export function transitionWaveBrokerTicket(ticket: WaveBrokerTicket, to: WaveBrokerTicketState, now = new Date().toISOString()): WaveBrokerTicket {
...
export function planWaveBrokerBatch(input: {
  ...
  const candidates = input.document.tickets.filter((ticket) => {
    if (nonBatchableStates.has(ticket.state)) return false;
    if (input.waveId && ticket.waveId !== input.waveId) return false;
    if (input.surfaceKind && ticket.surfaceKind !== input.surfaceKind) return false;
    if (input.surfaceFamily && ticket.surfaceFamily !== input.surfaceFamily) return false;
    return true;
  });
  ...
  const selectedSurfaceKind = input.surfaceKind ?? (surfaceKinds.length === 1 ? surfaceKinds[0] as WaveBrokerSurfaceKind : null);
  ...
  const selected = candidates.filter((ticket) => (
    ticket.waveId === selectedWaveId
