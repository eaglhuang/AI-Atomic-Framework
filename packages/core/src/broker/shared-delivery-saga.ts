...
export type SharedDeliverySagaPhase =
  | 'prepare-inputs'
  | 'plan-blockers'
...
export type SharedDeliveryKillpoint =
  | 'before-blocker-plan'
  | 'after-blocker-plan'
  | 'after-temp-tree'
  | 'after-commit-object'
...
export interface SharedDeliverySagaReceipt {
  ...
  readonly specVersion: '0.1.0';
  readonly migration: { readonly strategy: 'none'; readonly fromVersion: null; readonly notes: string };
  readonly sagaId: string;
  readonly waveId: string;
...
const ORDERED_PHASES: readonly SharedDeliverySagaPhase[] = [
  'prepare-inputs',
  'plan-blockers',
  'compose-and-semantic-checks',
  'prepare-temp-index',
...
export function planSharedDeliverySaga(input: {
  ...
  if (input.decision.verdict !== 'batch-ready') blockers.push(`batch decision is ${input.decision.verdict}`);
  if (input.decision.surfaceKind !== 'commit') blockers.push('shared delivery saga only publishes commit surfaces');
  if (!input.expectedHeadSha.trim()) blockers.push('expected HEAD is required before publish');
  ...
  if (members.length !== input.decision.ticketIds.length) blockers.push('every selected ticket must resolve to one member');
  for (const member of members) {
    if (member.fileSlice.length === 0) blockers.push(`task ${member.taskId} has no file slice`);
    if (member.validatorRefs.length === 0) blockers.push(`task ${member.taskId} has no validator refs`);
  }
  ...
  const journal: SharedDeliverySagaJournal = {
...
function membersForDecision(input: {
  ...
  readonly validatorRefs: Readonly<Record<string, readonly string[]>>;
  readonly semanticRefs?: Readonly<Record<string, readonly string[]>>;
}): readonly SharedDeliverySagaMember[] {
...
function completedPhasesFor(killpoint: SharedDeliveryKillpoint | null | undefined, receipt: SharedWriteReceipt | null | undefined): readonly SharedDeliverySagaPhase[] {
  if (receipt?.commitSha) return ORDERED_PHASES.slice(0, 7);
  if (!killpoint) return ORDERED_PHASES.slice(0, 5);
  const stopAt: Record<SharedDeliveryKillpoint, SharedDeliverySagaPhase> = {
...
function recoveryActionFor(
  killpoint: SharedDeliveryKillpoint | null | undefined,
  sideEffects: readonly SharedDeliverySagaSideEffect[]
): SharedDeliverySagaReceipt['recoveryAction'] {
  if (!killpoint) return 'none';
  if (sideEffects.some((effect) => effect.acknowledged && effect.kind === 'update-ref')) return 'replay-receipt';
  if (sideEffects.some((effect) => effect.acknowledged && effect.compensation)) return 'compensate';
  return 'rearbitrate';
}
