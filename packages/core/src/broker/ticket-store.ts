...
export type BrokerTicketStoreAction = 'enqueue' | 'transition' | 'wake-next' | 'adopt-orphan' | 'cancel' | 'reconcile-side-effect';
export type BrokerTicketStoreDocument = Readonly<{
  schemaId: 'atm.brokerTicketStore.v1'; specVersion: '0.1.0'; storeId: string; generation: number;
  tickets: readonly BrokerTicket[]; transitions: readonly BrokerTicketStoreTransitionEvidence[]; updatedAt: string;
}>;
export type BrokerTicketStoreSnapshot = Readonly<{
  schemaId: 'atm.brokerTicketStoreSnapshot.v1'; storePath: string; generation: number; digest: string; document: BrokerTicketStoreDocument;
}>;
export type BrokerTicketStoreTransitionEvidence = Readonly<{
  schemaId: 'atm.brokerTicketStoreTransitionEvidence.v1'; transitionId: string; idempotencyKey: string;
  taskId: string; actorId: string; laneId: string | null; ticketId: string | null; action: BrokerTicketStoreAction;
  previousDigest: string; nextDigest: string; previousGeneration: number; nextGeneration: number; occurredAt: string;
}>;
export type BrokerTicketStoreReceipt = Readonly<{
  schemaId: 'atm.brokerTicketStoreReceipt.v1'; status: 'committed' | 'idempotent-replay' | 'revalidation-required';
  action: BrokerTicketStoreAction; taskId: string; actorId: string; laneId: string | null; idempotencyKey: string;
  storePath: string; baseDigest: string; nextDigest: string; baseGeneration: number; nextGeneration: number;
  transitionEvidence: BrokerTicketStoreTransitionEvidence | null; ticket: BrokerTicket | null;
  revalidationTicket: BrokerTicket | null; recoveryCommand: string | null;
}>;

export interface BrokerTicketStore {
  readonly storePath: string;
  read(): BrokerTicketStoreSnapshot;
  commit(input: BrokerTicketStoreCommitInput): BrokerTicketStoreReceipt;
}

export type BrokerTicketStoreCommitInput = Readonly<{
  base: BrokerTicketStoreSnapshot; action: BrokerTicketStoreAction; taskId: string; actorId: string;
  laneId?: string | null; idempotencyKey: string; now?: string;
  mutate: (document: BrokerTicketStoreDocument, context: BrokerTicketStoreMutationContext) => BrokerTicketStoreMutationResult;
}>;
export type BrokerTicketStoreMutationContext = Readonly<{ now: string }>;
export type BrokerTicketStoreMutationResult = Readonly<{ document: BrokerTicketStoreDocument; ticket?: BrokerTicket | null }>;
export type EnqueueBrokerTicketInput = Readonly<{
  taskId: string; actorId: string; resourceKey: string; laneId?: string | null; idempotencyKey?: string; now?: string; ttlSeconds?: number;
}>;
export type TransitionStoredBrokerTicketInput = Readonly<{
  ticketId: string; to: BrokerTicketState; taskId: string; actorId: string; laneId?: string | null; reason: string; idempotencyKey: string; now?: string;
}>;
export type WakeNextBrokerTicketInput = Readonly<{
  resourceKey: string; taskId: string; actorId: string; laneId?: string | null; idempotencyKey: string;
  maxBypassCount?: number; maxEligibleWaitMs?: number; now?: string;
}>;
export type AdoptOrphanBrokerTicketInput = Readonly<{
  ticketId: string; taskId: string; actorId: string; laneId?: string | null; reason: string; idempotencyKey: string; now?: string;
}>;
export type ReconcileBrokerTicketInput = AdoptOrphanBrokerTicketInput;

export function createEmptyBrokerTicketStoreDocument(input: {
  readonly storeId?: string;
  readonly now?: string;
} = {}): BrokerTicketStoreDocument {
...
export function createBrokerTicketStore(storePath: string): BrokerTicketStore {
  return {
    storePath,
...
export function readBrokerTicketStoreSnapshot(storePath: string): BrokerTicketStoreSnapshot {
...
export function digestBrokerTicketStoreDocument(document: BrokerTicketStoreDocument): string {
  return `sha256:${createHash('sha256').update(JSON.stringify(canonicalize(document))).digest('hex')}`;
}

export function enqueueBrokerTicket(
  store: BrokerTicketStore,
  input: EnqueueBrokerTicketInput
): BrokerTicketStoreReceipt {
  ...
  return store.commit({
    ...
    mutate: (document, context) => {
      ...
      if (existing) return { document, ticket: existing };
...
export function transitionStoredBrokerTicket(
  store: BrokerTicketStore,
  input: TransitionStoredBrokerTicketInput
): BrokerTicketStoreReceipt {
  const base = store.read();
  return store.commit({
    base,
...
export function wakeNextBrokerTicket(
  store: BrokerTicketStore,
  input: WakeNextBrokerTicketInput
): BrokerTicketStoreReceipt {
  const base = store.read();
  return store.commit({
    base,
    ...
    mutate: (document, context) => {
      ...
      if (active) return { document, ticket: active };
      ...
      if (!selected) return { document, ticket: null };
      ...
      const nextTickets = document.tickets.map((ticket) => {
        if (ticket.ticketId === transitioned.ticketId) return transitioned;
        if (ticket.resourceKey !== input.resourceKey || selectedIds.has(ticket.ticketId)) return ticket;
        if (ticket.state !== 'queued' && ticket.state !== 'ready') return ticket;
...
export function adoptOrphanBrokerTicket(
  store: BrokerTicketStore,
  input: AdoptOrphanBrokerTicketInput
): BrokerTicketStoreReceipt {
  const base = store.read();
  return store.commit({
    base,
...
export function cancelBrokerTicket(
  store: BrokerTicketStore,
  input: ReconcileBrokerTicketInput
): BrokerTicketStoreReceipt {
  const base = store.read();
  return store.commit({
    base,
...
export function reconcileBrokerTicketSideEffect(
  store: BrokerTicketStore,
  input: ReconcileBrokerTicketInput
): BrokerTicketStoreReceipt {
  const base = store.read();
...
export function isBrokerTicketOrphan(ticket: BrokerTicket, now = new Date().toISOString()): boolean {
  const heartbeatMs = Date.parse(ticket.heartbeatAt);
  const nowMs = Date.parse(now);
  if (!Number.isFinite(heartbeatMs) || !Number.isFinite(nowMs)) return false;
  return heartbeatMs + ticket.ttlSeconds * 1000 <= nowMs;
}

export function commitBrokerTicketStoreTransaction(
  storePath: string,
  input: BrokerTicketStoreCommitInput
): BrokerTicketStoreReceipt {
...
function buildRevalidationReceipt(
  storePath: string,
  input: BrokerTicketStoreCommitInput,
  current: BrokerTicketStoreSnapshot
): BrokerTicketStoreReceipt {
...
function buildTransitionEvidence(input: {
  readonly input: BrokerTicketStoreCommitInput;
  readonly storePath: string;
  readonly ticketId: string | null;
  readonly previous: BrokerTicketStoreSnapshot;
  readonly nextGeneration: number;
  readonly nextDigest: string;
  readonly now: string;
}): BrokerTicketStoreTransitionEvidence {
...
function replaceTicket(document: BrokerTicketStoreDocument, ticket: BrokerTicket, now: string): BrokerTicketStoreDocument {
  return {
    ...document,
...
function requireTicket(document: BrokerTicketStoreDocument, ticketId: string): BrokerTicket {
...
function compareTicketPriority(now: string, maxBypassCount: number, maxEligibleWaitMs: number) {
  const nowMs = Date.parse(now);
  return (left: BrokerTicket, right: BrokerTicket): number => {
...
function forcedPriority(ticket: BrokerTicket, nowMs: number, maxBypassCount: number, maxEligibleWaitMs: number): number {
...
function parseBrokerTicketStoreDocument(storePath: string, raw: string): BrokerTicketStoreDocument {
  let parsed: unknown;
  try {
...
function isBrokerTicketStoreDocument(value: unknown): value is BrokerTicketStoreDocument {
  return Boolean(
    value
    && typeof value === 'object'
    && !Array.isArray(value)
    && (value as { schemaId?: unknown }).schemaId === 'atm.brokerTicketStore.v1'
    && (value as { specVersion?: unknown }).specVersion === '0.1.0'
    && Array.isArray((value as { tickets?: unknown }).tickets)
    && Array.isArray((value as { transitions?: unknown }).transitions)
