...
import { candidatesToWriteIntent, type BridgeAtomCandidate } from '../candidate-bridge.ts';
import { calculateBrokerDecision } from '../decision.ts';
import {
  cleanupGeneratedProjectionSteward,
  classifyBacklogItemShardProjectionWork,
  emptyGeneratedProjectionSteward,
  enqueueGeneratedProjectionRebuild,
  governanceBacklogProjectionKeyForPath
} from '../generated-projection-steward.ts';
...
function makeCandidate(filePath: string, taskId: string): BridgeAtomCandidate {
  return {
...
function makeIntent(taskId: string, targetFiles: readonly string[]): WriteIntent {
