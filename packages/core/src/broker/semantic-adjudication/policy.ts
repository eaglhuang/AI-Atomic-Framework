import type {
  ...
  SemanticRevalidationRequest,
  SemanticRevalidationResult,
  SemanticTicketNextAction,
...
export function adjudicateSemanticRevalidation(request: SemanticRevalidationRequest): SemanticRevalidationResult {
...
export function findStaleReadAnchors(
  readSet: readonly SemanticReadObservation[],
  publishedWriteSet: readonly SemanticWriteFact[]
): readonly string[] {
...
export function canComposeOperations(left: SemanticWriteFact, right: SemanticWriteFact): boolean {
...
function findOperationConflicts(writes: readonly SemanticWriteFact[]): readonly string[] {
...
function sameSemanticAnchor(read: SemanticReadObservation, write: SemanticWriteFact): boolean {
...
function sameWriteSurface(left: SemanticWriteFact, right: SemanticWriteFact): boolean {
...
function operationPair(left: SemanticWriteFact, right: SemanticWriteFact): string {
  return [left.operation, right.operation].sort().join('+');
}

function resultFor(
  request: SemanticRevalidationRequest,
  verdict: SemanticRevalidationResult['verdict'],
  ticketNextAction: SemanticTicketNextAction,
  reasons: readonly string[],
  staleReadAnchors: readonly string[],
  operationConflicts: readonly string[]
): SemanticRevalidationResult {
  return {
...
function normalizeValidators(validators: readonly SemanticValidatorRef[]): readonly SemanticValidatorRef[] {
