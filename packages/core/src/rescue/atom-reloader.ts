...
export interface ReloadAtomsResult {
  ...
  restoredFiles: Array<{ atomId: string; filePath: string }>;
  skippedCapsules: string[];
  errors: string[];
}

export function reloadAtomsFromCapsules(
  ...
  const result: ReloadAtomsResult = {
    dryRun,
    backedUpTo: null,
    restoredFiles: [],
    skippedCapsules: [],
  ...
  for (const filename of capsuleFiles) {
    ...
    try {
      ...
      const compressedPayload: string | undefined = content.compressedPayload;
      ...
      const atomId: string = bundle?.atomId ?? filename.replace('.json', '');
      ...
      if (existsSync(registryPath)) {
        ...
        const codePaths: string[] = entry?.selfVerification?.sourcePaths?.code ?? [];
