...
export function scaffoldAtomWorkbench(normalizedModel: any, options: any = {}) {
  ...
  const result: {
    ...
    createdFiles: Array<{ kind: string; outputPath: string }>;
    overwrittenFiles: Array<{ kind: string; outputPath: string }>;
    skippedFiles: Array<{ kind: string; outputPath: string; reason: string }>;
    renderedFiles: Array<{ kind: string; outputPath: string; templatePath: string }>;
  } = {
    ok: true,
