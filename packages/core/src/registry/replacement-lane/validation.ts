import { ReplacementMode, evidenceRequirementByTarget, orderedReplacementModes } from './constants.ts';
import {
  findHumanReviewEvidence,
  findMapEquivalenceEvidence,
  findPropagationEvidence,
  findRetirementProofEvidence,
  findReviewAdvisoryEvidence,
  findRollbackProofEvidence,
  loadEvidenceDocuments
} from './evidence.ts';
import { createReplacementLaneError, requiresEvidence } from './support.ts';
import type { ReplacementTransitionInput } from './types.ts';

export function validateTransition(input: ReplacementTransitionInput) {
