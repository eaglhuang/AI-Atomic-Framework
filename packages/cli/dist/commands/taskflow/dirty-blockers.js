...
export function buildGovernanceDirtyBlocker(input) {
    if (input.dirtyGuard.governanceTrackedDirtyFiles.length === 0)
        return null;
    return {
...
export function buildUnexpectedStagedBlocker(unexpectedStagedTasks) {
    ...
    return {
        ...
        files,
        taskIds,
        remediationChoices: unexpectedStagedTasks.map((entry) => ({ id: 'defer-foreign-staged', summary: entry.restoreChoice, requiredCommand: entry.deferCommand })),
