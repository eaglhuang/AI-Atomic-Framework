...
function stageGovernanceFile(repoRoot, taskId, suffix = 'json') {
...
try {
    ...
    assert.throws(() => assertCloseWindowStagingAllowed({
        cwd: repoRoot,
        taskId: foreignTaskId,
        operation: 'stageRepoBundle'
    }), (error) => error instanceof CliError && error.code === 'ATM_CLOSE_WINDOW_STAGED_INDEX_LOCKED');
    ...
    assert.equal(readCloseWindowStagedIndexLockReport(repoRoot), null);
    assert.equal(readFileSync(path.join(repoRoot, `.atm/history/tasks/${taskId}.json`), 'utf8'), previousTaskContent);
