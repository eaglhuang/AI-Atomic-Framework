...
export async function runExperience(argv) {
    ...
    const behaviorDetails = (behaviorOutput.evidence[0]?.details ?? {});
    const candidate = behaviorDetails.skillCandidate;
    const proposalSnapshot = behaviorDetails.proposalSnapshot;
...
function parseExperienceOptions(argv) {
    const options = {
        cwd: process.cwd(),
        fromTask: '',
        inputPath: '',
...
function normalizeExtractionInput(rawInput, fromTaskOverride) {
...
function createExperienceAdvisoryReport(candidate, proposalSnapshot, crossedThreshold, inputPath, cwd) {
...
function enqueueExperienceProposal(cwd, queuePathInput, projectionPathInput, proposalSnapshot) {
    ...
    const nextRecord = createHumanReviewQueueRecord(proposalSnapshot);
