...
import { buildKnowledgeIndex, buildKnowledgeStats } from './metadata.js';
import { applyHybridRerank, buildFilters, buildHybridRequest, deriveQueryText, rankKnowledgeHits } from './ranking.js';
import { buildManifest, isInsidePath, isRuntimePrunableCache, parsePositiveInteger, resolveKnowledgeOutputs } from './runtime-utils.js';
export function runKnowledgeBuild(options, cwd, permission) {
...
export function runKnowledgeQuery(options, cwd, permission) {
    const outputs = resolveKnowledgeOutputs(cwd);
...
export function runKnowledgeStats(options, cwd, permission) {
...
export function runKnowledgeCompact(options, cwd, permission) {
    const stats = buildKnowledgeStats(cwd, options);
    const outputs = resolveKnowledgeOutputs(cwd);
    const dryRun = Boolean(options['dry-run']) || !Boolean(options.write);
