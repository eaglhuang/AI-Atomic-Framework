...
import { readWorkAdmissionTicket } from './git-governance/work-admission-check.js';
import { isPathAllowedByScope } from './work-channels.js';
import { forEachPathspecBatch } from './git-governance/implementation/pathspec-argv-batching.js';
...
export function authorizeGitIndexOverrideLease(input) {
    ...
    const liveEntries = input.report.entries.filter((entry) => entry.ownership === 'foreign-active-owned'
...
export function consumeGitIndexOverrideLease(cwd, lease) {
...
export function inspectGitIndexOwnership(input) {
    ...
    const entries = stagedFiles.map((filePath) => {
        ...
        const stagedState = stagedDeletions.has(normalizeRelativePath(filePath).toLowerCase())
            ? 'deleted'
            : stagedBlob
                ? 'present'
                : 'unknown';
...
function readStagedBlobMap(cwd, stagedFiles) {
    ...
    if (stagedFiles.length === 0)
        return map;
    try {
        forEachPathspecBatch({ paths: stagedFiles, fixedArgs: ['ls-files', '-s', '--'] }, (batch) => {
...
function readStagedDeletionSet(cwd, stagedFiles) {
    const deletions = new Set();
    if (stagedFiles.length === 0)
        return deletions;
    try {
        forEachPathspecBatch({ paths: stagedFiles, fixedArgs: ['diff', '--cached', '--diff-filter=D', '--name-only', '--'] }, (batch) => {
...
function extractGovernanceTaskId(filePath) {
    ...
    if (normalized.toLowerCase() === '.atm/history/evidence/git-head.jsonl')
        return null;
