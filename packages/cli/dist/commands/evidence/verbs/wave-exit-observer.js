...
export function runWaveExitObserver(argv, deps = {}) {
...
function parseWaveExitObserverArgv(argv) {
    let cwd = process.cwd();
    let exitItemId = '';
    let actor = '';
    let observerRole = '';
    let basisOwners = [];
    let declaredBasisActor = null;
    let claimedCommand = null;
    let claimedArtifactPath = null;
    const extraFlags = [];
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--cwd')
            ...
            exitItemId = requireValue(argv, ++index, arg);
        else if (arg === '--actor')
            actor = requireValue(argv, ++index, arg);
        else if (arg === '--observer-role')
            observerRole = requireValue(argv, ++index, arg);
        else if (arg === '--basis-owners') {
            basisOwners = requireValue(argv, ++index, arg).split(',').map((entry) => entry.trim()).filter(Boolean);
        }
        else if (arg === '--declared-basis-actor')
            declaredBasisActor = requireValue(argv, ++index, arg);
        else if (arg === '--command') {
    ...
    if (!exitItemId)
        throw new CliError('ATM_CLI_USAGE', 'evidence wave-exit-observer requires --exit EXIT-NN', { exitCode: 2 });
    if (!observerRole)
        throw new CliError('ATM_CLI_USAGE', 'evidence wave-exit-observer requires --observer-role <sealed-role>', { exitCode: 2 });
    return {
...
function gitShowAtHead(cwd, head, relativePath) {
    try {
...
function runSealedNodeCommand(cwd, command) {
