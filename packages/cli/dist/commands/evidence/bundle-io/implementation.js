...
function canAutoRunDeclaredValidator(gate, command, taskDeclared) { if (validatorRequiresOperatorApproval(gate, command))
    ...
    return true; if (isTaskDeclaredValidatorGate(gate, taskDeclared))
...
function hasHealthyAtomEvidence(taskDocument, bundle) { const targets = extractAtomizationTargets(taskDocument); if (targets.length === 0)
    ...
    const candidates = details && Array.isArray(details.atomHealthClaims) ? details.atomHealthClaims : details && isRecord(details.historicalBatch) && Array.isArray(details.historicalBatch.atomHealthClaims) ? details.historicalBatch.atomHealthClaims : [];
...
function hasTeamClosureAttestation(value) { if (!isRecord(value))
    return false; if (isRecord(value.details)) {
...
function sleepMs(ms) { if (!Number.isFinite(ms) || ms <= 0)
    return; Atomics.wait(evidenceWriteSleepBuffer, 0, 0, ms); }
