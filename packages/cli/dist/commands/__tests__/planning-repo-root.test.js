...
import { PLANNING_REPO_ROOT_ENV, isExternalPlanningStoredPath, readConfiguredPlanningRoots, resolveStoredPlanningPath, toStoredPlanningPath } from '../planning-repo-root.js';
describe('planning-repo-root', () => {
    ...
    it('honors ATM_PLANNING_REPO_ROOT when resolving stored paths', () => {
        ...
        try {
            ...
            if (previous === undefined)
                ...
                process.env[PLANNING_REPO_ROOT_ENV] = previous;
            rmSync(targetCwd, { recursive: true, force: true });
        }
    });
    it('prefers an existing repo-local docs path over external planning roots', () => {
        ...
        const resolved = resolveStoredPlanningPath(targetCwd, 'docs/ai_atomic_framework/atm-agent-first-operability/tasks/TASK-LOCAL.task.md');
