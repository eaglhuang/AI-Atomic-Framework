...
async function testResidueReconcileAppliesOnlySafeOwnerAwareResidue() {
    ...
    try {
        ...
        assert.ok(dryRun.evidence.report.actions.some((action) => action.path === '.atm/runtime/git-index-leases/git-stage-override-expired.json'), 'expired git-index override lease without active owner should be planned for cleanup');
        ...
        assert.ok(dryRun.evidence.report.deferred.some((entry) => entry.path === '.atm/runtime/git-index-leases/git-stage-override-active.json'), 'active git-index override lease must be deferred');
        assert.ok(dryRun.evidence.report.deferred.some((entry) => entry.path === '.atm/runtime/git-index-leases/git-stage-override-active-owner.json'), 'expired git-index override lease owned by an active task must be deferred');
        assert.ok(dryRun.evidence.report.deferred.some((entry) => entry.path === '.atm/runtime/git-index-leases/git-stage-override-unreadable.json'), 'unreadable git-index override lease must be deferred');
