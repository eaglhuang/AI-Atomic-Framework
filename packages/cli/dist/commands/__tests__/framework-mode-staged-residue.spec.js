...
async function testCrossFileConsistency() {
    ...
    try {
        ...
        const consistencyFinding = hookResult1.evidence.blockingFindings.find((finding) => finding.code === 'ATM_PRE_COMMIT_CROSS_FILE_INCONSISTENCY');
...
async function testCrossFileConsistencyIgnoresTemplateLiteralImportLookalikes() {
    ...
    try {
        ...
        writeFileSync(path.join(tempRoot, closeOrchestrationFile), 'export const alpha = 1;\n', 'utf8');
        writeFileSync(path.join(tempRoot, taskflowFile), [
            'import { alpha } from "./taskflow/close-orchestration.js";',
            'const lookalike = `',
            'import { validatorSurfaces } from "./taskflow/close-orchestration.js";',
            '`;',
        ...
        execFileSync('git', ['commit', '-m', 'initial'], { cwd: tempRoot });
        writeFileSync(path.join(tempRoot, closeOrchestrationFile), [
        ...
        const hasConsistencyError = hookResult.evidence.blockingFindings.some((finding) => finding.code === 'ATM_PRE_COMMIT_CROSS_FILE_INCONSISTENCY');
