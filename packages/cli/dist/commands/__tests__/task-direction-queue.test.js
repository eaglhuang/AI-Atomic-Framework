import { advanceTaskQueueHead, createOrRefreshTaskQueue, findActiveTaskQueue, readActiveTaskDirectionLocks } from '../task-direction.js';
import { mkdirSync, mkdtempSync, writeFileSync } from 'node:fs';
import os from 'node:os';
import path from 'node:path';
import { listActiveBatchRuns, readBatchRunById, writeBatchRun } from '../work-channels.js';
...
writeFileSync(path.join(ghostTaskRoot, 'TASK-GHOST-0002.json'), `${JSON.stringify({
...
writeFileSync(path.join(unfinishedTaskRoot, 'TASK-BLOCKED-0002.json'), `${JSON.stringify({
    schemaVersion: 'atm.workItem.v0.2',
