 ...
 */
export function selectTicketValidatedCommitFiles(stagedFiles, ticket, deferForeignStaged, stagesExactlyTheTicketBundle = false) {
    if (!ticket)
        return stagedFiles;
    ...
    // can be produced.
    if (stagedFiles.length === 0 && fileScopes.length > 0)
        return fileScopes;
    if (stagesExactlyTheTicketBundle)
        return fileScopes;
