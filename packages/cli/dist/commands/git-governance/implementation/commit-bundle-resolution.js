...
import { isUncommittableTaskEvidenceArtifact } from './task-evidence-admission.js';
import { resolvePublicationSliceAdmission } from './publication-slice-admission.js';
function isWipGovernanceStatePath(filePath) {
...
export function resolveTaskScopedCommitBundle(input) {
    ...
    const wipGovernanceStagedFiles = input.wip && input.apply
        ? stagedFiles.filter((filePath) => isWipGovernanceStatePath(filePath))
        : [];
    let wipGovernanceSnapshotPath = null;
    if (wipGovernanceStagedFiles.length > 0) {
        wipGovernanceSnapshotPath = deferStagedFilePaths(input.cwd, input.taskId, wipGovernanceStagedFiles);
    ...
    const inScopeUnstagedDirty = uniqueSorted([
        ...unstagedDirtyFiles,
        ...taskOwnedProtectedOverrideAudits,
    ]).filter((filePath) => (!input.wip || !isWipGovernanceStatePath(filePath)) &&
