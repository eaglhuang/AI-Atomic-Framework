...
import { splitCsvPaths } from './task-scope-staging.js';
export function parseGitOptions(argv) {
    const options = {
        cwd: process.cwd(),
        action: null,
        leaseKind: null,
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === "--stage-override-lease") {
            options.stageOverrideLease = requireValue(argv, index, "--stage-override-lease");
            index += 1;
            continue;
        }
        if (arg === "--dry-run") {
