...
function ledgerTaskId(filePath) {
    const match = normalize(filePath).match(/^\.atm\/history\/tasks\/([^/]+)\.json$/i);
    return match ? match[1] : null;
}
function eventRef(filePath) {
    const normalized = normalize(filePath);
    const match = normalized.match(/^\.atm\/history\/task-events\/([^/]+)\/[^/]*Z-([a-z][a-z-]*)-[0-9a-f]{6,}\.json$/i);
    if (!match)
        return null;
...
export function classifyBlockLifecycleRecordBundle(input) {
    ...
    const ledgerFiles = [];
    const blockEventFiles = [];
    const otherRecordFiles = [];
    const ledgerIds = new Set();
    const blockEventPathIds = [];
    ...
    const reject = (reasonCode, reason, taskId) => ({ kind: 'ineligible', reasonCode, reason, taskId, stagedFiles });
    const candidateTaskId = ledgerIds.size === 1 ? [...ledgerIds][0] : blockEventPathIds.length > 0 ? blockEventPathIds[0] : null;
    if (otherRecordFiles.length > 0) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.extraRecordFiles, `record-only block bridge accepts only one ledger + one block event; extra record file(s) present: ${otherRecordFiles.join(', ')}.`, candidateTaskId);
    }
    if (ledgerIds.size > 1) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.multipleLedgers, `record-only block bridge accepts a single card; multiple ledgers staged: ${[...ledgerIds].sort().join(', ')}.`, null);
    }
    if (blockEventFiles.length === 0) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.incompletePair, 'record-only block bridge requires the matching block event; ledger staged without its block event.', candidateTaskId);
    }
    if (ledgerFiles.length === 0) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.incompletePair, 'record-only block bridge requires the card ledger; block event staged without its ledger.', candidateTaskId);
    }
    if (blockEventFiles.length > 1) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.multipleBlockEvents, `record-only block bridge accepts exactly one block event; ${blockEventFiles.length} staged.`, candidateTaskId);
    }
    ...
    if (blockEventPathIds[0] !== taskId) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.mixedTask, `record-only block bridge requires the ledger and block event to share a card; ledger ${taskId} vs event path ${blockEventPathIds[0]}.`, taskId);
    }
    const ledger = input.readLedgerRecord(taskId);
    if (!ledger) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.ledgerMissing, `record-only block bridge could not read live ledger state for ${taskId}.`, taskId);
    }
    ...
    if (!nonEmpty(ledger.workItemId) || ledger.workItemId !== taskId) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.ledgerIdMismatch, `record-only block bridge requires the ledger document workItemId to equal its path id; path ${taskId} vs document ${ledger.workItemId ?? 'null'}.`, taskId);
    }
    if (ledger.status !== BLOCKED_STATUS || ledger.claimState !== RELEASED_CLAIM_STATE) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.ledgerNotBlockedReleased, `record-only block bridge requires status=blocked and claimState=released; ${taskId} is status=${ledger.status} claimState=${ledger.claimState ?? 'null'}.`, taskId);
    }
    // Parse and validate the block event content — the file name is not authority.
    const event = input.readEventRecord(eventPath);
    if (!event) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.eventUnreadable, `record-only block bridge could not parse the staged block event ${eventPath}.`, taskId);
    }
    if (event.action !== BLOCK_ACTION || event.toStatus !== BLOCKED_STATUS) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.eventNotBlock, `record-only block bridge requires the event to be a block transition; event action=${event.action ?? 'null'} toStatus=${event.toStatus ?? 'null'}.`, taskId);
    }
    if (event.taskId !== taskId || (nonEmpty(event.taskPath) && normalize(event.taskPath) !== ledgerPath)) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.eventTaskMismatch, `record-only block bridge requires the event content to name the same card and ledger; event taskId=${event.taskId ?? 'null'} taskPath=${event.taskPath ?? 'null'} vs ${taskId}/${ledgerPath}.`, taskId);
    }
    ...
    if (!nonEmpty(ledger.claimActorId) || !nonEmpty(ledger.claimLeaseId) || !nonEmpty(event.actorId)) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.attributionMissing, `record-only block bridge requires retained actor + lease attribution; ledger actor=${ledger.claimActorId ?? 'null'} lease=${ledger.claimLeaseId ?? 'null'} event actor=${event.actorId ?? 'null'}.`, taskId);
    }
    if (event.actorId !== ledger.claimActorId) {
        return reject(BLOCK_BRIDGE_REJECTION_CODES.attributionMismatch, `record-only block bridge requires the block event actor to match the retained ledger claim actor; event ${event.actorId} vs ledger ${ledger.claimActorId}.`, taskId);
    }
    return {
        kind: 'eligible',
        taskId,
 ...
 */
export function recordOnlyClaimScopeExemptCovers(exemptPaths, outOfScopeStagedFiles) {
