...
import { compileAdapterParity, EDITORS } from '../../_vendor/core/dist/evidence/adapter-parity.js';
import { resolveValue } from '../shared.js';
import { createIntegrationAdapter, createIntegrationContext } from './adapters.js';
import { readIntegrationManifest, verifyInstalledManifest } from './health.js';
const digest = (value) => `sha256:${createHash('sha256').update(JSON.stringify(value)).digest('hex')}`;
const sourceCatalogDigest = (manifest) => typeof manifest?.metadata?.sourceCatalogDigest === 'string'
    ? manifest.metadata.sourceCatalogDigest
    : '';
/** Stable digest of a projection manifest; timestamps are deliberately excluded. */
export function digestIntegrationManifest(manifest) {
    return digest({
...
function digestCompilerExpectation(manifest) {
    return digest({
 ...
 */
export async function collectAdapterParity(repositoryRoot) {
