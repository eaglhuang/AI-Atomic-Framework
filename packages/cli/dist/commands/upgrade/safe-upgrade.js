...
import { createATMVersionSummary, loadCompatibilityMatrix, runATMChart } from '../atm-chart.js';
import { CliError, makeResult, message, readJsonFile, writeJsonFile } from '../shared.js';
import { normalizeRepositoryRelativePath, requireOptionValue, resolveRepositoryPath, safeReadJson, sha256File } from './path-helpers.js';
import { parseCanaryPercent, resolveCanarySelection, shouldApplyUpgradeFile } from './canary.js';
// ─── Action detection ──────────────────────────────────────────────────────
export function firstSafeUpgradeAction(argv) {
...
export function parseSafeUpgradeOptions(argv, action) {
    const options = {
        action,
        cwd: process.cwd(),
        out: null,
...
// ─── Plan ─────────────────────────────────────────────────────────────────
export function runSafeUpgradePlan(options) {
...
// ─── Apply ────────────────────────────────────────────────────────────────
export async function runSafeUpgradeApply(options) {
    ...
    const willModify = Array.isArray(plan.willModify) ? plan.willModify.map((entry) => normalizeRepositoryRelativePath(String(entry))) : [];
...
// ─── Rollback ─────────────────────────────────────────────────────────────
export function runSafeUpgradeRollback(options) {
...
// ─── File collection ───────────────────────────────────────────────────────
export function collectSafeUpgradeFiles(cwd) {
...
function addManifestFiles(records, cwd, manifestDir, role) {
    ...
    if (!existsSync(absoluteManifestDir))
        return;
...
function addBackupRecord(records, cwd, filePath, details) {
