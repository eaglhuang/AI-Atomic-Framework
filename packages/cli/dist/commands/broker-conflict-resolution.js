...
export function collectResolutionAuthorizedForeignTaskIds(cwd, taskId, explicitArtifactPath) {
    ...
    const addArtifact = (sourcePath) => {
        const absolutePath = path.resolve(cwd, sourcePath);
        if (seenSources.has(absolutePath) || !existsSync(absolutePath))
            return;
        seenSources.add(absolutePath);
        try {
            const artifact = JSON.parse(readFileSync(absolutePath, 'utf8'));
            if (artifact.schemaId === 'atm.brokerConflictResolution.v1')
                artifacts.push({ artifact, sourcePath });
        }
        catch {
            // Invalid sidecars remain fail-closed.
        }
    };
    if (explicitArtifactPath?.trim())
        addArtifact(explicitArtifactPath);
    ...
    for (const { artifact } of artifacts) {
        if (normalizeTaskId(artifact.primaryTaskId) !== normalizedTaskId
            ...
            continue;
        for (const foreignTaskId of readBlockedTaskIds(artifact))
...
function authoritySortKey(artifact) {
    return `${String(artifact.createdAt ?? '')}\u0000${String(artifact.resolutionId ?? '')}`;
}
function hasNewerOpposingAuthority(candidate, artifacts) {
