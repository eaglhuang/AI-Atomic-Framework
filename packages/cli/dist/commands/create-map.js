...
function buildCreateMapNextActionHint(cwd, mapId, dryRun) {
    if (dryRun || !mapId) {
        return null;
    }
    return {
...
function resolveCreateMapInput(options) {
    if (options.specPath) {
    ...
    if (options.fromPlanPath) {
        ...
        try {
            ...
            if (error && typeof error === 'object' && 'code' in error && error.code === 'ATM_DECOMP_PLAN_INVALID') {
...
function loadCreateMapInputFromSpec(cwd, specPath) {
    ...
    const document = readJsonFile(absoluteSpecPath, 'ATM_MAP_SPEC_INVALID');
...
function validateCreateMapSpecInput(cwd, specPath, absoluteSpecPath) {
    try {
