...
import { HARD_CAUSAL_DEPENDENCY_SEMANTICS, areTaskDependenciesSatisfied, findTaskClaimDependencyBlockers } from '../dependency-gate.js';
 ...
 */
function completeProof(producerOutput, overrides = {}) {
    return {
        producerOutput,
...
function typedConsumer(dependencies) {
    return {
        status: 'ready',
...
// not a whole-task dependency, so the lane starts.
{
    ...
    for (const relation of ['validation', 'publication', 'observation', 'soft-order', 'file-overlap', 'atom-overlap']) {
        const nonHard = findTaskClaimDependencyBlockers(repo, 'TASK-CONSUMER-0406', typedConsumer([{ taskId: 'TASK-DEP-OPEN-0406', relation }]), { claimFiles: CODE_CLAIM_FILES });
...
// the gate is answered by a result rather than by a status word.
{
    ...
    assert(before[0]?.requiredCommand?.includes('dependency-gate.test.ts'), 'the refusal must hand back the declared negative control as its recovery command');
...
{
    const unprovable = findTaskClaimDependencyBlockers(repo, 'TASK-CONSUMER-0406', typedConsumer([
        {
...
{
    const fallback = findTaskClaimDependencyBlockers(repo, 'TASK-CONSUMER-0406', typedConsumer(['TASK-DEP-OPEN-0406']), { claimFiles: CODE_CLAIM_FILES });
    ...
    const legacy = findTaskClaimDependencyBlockers(repo, 'TASK-CONSUMER-0406', { status: 'ready', dependencies: ['TASK-DEP-OPEN-0406'] }, { claimFiles: CODE_CLAIM_FILES });
