...
import { readClaimLaneSessionId } from './claim-ownership.js';
import { runAtmGit } from '../git-governance.js';
export function isConfirmedWipCommitResult(result) {
    const resultRecord = result && typeof result === 'object' ? result : null;
    const evidence = resultRecord?.evidence && typeof resultRecord.evidence === 'object' ? resultRecord.evidence : null;
    const workAdmission = evidence?.workAdmission && typeof evidence.workAdmission === 'object' ? evidence.workAdmission : null;
    const decision = workAdmission?.decision && typeof workAdmission.decision === 'object' ? workAdmission.decision : null;
...
export async function prepareReleaseWip(input) {
    ...
    if (input.dirtyInScopeFiles.length > 0) {
        if (input.discardWip) {
            ...
            for (const file of input.dirtyInScopeFiles) {
                ...
                try {
                    execFileSync('git', ['-C', input.cwd, 'checkout', 'HEAD', '--', relPath], { stdio: 'pipe', encoding: 'utf8' });
                }
                catch {
                    rmSync(path.resolve(input.cwd, relPath), { force: true });
                }
            }
        }
            ...
            const evidence = gitResult?.evidence && typeof gitResult.evidence === 'object' ? gitResult.evidence : null;
            const commitSha = typeof evidence?.commitSha === 'string' ? evidence.commitSha.trim() : '';
            if (!isConfirmedWipCommitResult(gitResult))
                throw new CliError('ATM_RELEASE_DIRTY_WIP_BLOCKED', `WIP preservation for ${input.taskId} did not produce a governed commit SHA; claim release was not applied.`, { exitCode: 1, details: { taskId: input.taskId, actorId: input.actorId, failureKind: 'preservation-commit-not-confirmed', committedFiles: input.dirtyInScopeFiles, gitResult: gitResult?.evidence ?? null } });
    ...
    if (plan.dirtyPathCount > 0) {
        ...
        if (!retainedWip)
            throw new CliError('ATM_RELEASE_DIRTY_WIP_BLOCKED', `Release for ${input.taskId} could not persist retained WIP ownership; claim release was not applied.`, { exitCode: 1, details: { taskId: input.taskId, actorId: input.actorId, failureKind: 'ownership-retention-not-confirmed', dirtyInScopeFiles: input.dirtyInScopeFiles, transition: plan } });
