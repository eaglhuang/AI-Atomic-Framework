...
import { runHelloWorldSmoke } from './test.js';
import { runVerify } from './verify.js';
import { CliError, makeResult, message, parseOptions, relativePathFrom, resolveValue } from './shared.js';
...
export async function runSelfHostAlphaAsync(argv) {
    ...
    try {
        ...
        const selfHostingArtifacts = await materializeSelfHostingArtifacts(sandbox, bootstrapEvidence, helloWorld, neutrality, criteria, ok);
...
async function materializeSelfHostingArtifacts(cwd, bootstrapEvidence, helloWorld, neutrality, criteria, ok) {
    ...
    if (!contextBudgetGuard || !runReportStore || !contextSummaryStore) {
        throw new CliError('ATM_SELF_HOST_ALPHA_STORE_MISSING', 'Required governance stores are not available for self-host-alpha.');
    }
    const budgetEvaluation = await resolveValue(contextBudgetGuard.evaluateBudget({
    ...
    await resolveValue(adapter.stores.artifactStore.writeArtifact({
        workItemId: bootstrapTaskId,
        artifactPath,
        artifactKind: 'report',
    ...
    await resolveValue(adapter.stores.logStore.appendLog(bootstrapTaskId, `${now} self-host-alpha smoke ${helloWorld.ok ? 'passed' : 'failed'} (${helloWorld.passCount}/${helloWorld.total})`));
    await resolveValue(runReportStore.writeRunReport(phaseBReportId, {
    ...
    await resolveValue(adapter.stores.evidenceStore.writeEvidence(bootstrapTaskId, {
        workItemId: bootstrapTaskId,
