...
export function inspectPostComposeSemanticValidation(input) {
    ...
    return makeResult({
        ...
        cwd: input.cwd,
        messages: [
            message(decision.verdict === 'pass' ? 'info' : 'error', decision.code ?? 'ATM_BROKER_POST_COMPOSE_SEMANTIC_VALIDATION_PASSED', summarizeDecision(decision))
        ],
...
function resolveCandidate(input) {
    ...
    try {
        ...
        throw new CliError('ATM_BROKER_COMPOSE_SEMANTIC_VALIDATION_UNAVAILABLE', `Unable to read command-backed post-compose candidate file: ${absolute}`, {
            exitCode: 1,
            details: {
...
function maybeExecuteDeclaredValidators(cwd, candidate) {
    ...
    const executed = [...existing];
    for (const command of commands) {
        if (covered.has(command.validatorId))
            continue;
...
function runOneDeclaredValidator(cwd, command) {
