export function mapConversationPatchDraftsToMachineFindings(report) {
    const findings = [];
...
function mapDraftToMachineFinding(draft) {
...
function checkDraftBlockingConditions(draft) {
    const findings = [];
    ...
    if (draft.findingKind === 'style-format-correction' &&
    ...
    const requiresHumanReview = draft.requiresHumanReview;
    ...
    const reversibility = draft.upgradeProposalDraft?.reversibility;
...
function severityForDraft(draft) {
...
function routeHintForDraft(draft) {
...
function messageForDraft(draft) {
    if (draft.draftKind === 'atom-patch') {
...
function sanitizeFindingSegment(value) {
