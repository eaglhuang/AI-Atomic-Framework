...
export function computeDecisionSnapshotHash(proposal) {
    return `sha256:${createHash('sha256').update(stableStringify(proposal), 'utf8').digest('hex')}`;
}
export function createHumanReviewQueueRecord(proposal, options = {}) {
    const proposalSnapshot = cloneJson(proposal);
    ...
    return options.review
        ? {
            ...record,
            review: cloneJson(options.review)
        }
        : record;
}
export function createHumanReviewQueueDocument(entries, options = {}) {
    return {
...
export function loadHumanReviewQueueDocument(filePath) {
...
export function writeHumanReviewQueueDocument(filePath, document) {
    mkdirSync(path.dirname(filePath), { recursive: true });
    writeFileSync(filePath, `${JSON.stringify(document, null, 2)}\n`, 'utf8');
    return document;
}
export function normalizeHumanReviewQueueDocument(document) {
...
export function findHumanReviewQueueRecord(document, proposalId) {
    return document.entries.find((entry) => entry.proposalId === proposalId) ?? null;
}
export function replaceHumanReviewQueueRecord(document, nextRecord) {
    const entries = document.entries.map((entry) => entry.proposalId === nextRecord.proposalId ? cloneJson(nextRecord) : entry);
    return createHumanReviewQueueDocument(entries, {
        generatedAt: new Date().toISOString(),
        migration: document.migration
    });
}
export function renderHumanReviewQueueMarkdown(document) {
    const lines = [
        '# Upgrade Proposals',
        '',
...
export function validateHumanReviewQueueDocument(document) {
...
export function validateHumanReviewQueueRecord(record) {
    const issues = [];
...
function summarizeAutomatedGates(entry) {
...
function isHumanReviewQueueDocument(value) {
