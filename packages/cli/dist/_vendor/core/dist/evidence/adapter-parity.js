...
export const ADAPTER_PARITY_SCHEMA_ID = 'atm.adapterParityReceipt.v1';
export const EDITORS = ['codex', 'claude-code', 'cursor', 'copilot', 'gemini', 'antigravity'];
...
export function compileAdapterParity(input) {
    ...
    const expectedCompilerDigests = Object.fromEntries(Object.entries(input?.expectedCompilerDigests ?? {}).map(([editor, compilerDigest]) => [editor, normalize(compilerDigest)]));
    const expectedManifestDigests = Object.fromEntries(Object.entries(input?.expectedManifestDigests ?? {}).map(([editor, manifestDigest]) => [editor, normalize(manifestDigest)]));
    ...
    for (const adapter of adapters) {
        ...
        if (!EDITORS.includes(adapter.editor)) {
