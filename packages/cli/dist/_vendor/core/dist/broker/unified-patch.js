...
export function applyUnifiedPatch(before, patchText) {
    ...
    for (const hunk of hunks) {
        ...
        if (start > sourceLines.length) {
            throw new UnifiedPatchApplicationError(`hunk at old line ${hunk.oldStart} starts past the end of a ${sourceLines.length}-line file`);
        }
        ...
        for (const entry of hunk.lines) {
            ...
            if (actual === undefined || actual !== content) {
                throw new UnifiedPatchApplicationError(`patch context mismatch at line ${cursor + 1}: expected ${JSON.stringify(content)}, found ${JSON.stringify(actual ?? null)}`);
            }
            cursor += 1;
            if (marker === ' ')
                output.push(content);
