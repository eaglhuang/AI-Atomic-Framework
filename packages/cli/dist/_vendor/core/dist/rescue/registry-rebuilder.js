...
import { verifyPayloadHash, cidToShortId } from '../registry/atom-capsule.js';
import { loadCapsuleRegistry, saveCapsuleRegistry, getRepoRegistryPath, upsertCapsuleEntry } from '../registry/capsule-registry.js';
import { mapCidToShortId } from '../registry/map-capsule.js';
import { saveMapRegistry, getRepoMapRegistryPath, upsertMapEntry } from '../registry/map-capsule-registry.js';
export function rebuildCapsuleRegistry(repositoryRoot, options = {}) {
    ...
    for (const filename of capsuleFiles) {
        ...
        try {
            ...
            const compressedPayload = content.compressedPayload ?? content.bundle
            ...
            const atomId = content.bundle?.atomId ?? shortId;
...
export function rebuildMapRegistry(repositoryRoot, options = {}) {
    const dryRun = options.dryRun ?? true;
    ...
    const mapFiles = readdirSync(vendorMapsDir).filter((f) => f.endsWith('.json') && f !== 'map-registry.json');
    const rebuiltRegistry = {
    ...
    for (const filename of mapFiles) {
        ...
        try {
            ...
            const relPath = path.relative(repositoryRoot, filePath);
            const mapId = bundle?.mapId ?? shortId;
