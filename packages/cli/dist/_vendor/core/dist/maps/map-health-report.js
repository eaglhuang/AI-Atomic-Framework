...
export function generateMapHealthReport(repositoryRoot, mapId) {
...
function countGitEdits(repositoryRoot, sourcePath) {
    if (!sourcePath)
        return 0;
    try {
...
function countPoliceViolations(repositoryRoot, mapId, atomId) {
        ...
        for (const filename of readdirSync(reportsDir)) {
            ...
            try {
                ...
                for (const f of findings) {
                    if (f.atomId === atomId || f.mapId === mapId)
                        count++;
                }
            }
            catch { /* skip */ }
...
function checkFingerprintDrift(repositoryRoot, mapId, _atomId) {
...
function getLastModified(repositoryRoot, sourcePath) {
    if (!sourcePath)
        return null;
    try {
...
function computeRisk(gitEditCount, policeViolationCount, driftDetected) {
    if (driftDetected || policeViolationCount > 0)
        return 'high';
    if (gitEditCount > 10)
        return 'high';
    if (gitEditCount > 3)
