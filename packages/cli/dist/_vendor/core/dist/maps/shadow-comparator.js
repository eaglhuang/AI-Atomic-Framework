...
function sortedClone(value) {
    ...
    if (value !== null && typeof value === 'object') {
        ...
        for (const key of Object.keys(value).sort()) {
            sorted[key] = sortedClone(value[key]);
        }
        return sorted;
    }
    return value;
...
function diffSummary(legacyOutput, atomOutput) {
...
function computePromotion(consistencyRate, sampleSize, hasCriticalDivergence) {
    const reasons = [];
...
export function runShadowComparison(mapId, samples, options = {}) {
    const shadowPeriodDays = options.shadowPeriodDays ?? 0;
    const criticalFixtureIds = options.criticalFixtureIds ?? new Set();
    let matchCount = 0;
    const divergences = [];
    let totalLegacyMs = 0;
    let totalAtomMs = 0;
    ...
    const { recommendation, reasons } = computePromotion(outputConsistencyRate, sampleSize, hasCriticalDivergence);
    return {
...
export function writeShadowComparisonReport(repositoryRoot, report) {
...
export function readShadowComparisonReport(repositoryRoot, mapId) {
    const filePath = path.join(repositoryRoot, 'atomic_workbench', 'maps', mapId, 'shadow-comparison-report.json');
