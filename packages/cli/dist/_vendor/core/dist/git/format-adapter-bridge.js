...
import { defaultAdapterRegistry, fallbackFileLockAdapter, jsonRecordAdapter, pathToAtomMapAdapter, textRangeAdapter } from '../broker/adapters/index.js';
import { brokerAdapterMigration } from '../broker/types.js';
export function bridgeGitDiffEntriesToAdapterConflictKeys(input) {
...
function bridgeSingleEntry(input) {
...
function buildStructuredBridgeResult(input, filePath) {
    const parsed = input.adapter.parse({ filePath, content: input.parseContent });
    const conflictKeys = dedupeConflictKeys(input.requests.flatMap((request) => input.adapter.getConflictKeys(input.adapter.normalize(request), parsed)));
    return {
...
function failClosedResult(filePath, adapterId, diagnostics) {
...
function selectPreferredAdapter(file, beforeContent, afterContent) {
...
function buildJsonRecordRequests(input) {
    ...
    return (pointers.length > 0 ? pointers : ['']).map((pointer, index) => makeRequest({
        filePath: input.filePath,
...
function buildAtomMapRequests(input) {
    const beforeValue = input.beforeContent ? JSON.parse(input.beforeContent) : {};
    const afterValue = input.afterContent ? JSON.parse(input.afterContent) : {};
    const requests = [];
    const beforeMappings = indexAtomMapRows(beforeValue.mappings);
    const afterMappings = indexAtomMapRows(afterValue.mappings);
    const rowKeys = new Set([...beforeMappings.keys(), ...afterMappings.keys()]);
    let index = 0;
...
function buildTextRangeRequests(input) {
    return input.hunks.map((hunk, index) => makeRequest({
        filePath: input.filePath,
        actorId: input.actorId,
        taskId: input.taskId,
        op: 'replaceRange',
...
function diffJsonPointers(beforeValue, afterValue, pointer = '') {
    ...
    if (isPlainObject(beforeValue) && isPlainObject(afterValue)) {
        ...
        const pointers = [...keys].flatMap((key) => diffJsonPointers(beforeValue[key], afterValue[key], `${pointer}/${escapePointerToken(key)}`));
        return pointers.length > 0 ? pointers : [pointer];
    }
    if (Array.isArray(beforeValue) && Array.isArray(afterValue)) {
        const length = Math.max(beforeValue.length, afterValue.length);
        const pointers = Array.from({ length }, (_, index) => diffJsonPointers(beforeValue[index], afterValue[index], `${pointer}/${index}`)).flat();
        return pointers.length > 0 ? pointers : [pointer];
    }
    return [pointer];
}
function readValueAtPointer(root, pointer) {
    if (pointer === '') {
        return root;
    }
    let current = root;
...
function indexAtomMapRows(value) {
...
function readEntrySnapshots(input) {
...
function readGitBlob(cwd, blobRef, gitExecutable = 'git') {
    try {
...
function makeRequest(input, index = 0) {
    return {
...
function dedupeConflictKeys(keys) {
    const seen = new Set();
    const result = [];
