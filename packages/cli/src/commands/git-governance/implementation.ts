...
function normalizeCommitLaneSessionId(value) {
  return typeof value === "string" && value.trim() ? value.trim() : null;
}
function laneSessionIdFromRecord(value) {
...
function gitPushAttemptStatusRelativePath(actorId, branch, remote) {
  const safeActor = actorId.replace(/[^a-zA-Z0-9_.-]/g, "_");
...
function writeGitPushAttemptStatus(cwd, statusRelativePath, status) {
  try {
    const absolutePath = path.join(cwd, statusRelativePath);
    mkdirSync(path.dirname(absolutePath), { recursive: true });
    writeFileSync(absolutePath, `${JSON.stringify(status, null, 2)}\n`, "utf8");
...
function runGitLease(options) {
  ...
  if (
    ...
    (stagedEntries.length === 0 ||
      stagedEntries.some(
        (entry) => !entry.stagedBlobId || !entry.stagedMode,
      ) ||
...
function buildGitAdmissionComposerInput(options, result) {
  ...
  const remoteRequests = remoteBridge.requests.map((request) =>
...
function assertRecordCommitSingleTaskOwner(cwd, stagedFiles) {
  ...
  const conflictFiles = uniqueSorted(
    conflicts.flatMap((conflict) => conflict.conflictFiles),
  );
  throw new CliError(
...
function runGitRecordCommit(options) {
  ...
  const blockedFiles = stagedFiles.filter(
    (filePath) => !isRecordCommitAllowedPath(filePath),
  );
        ...
        const commitScopedBundle = () => {
          if (scopedCommitFiles.length > 0) {
            return withTaskScopedCommitIndex(
              ...
              scopedCommitFiles,
              actorId,
              (scopedEnv) => runCommit({ ...commitEnv, ...scopedEnv }),
            );
          }
...
function inspectHistoricalLedgerRestoreStagedArtifacts(cwd, taskId) {
  ...
  for (const eventPath of stagedFiles.filter((file) =>
  ...
  if (!hasEvidenceBundle) {
    ...
    const repairEventPaths = stagedFiles.filter((file) =>
      normalizeRelativePath(file)
        .toLowerCase()
        .startsWith(`.atm/history/task-events/${normalizedTaskId}/`),
    );
...
function isFileAllowedInTaskBundle(cwd, filePath, taskId, declaredScope) {
  return isTaskBundleAllowedByPolicy({
    filePath,
    declaredScope,
    allowedGovernanceArtifact: isAllowedGovernanceArtifactPath(
      cwd,
      filePath,
      taskId,
    ),
    ...
    .flatMap((entry) => {
      try {
        ...
        const claimFiles = Array.isArray(claim?.files)
          ...
          : [];
        const ownsPath = [...scopes, ...claimFiles].some((scope) =>
...
function readProtectedOverrideAuditTaskId(cwd, filePath) {
  const normalized = normalizeRelativePath(filePath);
  const lower = normalized.toLowerCase();
  if (
...
function buildProtectedForeignStagedOwnershipFiles(unexpectedStagedTasks) {
  return uniqueSorted(
    unexpectedStagedTasks.flatMap((entry) =>
      entry.stagedFiles.filter((filePath) =>
...
function deferForeignStagedFiles(cwd, taskId, unexpectedStagedTasks) {
  const files = uniqueSorted(
    unexpectedStagedTasks.flatMap((entry) => entry.stagedFiles),
  );
  return deferStagedFilePaths(cwd, taskId, files);
}
function deferStagedFilePaths(cwd, taskId, filesInput) {
  const files = uniqueSorted(
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ schemaId: "atm.foreignStagedSnapshot.v1", taskId, createdAt: new Date().toISOString(), files }, null, 2)}\n`,
    "utf8",
  );
  runGitCommand(
    cwd,
...
function recordGitIndexRestoreFailure(cwd, input) {
...
export function resolveTaskScopedCommitBundle(input) {
  ...
  const protectedForeignDeferAuthorized =
    ...
    unexpectedStagedTasks.every((entry) =>
  ...
  const residueCandidateFiles = dirtyFiles.filter(
    (filePath) => !isIgnorableTaskScopedDirtySideEffect(filePath),
  );
  ...
  const blockedResidue = residueReport.blockAndExplain.filter(
    (entry) =>
  ...
  const manualReviewResidue = residueReport.manualReview.filter(
    (entry) =>
  ...
  const unstagedDirtyFiles = effectiveDirtyFiles.filter(
    (filePath) => !stagedSet.has(filePath),
  );
  ...
  const inScopeStagedFiles = stagedFiles.filter((filePath) =>
  ...
  const commitAttributionStageCandidates = unstagedDirtyFiles.filter(
    (filePath) => isCommitAttributionSideEffectPath(filePath),
  );
    ...
    claimIntent === "closeout-only"
      ? uniqueSorted(
          commitFiles.filter((filePath) => {
            ...
            return declaredScope.some((scope) =>
  ...
  if (unexpectedStagedTasks.length > 0) {
    governanceBundleWarnings.push(
      `Foreign staged tasks remain in the shared index but will be excluded from the governed commit bundle: ${unexpectedStagedTasks.map((entry) => entry.taskId).join(", ")}`,
    );
  }
  if (autoCleanedResidue.length > 0) {
    governanceBundleWarnings.push(
      `ATM auto-cleaned generated residue before governed commit: ${autoCleanedResidue.map((entry) => `${entry.path} (${entry.reason})`).join("; ")}`,
    );
  }
  const deferredGovernanceResidue = residueReport.blockAndExplain.filter(
    (entry) =>
      input.deferForeignStaged &&
      isDeferrableForeignGovernanceResidue(input.taskId, entry),
  );
  if (deferredGovernanceResidue.length > 0) {
    governanceBundleWarnings.push(
      `Deferred foreign generated governance residue outside this task bundle: ${deferredGovernanceResidue.map((entry) => `${entry.path} (${entry.ownerTaskId})`).join("; ")}`,
    );
  }
  return {
...
function inspectTaskScopedUnstagedCommit(cwd, taskId, taskDocument) {
  ...
  const unstagedInScopeDirty = deliverableDirtyFiles.filter(
    (filePath) => !stagedFiles.includes(filePath),
  );
...
function isTaskOwnedProtectedOverrideAuditPath(
  cwd,
  filePath,
  normalizedTaskId,
) {
...
function isFrameworkGeneratedArtifactAllowed(
  filePath,
  claimedFiles,
  releaseGeneratedArtifacts,
) {
