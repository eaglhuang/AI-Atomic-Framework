...
import { shortHash } from './admission-command.ts';

type LegacyValue = ReturnType<typeof JSON.parse>;



export function runGitLease(options: LegacyValue) {
  ...
  if (!options.overrideReason?.trim()) {
    throw new CliError(
      "ATM_CLI_USAGE",
      "git lease requires --reason with the human-approved safety rationale.",
      { exitCode: 2 },
    );
  }
  const phrase =
  ...
  if (
    ...
    throw new CliError(
      ...
      "Stage override lease paths must exactly match the current foreign-active staged index entries.",
      {
        exitCode: 1,
        details: {
          requestedPaths,
  ...
  const lease = {
    ...
    stagedEntries:
      options.leaseKind === "stage-override"
        ? stagedEntries.map((entry: LegacyValue) => ({
            path: entry.path,
            stagedBlobId: entry.stagedBlobId,
            stagedMode: entry.stagedMode,
            stagedState: entry.stagedState,
  ...
  writeFileSync(
    path.join(options.cwd, leasePath),
  ...
  return makeResult({
    ...
    messages: [
      message(
        "info",
        "ATM_GIT_INDEX_OVERRIDE_LEASE_CREATED",
        `Created ${options.leaseKind} Git index override lease.`,
        {
          leaseId,
          leasePath,
