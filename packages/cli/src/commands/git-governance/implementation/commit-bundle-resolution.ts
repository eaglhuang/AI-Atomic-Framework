...
import { cleanupAutoGeneratedResidue, isIgnorableCommitStagingSideEffect, listTaskScopedWorktreeDirtyFiles, readGitNameOnly, resolveTaskDeclaredScope } from './task-scope-staging.ts';
import {
  derivePublicationDeliverySliceManifest,
  resolvePublicationDeliverySlice,
} from '../../../../../core/src/git/publication-delivery-slice.ts';
  ...
  try {
    ...
    if (Array.isArray(evidence?.attestations)) {
      for (const attestation of evidence.attestations) {
        if (typeof attestation?.taskId === 'string') taskIds.add(attestation.taskId.trim().toLowerCase());
      }
    }
    if (Array.isArray(evidence?.tasks)) {
      for (const task of evidence.tasks) {
        if (typeof task?.taskId === 'string') taskIds.add(task.taskId.trim().toLowerCase());
      }
    }
...
function isUncommittableTaskEvidenceArtifact(cwd: string, filePath: string, taskId: string, taskDocument: LegacyValue, terminalHistoryCleanupAllowed = false): boolean {
...
export function resolveTaskScopedCommitBundle(input: LegacyValue) {
  ...
  let deliverySliceRejection: { code: string; reason: string } | null = null;
  let deliverySliceStageFiles: readonly string[] | null = null;
  const hasManifest =
    typeof input.deliverySliceManifestPath === 'string' && input.deliverySliceManifestPath.trim();
  const hasReceipt =
    typeof input.deliverySliceReceiptPath === 'string' && input.deliverySliceReceiptPath.trim();
