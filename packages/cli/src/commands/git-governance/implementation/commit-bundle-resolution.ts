...
export function resolveTaskScopedCommitBundle(input: LegacyValue) {
  ...
  if (typeof input.deliverySliceManifestPath === 'string' && input.deliverySliceManifestPath.trim()) {
    const manifestRel = normalizeRelativePath(input.deliverySliceManifestPath);
    const manifestAbsolute = path.join(input.cwd, manifestRel);
    ...
    const receiptRel = typeof parsedManifest?.receiptPath === 'string'
      ? normalizeRelativePath(parsedManifest.receiptPath)
      : '';
    const receiptAbsolute = receiptRel ? path.join(input.cwd, receiptRel) : '';
    if (!deliverySliceRejection && receiptAbsolute && existsSync(receiptAbsolute)) {
      try {
        ...
        deliverySliceRejection = {
          code: 'ATM_GIT_COMMIT_DELIVERY_SLICE_INVALID',
          reason: `delivery-slice receipt is not valid JSON: ${receiptRel}`,
        };
      }
    }
