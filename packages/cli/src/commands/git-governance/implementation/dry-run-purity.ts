 ...
 */
export function resolveDryRunPurity(request: CommitRequest): boolean {
 ...
 */
export function assertDryRunReachedNoExecutor(
  dryRunPurity: boolean,
  context: { readonly taskId: string | null; readonly usesFrameworkClaimCommit: boolean }
): void {
  if (!dryRunPurity) return;
  throw new CliError(
    'ATM_GIT_COMMIT_DRY_RUN_PURITY_VIOLATED',
    'git commit --dry-run resolved no previewable authority and reached the commit executor; refusing to commit. Re-run with a live task claim or framework claim to obtain a preview.',
    {
      exitCode: 1,
      details: {
        dryRun: true,
