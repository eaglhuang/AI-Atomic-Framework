...
import { inspectCurrentBranchCommitQueueStatus } from './branch-commit-window.ts';

import { laneSessionIdFromRecord } from './command-router.ts';
...
export function resolveActorGitIdentityForCommit(cwd: LegacyValue, actorId: LegacyValue) {
...
export function evaluateGitGovernanceCheck(input: LegacyValue) {
  const cwd = path.resolve(input.cwd);
  const resolvedActor = resolveActorId(input.actorInput ?? undefined, cwd);
  if (!resolvedActor) {
    throw new CliError(
      "ATM_ACTOR_ID_MISSING",
      `git check requires --actor or ${actorIdEnvVar} (legacy alias: AGENT_IDENTITY).`,
      { exitCode: 2 },
    );
  }
...
export function runGitPrepare(options: LegacyValue) {
  ...
  if (!resolvedActor) {
    throw new CliError(
      "ATM_ACTOR_ID_MISSING",
      `git prepare requires --actor or ${actorIdEnvVar} (legacy alias: AGENT_IDENTITY).`,
      { exitCode: 2 },
    );
  }
  ...
  const identityPath =
    options.gitName !== null && options.gitEmail !== null
      ? writePreparedRuntimeIdentity(
          options.cwd,
          actorId,
          nextName,
          nextEmail,
  ...
  return makeResult({
    ...
    messages: [
      message(
        "info",
        "ATM_GIT_PREPARED",
        "Actor git identity has been prepared for the resolved actor.",
        {
          actorId,
          gitName: nextName,
          gitEmail: nextEmail,
...
export function runGitCommitStatus(options: LegacyValue) {
  const resolvedActor = resolveActorId(
    options.actorId ?? undefined,
    options.cwd,
  );
  if (!resolvedActor) {
    throw new CliError(
      "ATM_ACTOR_ID_MISSING",
      "git commit-status requires --actor or ATM_ACTOR_ID.",
      { exitCode: 2 },
    );
  }
  const actorId = resolvedActor.actorId;
  const status = readGitCommitAttemptStatus(
    options.cwd,
    actorId,
    options.taskId,
  );
  ...
  return makeResult({
    ...
    messages: [
      status
        ? message(
            "info",
            "ATM_GIT_COMMIT_STATUS_FOUND",
            `Last known governed commit attempt for ${label} is ${status.status} (phase: ${status.phase}).`,
            { status },
          )
        : message(
            "info",
            "ATM_GIT_COMMIT_STATUS_NOT_FOUND",
            `No recorded governed commit attempt for ${label}.`,
            {},
          ),
      branchQueueStatus.status === "free"
        ? message(
            "info",
            "ATM_GIT_COMMIT_BRANCH_QUEUE_FREE",
            "No active branch commit queue lock is present.",
            { branchQueueStatus },
          )
        : message(
            branchQueueStatus.status === "busy" ? "warning" : "warning",
            branchQueueStatus.status === "busy"
              ? "ATM_GIT_COMMIT_BRANCH_QUEUE_ACTIVE"
              : "ATM_GIT_COMMIT_BRANCH_QUEUE_STALE_OR_DEAD",
      ...
      stdinPathspecGitAddProcesses.length === 0
        ? message(
            "info",
            "ATM_GIT_COMMIT_STDIN_PATHSPEC_ADD_CLEAR",
            "No active git add stdin pathspec helper was detected.",
            {},
          )
        : message(
            "warning",
            "ATM_GIT_COMMIT_STDIN_PATHSPEC_ADD_ACTIVE",
            "An active git add --pathspec-from-file=- helper was detected; this can make a commit appear stuck while the helper waits for stdin.",
            {
              processes: stdinPathspecGitAddProcesses,
              recovery:
                "Terminate only the listed stuck helper after confirming it is not an intentional staging operation, then stage files explicitly with git add -- <paths>.",
            },
          ),
    ],
    evidence: {
...
export function requireExplicitGitActor(resolvedActor: LegacyValue, action: LegacyValue) {
  if (resolvedActor.source !== "repo-default") return;
  const editorHint =
    process.env.ATM_EDITOR_ID?.trim() ||
    (process.env.CODEX_HOME?.trim() ? "codex" : null) ||
    "<editor-id>";
  throw new CliError(
    "ATM_ACTOR_ID_EXPLICIT_REQUIRED",
    `${action} requires an explicit actor. Repo default identity is advisory only and must not be reused across editors or agents.`,
    {
      exitCode: 2,
      details: {
...
export function resolveGitIdentityProfile(cwd: LegacyValue, actorId: LegacyValue, actorRecord: LegacyValue, overrides: LegacyValue = {}) {
...
export function writePreparedRuntimeIdentity(
  ...
  actorId: LegacyValue,
  gitName: LegacyValue,
  gitEmail: LegacyValue,
  actorRecord: LegacyValue,
) {
  const defaultIdentity = readRuntimeIdentityDefault(cwd);
  const existing =
    readRuntimeIdentityForActor(cwd, actorId) ??
...
export function parseTaskClaim(value: LegacyValue) {
  ...
  const stateRaw =
    typeof candidate.state === "string" ? candidate.state.trim() : "active";
  const state =
    stateRaw === "released" ||
    stateRaw === "handoff" ||
    stateRaw === "taken_over"
...
export function resolveGitGovernanceSession(cwd: LegacyValue, input: LegacyValue) {
...
export function requireTrailerValue(trailers: LegacyValue, key: LegacyValue, expectedValue: LegacyValue, violations: LegacyValue, code: LegacyValue) {
