...
export function runDoctor(argv) {
  ...
  const hasTsNoCheck = listFiles(path.join(root, 'packages'))
    ...
    .filter((filePath) => !filePath.endsWith(path.join('packages', 'cli', 'src', 'commands', 'doctor.mjs')))
    .some((filePath) => /^\s*\/\/\s*@ts-nocheck\b/m.test(readFileSync(filePath, 'utf8')));
  ...
  const checks = [
    ...
    createCheck('self-host-alpha-entry', !frameworkContractExpected || (existsSync(path.join(root, 'packages/cli/src/commands/self-host-alpha.mjs')) && existsSync(path.join(root, 'docs/SELF_HOSTING_ALPHA.md'))), { command: 'packages/cli/src/commands/self-host-alpha.mjs', doc: 'docs/SELF_HOSTING_ALPHA.md' }),
...
function isFrameworkContractExpected(root, rootPackage) {
