...
import {
  ...
  clearDaemonPid,
  isProcessRunning,
  readDaemonNotifications,
  getDaemonDir
} from '../../../core/src/daemon/daemon-config.ts';
import { buildDefaultWatchPaths } from '../../../core/src/daemon/daemon-watcher.ts';
...
function parseDaemonArgs(argv: string[]): DaemonOptions {
...
export async function runDaemon(argv: string[]) {
  ...
  if (options.action === '_watcher') {
    const { startDaemonWatcher, buildDefaultWatchPaths: bwp } = await import('../../../core/src/daemon/daemon-watcher.ts');
  ...
  if (!KNOWN_DAEMON_ACTIONS.includes(options.action)) {
    throw new CliError('ATM_CLI_USAGE',
      `daemon subcommand "${options.action}" not recognized. Valid: ${KNOWN_DAEMON_ACTIONS.filter((a) => a !== '_watcher').join(', ')}`,
      { exitCode: 2 }
    );
  }
...
function runDaemonEnable(options: DaemonOptions) {
  ...
  return makeResult({
    ...
    cwd: options.cwd,
    messages: [
      message('info', 'ATM_DAEMON_ENABLED',
        'Daemon enabled. Use `atm daemon start` to start the background watcher.',
        {
          enabled: true,
...
function runDaemonDisable(options: DaemonOptions) {
  ...
  return makeResult({
    ...
    cwd: options.cwd,
    messages: [
      message('info', 'ATM_DAEMON_DISABLED',
        'Daemon disabled and kill switch set. `atm daemon start` will be refused until re-enabled.',
        { enabled: false }
      )
    ],
...
function runDaemonStart(options: DaemonOptions) {
  ...
  if (!isDaemonEnabled(options.cwd)) {
    return makeResult({
      ...
      cwd: options.cwd,
      messages: [
        message('error', 'ATM_DAEMON_NOT_ENABLED',
          'Daemon is not enabled. Run `atm daemon enable` first.',
          { hint: 'Run `node atm.mjs daemon enable --json` to opt in.' }
        )
      ],
      evidence: { enabled: false }
    });
  ...
  if (!rescueReport.healthy) {
    return makeResult({
      ...
      cwd: options.cwd,
      messages: [
        message('error', 'ATM_DAEMON_RESCUE_BLOCKED',
          `Daemon start refused: ${rescueReport.blockingFindings.length} Rescue Police finding(s). Resolve with \`atm rescue police\` first.`,
          {
  ...
  if (existingPid && isProcessRunning(existingPid.pid)) {
    return makeResult({
      ...
      cwd: options.cwd,
      messages: [
        message('error', 'ATM_DAEMON_ALREADY_RUNNING',
          `Daemon is already running with PID ${existingPid.pid}.`,
          { pid: existingPid.pid, startedAt: existingPid.startedAt }
        )
      ],
  ...
  if (!atmEntry) {
    throw new CliError('ATM_DAEMON_SPAWN_FAILED',
      'Cannot locate atm.mjs entry point to spawn daemon process.',
      { exitCode: 1 }
    );
  }
  ...
  return makeResult({
    ...
    cwd: options.cwd,
    messages: [
      message('info', 'ATM_DAEMON_STARTED',
        `Daemon started (PID ${child.pid ?? 'unknown'}). Watching ${watchPaths.length} path(s).`,
        { pid: child.pid, watchPaths, status: 'running' }
      )
    ],
...
function runDaemonStop(options: DaemonOptions) {
  const pidRecord = readDaemonPid(options.cwd);
  if (!pidRecord) {
    return makeResult({
      ok: true,
  ...
  try {
    ...
    return makeResult({
      ...
      cwd: options.cwd,
      messages: [
        message('info', 'ATM_DAEMON_STOPPED',
          `Daemon (PID ${pidRecord.pid}) stopped.`,
          { pid: pidRecord.pid }
        )
      ],
    ...
    throw new CliError('ATM_DAEMON_STOP_FAILED',
      `Failed to stop daemon PID ${pidRecord.pid}: ${err}`,
      { exitCode: 1, details: { pid: pidRecord.pid } }
    );
  }
}

function runDaemonStatus(options: DaemonOptions) {
  ...
  return makeResult({
    ...
    cwd: options.cwd,
    messages: [
      message('info', 'ATM_DAEMON_STATUS',
        isRunning
...
function runDaemonLog(options: DaemonOptions) {
  ...
  return makeResult({
    ...
    cwd: options.cwd,
    messages: [
      message('info', 'ATM_DAEMON_LOG',
        `${notifications.length} notification(s) found.`,
        { count: notifications.length, tail: options.tail ?? 20 }
      )
    ],
...
function resolveAtmEntry(): string | null {
