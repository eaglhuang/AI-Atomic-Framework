...
async function testProviderExecutionUsesGlobalDefaultsForAllRoles() {
  ...
  const execution = await runTeamProviderExecution({
    ...
    runtimeContract: {
      ...
      workerAdapter: {} as any,
      artifactHandoff: {} as any,
      retryBudget: {} as any,
      commitLane: {} as any,
      brokerSubagent: {} as any,
      editorSubagentBridge: {} as any
    },
    runtimePilot: { schemaId: 'atm.teamRuntimePilot.v1', mode: 'direct-provider' } as any,
...
async function testTeamStartExecuteWithGlobalDefaultsPopulatesRoles() {
  ...
  const evidence = started.evidence as {
    providerOrchestration?: { results?: unknown[]; blockedReason?: string | null };
    runtimeContract?: { runtimeMode?: string; selectionReason?: string };
  };
