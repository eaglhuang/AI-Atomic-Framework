...
import {
  coordinatorExclusivePermissions,
  readOnlyTeamRoles,
...
export function normalizeTeamLeasePath(value: string, repoRoot?: string) {
...
export function normalizeRepoAbsoluteLeasePath(rawPath: string, repoRoot?: string) {
...
export function deriveAllowedWriteScope(task: Record<string, unknown> | null | undefined, repoRoot?: string) {
...
export function normalizeTaskWriteScope(paths: string[], repoRoot?: string) {
  return uniqueStrings(paths.map((entry) => normalizeTeamLeasePath(entry, repoRoot)).filter(Boolean));
}

export function mergeValidation(...reports: { ok: boolean; findings: PermissionFinding[] }[]) {
...
export function buildSuggestedPermissionLeases(recipe: TeamRecipe, writePaths: string[], options: TeamPermissionValidationOptions = {}): PermissionLease[] {
