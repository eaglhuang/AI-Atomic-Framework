...
import { resolveTeamRuntimeProviderSelection } from '../role-provider-resolution.ts';
import {
...
import type {
  PermissionLease,
  ReviewerIdentity,
  TeamBrokerSubagentContract,
  TeamClosureAttestationInput,
  TeamCommitLaneContract,
...
export function normalizeTeamRuntimeMode(value: unknown): TeamRuntimeMode {
...
export function normalizeOptionalRuntimeString(value: unknown): string | null {
  const normalized = String(value ?? '').trim();
  return normalized.length > 0 ? normalized : null;
}

export function buildCliGlobalProviderDefault(options: Record<string, unknown>): Partial<TeamRoleProviderOverride> | null {
