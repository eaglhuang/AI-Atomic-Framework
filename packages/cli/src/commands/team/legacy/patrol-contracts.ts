...
interface PatrolRun {
  ...
  brokerSubagent?: PatrolBrokerSubagent;
  runtimeContract?: {
    brokerSubagent?: PatrolBrokerSubagent;
    commitLane?: PatrolCommitLane;
    artifactHandoff?: PatrolArtifactHandoff;
  };
  commitLane?: PatrolCommitLane;
  artifactHandoff?: PatrolArtifactHandoff;
  reworkRoute?: {
...
type PatrolCommitLane = {
  serializedBy?: string;
  ownerRole?: string;
  workerGitWrite?: boolean;
};

type PatrolArtifactHandoff = {
  findings?: Array<{ blocking?: boolean; summary?: string; role?: string; agentId?: string; artifact?: string }>;
};

export function buildTeamRunPatrolFindings(
  teamRun: Record<string, unknown> | null | undefined,
  input: { taskId: string; mode: TeamPatrolMode }
): TeamPatrolFinding[] {
...
export function summarizePatrolSeverity(findings: TeamPatrolFinding[]): TeamPatrolFindingLevel {
...
export function suggestedPatrolCommand(taskId: string, mode: TeamPatrolMode, severity: TeamPatrolFindingLevel) {
  if (severity === 'blocker') {
...
export function buildTeamPatrolFollowUp(taskId: string, mode: TeamPatrolMode, findings: TeamPatrolFinding[]) {
...
function buildBrokerSubagentPatrolFindings(run: PatrolRun, brokerSubagent: PatrolBrokerSubagent): TeamPatrolFinding[] {
  const findings: TeamPatrolFinding[] = [];
...
export function teamPatrolFinding(input: TeamPatrolFinding): TeamPatrolFinding {
  return input;
}
