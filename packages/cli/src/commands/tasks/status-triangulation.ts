...
export function readLastTransitionEventRecord(
  cwd: string,
  taskId: string,
 ...
 */
export function readScopeAmendmentEvents(cwd: string, taskId: string): TaskScopeAmendmentSnapshot[] {
  const policy = readTaskLedgerPolicy(cwd);
    ...
    try {
      ...
      snapshots.push({
        ...
        amendmentMode: meta?.amendmentMode === 'normal' || meta?.amendmentMode === 'repair'
          ? (meta.amendmentMode as 'normal' | 'repair')
          : null,
...
export function isOpenPlanningParityStatus(status: string | null): boolean {
  if (!status) return false;
  return ['draft', 'planned', 'open', 'ready', 'running', 'in_progress'].includes(status);
}

export function hasOnlyStatusDivergence(divergence: readonly TaskResidueDivergence[]): boolean {
...
export function buildPlanningMirrorParityOverride(
  input: PlanningMirrorParityOverrideInput
): PlanningMirrorParityOverride | null {
 ...
 */
export function buildTaskStatusTriangulation(
  cwd: string,
  taskId: string,
  taskDocument: Record<string, unknown>
): TaskStatusTriangulation {
