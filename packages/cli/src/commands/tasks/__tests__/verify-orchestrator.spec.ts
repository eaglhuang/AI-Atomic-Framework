...
try {
  ...
  const report = (result.evidence as { report?: { findings?: readonly { code: string }[] } })?.report;
...
try {
  ...
  const report = (result.evidence as { report: { findings: readonly { code: string; level: string }[] } }).report;
