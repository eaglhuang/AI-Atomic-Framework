...
function writePlan(root: string, taskId: string, title: string) {
...
try {
  ...
  await assert.rejects(
    () => runTasksImport(['--cwd', root, '--from', activeClaimPlan, '--write', '--force', '--json']),
    (error) => error instanceof CliError && error.code === 'ATM_EMERGENCY_LANE_APPROVAL_REQUIRED',
    'same-task active claim force import must still require protected approval'
  );
} finally {
