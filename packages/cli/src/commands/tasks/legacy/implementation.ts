...
import { readActiveBatchRun, isPathAllowedByScope } from '../work-channels.ts';
import {
...
import { runTasksClaimLifecycle as delegatedRunTasksClaimLifecycle } from './claim-orchestrator.ts';
import { runTasksRepairClaim } from './repair-claim-orchestrator.ts';
import { prepareTaskForClaim as delegatedPrepareTaskForClaim } from './claim-preparation.ts';
import { runTasksRepairClosure } from './repairclose-orchestrator.ts';
import { runTasksReconcile } from './reconcile-orchestrator.ts';
import { runTasksDeliverAndClose as delegatedRunTasksDeliverAndClose } from './deliver-close-orchestrator.ts';
export { runTasksClose, runTasksImport, runTasksVerify };
export { runTasksClaimLifecycle } from './claim-orchestrator.ts';
import { classifyResetOpenImport, type TaskImportResetOpenClassification } from './import-verify.ts';
...
import { parseClaimRecord, createClaimRecord, isClaimExpired, listRuntimeLockTaskIds } from './task-ledger-readers.ts';
import { isFrontmatterScalar as delegatedIsFrontmatterScalar } from './is-frontmatter-scalar-helper.ts';
import { normalizeStringValue as delegatedNormalizeStringValue } from './normalize-string-value-helper.ts';
import { normalizeTaskDocumentId as delegatedNormalizeTaskDocumentId } from './normalize-task-document-id-helper.ts';
import { sha256 as delegatedSha256 } from './sha256-helper.ts';
import {
  ...
  buildScopeAmendmentCommand as delegatedBuildScopeAmendmentCommand,
...
} from './task-git-helpers.ts';
import { readCloseWindowStagedIndexLockReport } from './close-window-lock.ts';
// TASK-RFT-0013: close-helper cluster split.
import {
  readDeferredForeignStagedFilesForActiveCloseWindow as delegatedReadDeferredForeignStagedFilesForActiveCloseWindow,
  evaluateFrameworkDeliveryWindow as delegatedEvaluateFrameworkDeliveryWindow,
...
import {
  extractTaskCloseDeclaredFiles as delegatedExtractTaskCloseDeclaredFiles,
  extractTaskDeliverableFiles as delegatedExtractTaskDeliverableFiles,
  taskDeliveryPrincipleText as delegatedTaskDeliveryPrincipleText,
  evaluateTaskDeliverableGate as delegatedEvaluateTaskDeliverableGate,
  stageTaskCloseArtifacts as delegatedStageTaskCloseArtifacts,
  existingTaskCloseArtifacts as delegatedExistingTaskCloseArtifacts
} from './close-helpers/close-artifact-staging.ts';
import {
  writeTaskDocumentWithTransition as delegatedWriteTaskDocumentWithTransition
...
} from './task-option-parsers.ts';
import {
  type TaskImportSource as TaskImportSourceContract,
  type TaskCardImportDiagnostic as TaskCardImportDiagnosticContract,
  type TaskImportRecord as TaskImportRecordContract,
  type TaskImportStatus as TaskImportStatusContract,
  type TaskImportManifest as TaskImportManifestContract,
  type TaskDeliverableGateReport as TaskDeliverableGateReportContract,
  type TaskImportDiagnostic as TaskImportDiagnosticContract,
  type TaskVerifyReport as TaskVerifyReportContract,
  type TaskLegacyLedgerMigrationReport as TaskLegacyLedgerMigrationReportContract,
  type TaskLegacyLedgerMigrationEntry as TaskLegacyLedgerMigrationEntryContract,
  type TaskLegacyLedgerMigrationSkip as TaskLegacyLedgerMigrationSkipContract
} from './result-contracts.ts';
import {
  buildTaskStatusTriangulation as buildTaskStatusTriangulationDelegated,
  readScopeAmendmentEvents as readScopeAmendmentEventsDelegated,
  readLastTransitionEventRecord as readLastTransitionEventRecordDelegated,
  resolvePlanningCardPath as resolvePlanningCardPathDelegated
} from './status-triangulation.ts';
import {
  recordStaleRunnerOverride as recordStaleRunnerOverrideDelegated,
...
// `./tasks/result-contracts.ts` directly.
// ---------------------------------------------------------------------------
...
async function runTasksDeliverAndClose(argv: string[]): Promise<CommandResult> {
  return delegatedRunTasksDeliverAndClose(argv, { runTasks });
}
...
export function prepareTaskForClaim(input: {
  ...
  return delegatedPrepareTaskForClaim({
    ...input,
    parseSingleCard,
    writeTaskFiles,
    writeImportEvidence
...
async function runTasksScopeAdd(argv: string[]) {
  ...
  const taskPath = taskPathFor(options.cwd, options.taskId);
  const amendmentMetadata: ScopeAmendmentMetadata = {
 ...
 */
function runTasksScopeRepair(argv: string[]) {
...
async function cleanupTaskLock(input: {
  ...
  let taskLedgerTransitionPath: string | null = null;
  if (taskDocument) {
...
async function runTasksClaimLifecycle(action: 'claim' | 'renew' | 'release' | 'handoff' | 'takeover', argv: string[]) {
  return delegatedRunTasksClaimLifecycle(action, argv);
}
