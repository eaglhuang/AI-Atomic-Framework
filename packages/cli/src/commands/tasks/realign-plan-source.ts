...
import { runTasksSealPlanSource } from './seal-plan-source.ts';
...
export function parseRealignMapFile(mapPath: string): readonly RealignPathMapping[] {
...
function parseMappingEntries(entries: readonly unknown[]): readonly RealignPathMapping[] {
...
export function parseRealignPlanSourceArgv(argv: readonly string[]): RealignPlanSourceOptions {
  let cwd = process.cwd();
  let mapPath: string | null = null;
  let dryRun = false;
  let write = false;
  let actorId: string | null = null;
  let planningRepoRoot: string | null = null;
  let json = false;
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--help' || token === '-h') {
      throw new CliError(
        'ATM_CLI_USAGE',
        'tasks realign-plan-source --map <from-to.json> [--dry-run|--write] [--actor <id>] [--planning-repo <path>] [--json]',
        { exitCode: 2 }
      );
    }
...
function listTaskLedgerFiles(cwd: string): readonly string[] {
...
function readSeal(taskDocument: Record<string, unknown>): PlanningSourceSeal | null {
  const source = taskDocument.source;
  if (!source || typeof source !== 'object' || Array.isArray(source)) return null;
  const seal = (source as Record<string, unknown>).planningSourceSeal;
  ...
  if (typeof record.taskCardPath !== 'string' || typeof record.contentDigest !== 'string') return null;
  return {
...
function resolveMappedPath(planPath: string, mappings: readonly RealignPathMapping[]): string | null {
...
function snapshotProtectedFields(taskDocument: Record<string, unknown>): Record<string, unknown> {
...
function assertProtectedFieldsUnchanged(
  ...
  for (const field of PROTECTED_LIFECYCLE_FIELDS) {
    ...
    if (left !== right) {
      throw new CliError(
        'ATM_TASKS_REALIGN_PROTECTED_FIELD_MUTATION',
        `tasks realign-plan-source refused to mutate protected lifecycle field: ${field}.`,
        { exitCode: 1, details: { field, before: before[field] ?? null, after: after[field] ?? null } }
      );
    }
...
export function buildRealignProposals(input: {
  ...
  for (const taskPath of listTaskLedgerFiles(input.cwd)) {
    ...
    if (!existsSync(absoluteNew)) {
      throw new CliError(
        'ATM_TASKS_REALIGN_TARGET_MISSING',
        `Mapped planning card is missing for ${taskId}: ${mapped}`,
        { exitCode: 1, details: { taskId, mapped, absoluteNew } }
      );
    }
...
function applyRealignToDocument(
  taskDocument: Record<string, unknown>,
  proposal: RealignProposal,
  planningCwd: string
  ...
  const nextSeal = buildPlanningSourceSeal({
    ...
    sealedAt: typeof (source.planningSourceSeal as { sealedAt?: string } | undefined)?.sealedAt === 'string'
      ? (source.planningSourceSeal as { sealedAt: string }).sealedAt
...
export function assertCommitContainsPaths(input: {
  ...
  if (missing.length > 0) {
    throw new CliError(
      ...
      `Commit ${input.commitSha} dropped explicitly staged record file(s): ${missing.join(', ')}.`,
      {
        exitCode: 1,
        details: {
...
export async function runTasksRealignPlanSource(argv: string[]): Promise<CommandResult> {
  if (argv.includes('--seal-plan-source')) return runTasksSealPlanSource(argv.filter((value) => value !== '--seal-plan-source'));
  ...
  if (refusals.length > 0 && options.write) {
    throw new CliError(
      'ATM_TASKS_REALIGN_DIGEST_MISMATCH',
      `tasks realign-plan-source refused ${refusals.length} digest-mismatched task(s).`,
      { exitCode: 1, details: { refusals } }
    );
  }
  ...
  for (const proposal of realignable) {
    ...
    const current = JSON.parse(readFileSync(absoluteTaskPath, 'utf8')) as Record<string, unknown>;
