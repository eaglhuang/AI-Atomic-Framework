...
interface DoOptions {
  cwd: string;
  taskId?: string;
  action: 'start' | 'complete' | 'status';
  evidencePath?: string;
  dryRun: boolean;
  actor?: string;
}

function parseDoArgs(argv: string[]): DoOptions {
  const cwd = process.cwd();
  let taskId: string | undefined;
  let evidencePath: string | undefined;
  let dryRun = false;
  let actor: string | undefined;
  let action: 'start' | 'complete' | 'status' = 'start';
...
export async function runDo(argv: string[]) {
...
function getTaskDocument(cwd: string, taskId: string): Record<string, unknown> | null {
...
function getTaskStatus(doc: Record<string, unknown> | null): string {
  return doc ? String(doc.status ?? 'unknown') : 'not-found';
}

async function runDoStart(options: DoOptions) {
  ...
  if (currentStatus === 'running') {
    return makeResult({
      ...
      cwd,
      messages: [
        message('info', 'ATM_DO_ALREADY_CLAIMED',
          `Task ${taskId} is already in running state. Continuing.`,
          {
            taskId,
            phase: 'claimed',
  ...
  if (currentStatus === 'reserved' || currentStatus === 'ready') {
    ...
    if (owner && owner !== actorId) {
      return makeResult({
        ...
        cwd,
        messages: [
          message('error', 'ATM_DO_TASK_LOCKED',
            `Task ${taskId} is locked by ${owner}.`,
            {
              taskId,
              lockOwner: owner,
  ...
  const blockedBy: string[] = Array.isArray(doc?.blockedBy)
    ? doc!.blockedBy as string[]
    : Array.isArray(doc?.dependencies)
    ? (doc!.dependencies as string[])
    : [];
  ...
  if (blockingTasks.length > 0) {
    return makeResult({
      ...
      cwd,
      messages: [
        message('error', 'ATM_DO_TASK_BLOCKED',
          `Task ${taskId} is blocked by ${blockingTasks.length} incomplete dependency task(s).`,
  ...
  if (dryRun) {
    return makeResult({
      ...
      cwd,
      messages: [
        message('info', 'ATM_DO_DRY_RUN',
          `Dry-run: would execute reserve → promote → claim for task ${taskId}.`,
          {
            taskId,
            actorId,
  ...
  if (failedStep) {
    return makeResult({
      ...
      cwd,
      messages: [
        message('error', 'ATM_DO_STEP_FAILED',
          `Task ${taskId} do-start failed at step "${failedStep}". State rolled back.`,
          { taskId, failedStep, failureReason, stepsCompleted, phase: 'rolled-back-to-pristine' }
        )
      ],
  ...
  return makeResult({
    ...
    cwd,
    messages: [
      message('info', 'ATM_DO_STARTED',
        `Task ${taskId} is now claimed and ready to work on.`,
        {
          taskId,
          actorId,
          phase: 'claimed',
...
async function runDoComplete(options: DoOptions) {
  ...
  if (!existsSync(resolvedEvidence)) {
    throw new CliError('ATM_DO_EVIDENCE_NOT_FOUND',
      `Evidence file not found: ${evidencePath}`,
      { exitCode: 2, details: { evidencePath } }
    );
  }

  // Validate evidence _isValid
  let evidenceContent: Record<string, unknown>;
  try {
  ...
  if (evidenceContent.evidenceType === 'diff-as-evidence' && evidenceContent._isValid !== true) {
    throw new CliError('ATM_DO_EVIDENCE_NOT_VALID',
      `Evidence _isValid is false. Fill in intent, impact, and testCoverage before completing.`,
      {
        exitCode: 1,
  ...
  if (dryRun) {
    return makeResult({
      ...
      cwd,
      messages: [
        message('info', 'ATM_DO_COMPLETE_DRY_RUN',
          `Dry-run: would close task ${taskId} as done with evidence from ${evidencePath}.`,
          { taskId, actorId, evidencePath }
        )
      ],
  ...
  return makeResult({
    ...
    cwd,
    messages: [
      message('info', 'ATM_DO_COMPLETED',
        `Task ${taskId} closed as done.`,
        { taskId, actorId, evidencePath, phase: 'closed', closedAt: new Date().toISOString() }
      )
    ],
...
function runDoStatus(options: DoOptions) {
  const { cwd } = options;
  ...
  const activeTasks: Array<{ taskId: string; phase: string; claimedAt?: string; owner?: string }> = [];

  for (const filename of readdirSync(tasksDir)) {
    if (!filename.endsWith('.json')) continue;
    try {
      const doc = JSON.parse(readFileSync(path.join(tasksDir, filename), 'utf-8')) as Record<string, unknown>;
  ...
  return makeResult({
    ...
    cwd,
    messages: [
      message('info', 'ATM_DO_STATUS',
        `${activeTasks.length} active task(s) in progress.`,
        { activeTasks }
      )
    ],
    evidence: { activeTasks }
