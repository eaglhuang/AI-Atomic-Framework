...
interface RawGitMutationFinding {
  readonly action: string;
  readonly args: readonly string[];
  readonly riskLevel: RawGitMutationRiskLevel;
  readonly requiredCommand: string;
  readonly overridePolicy: {
    readonly chatTextAccepted: false;
    readonly stageOnlyPhrase: string;
...
function classifyRawGitMutationCommand(command: string | null): RawGitMutationFinding | null {
  ...
  if (isReadOnlyGitAction(action)) return null;
  if (!isGuardedRawGitAction(action)) return null;
...
function tokenizeShellLike(command: string | null): readonly string[] {
...
function isReadOnlyGitAction(action: string): boolean {
  return [
    'status',
    'diff',
    'log',
...
function isGuardedRawGitAction(action: string): boolean {
  return [
    'add',
    'restore',
    'reset',
...
function classifyRawGitRisk(action: string, args: readonly string[]): RawGitMutationRiskLevel {
