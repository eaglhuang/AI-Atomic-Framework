...
import { withCloseTransactionMutex } from './close-transaction-mutex.ts';
...
function taskflowSealManifestPath(taskId: string): string {
  return `.atm/history/evidence/${taskId}.seal-and-commit.json`;
}

function readPayloadEntries(repoRoot: string | null, stageFiles: readonly string[], excludeFiles: readonly string[] = []) {
  if (!repoRoot) return [];
...
function payloadDigest(repoRoot: string | null, stageFiles: readonly string[], excludeFiles: readonly string[] = []): string {
  return sha256Json(readPayloadEntries(repoRoot, stageFiles, excludeFiles));
}

function evidenceDigest(repoRoot: string | null, stageFiles: readonly string[], excludeFiles: readonly string[] = []): string {
  return payloadDigest(
    repoRoot,
    stageFiles.filter((file) => file.startsWith('.atm/history/evidence/')),
    excludeFiles
  );
}
...
function buildSealAndCommitReceipt(input: {
  ...
  targetStageFiles: readonly string[];
  planningRepoRoot: string | null;
  planningStageFiles: readonly string[];
  historicalDeliveryRefs: readonly string[];
  historicalBatchRef?: string | null;
  createdAt?: string;
}): TaskflowSealAndCommitReceipt {
  ...
  const withoutSeal = {
    schemaId: 'atm.taskflowSealAndCommitReceipt.v1' as const,
    taskId: input.taskId,
...
function writeSealAndCommitReceipt(repoRoot: string | null, receipt: TaskflowSealAndCommitReceipt): TaskflowSealAndCommitReceipt {
  if (!repoRoot) return receipt;
...
function appendSealTrailers(messageText: string, receipt: TaskflowSealAndCommitReceipt, surface: 'target' | 'planning' | 'shared'): string {
  return [
    messageText,
    '',
...
function isSameRepoBundle(bundle: TaskflowGovernedCommitBundle): boolean {
  if (!bundle.targetRepo.repoRoot || !bundle.planningRepo.repoRoot) return false;
  return path.resolve(bundle.targetRepo.repoRoot) === path.resolve(bundle.planningRepo.repoRoot);
}

function buildSharedRepoBundle(repoRoot: string, stageFiles: readonly string[]): TaskflowCommitRepoBundle {
  return {
    repoRoot,
...
function buildIndexIsolation(repo: TaskflowCommitRepoBundle, stagedFiles: string[], taskId?: string | null): TaskflowIndexIsolation {
