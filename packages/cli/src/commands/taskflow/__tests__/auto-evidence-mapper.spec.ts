...
import { mapAutoEvidenceCommand } from '../auto-evidence-mapper.ts';
...
const knownMapping = mapAutoEvidenceCommand(
...
const knownWithArgsMapping = mapAutoEvidenceCommand(
...
const mismatchedTail = mapAutoEvidenceCommand(
...
const unknownMapping = mapAutoEvidenceCommand(
  'node --strip-types scripts/validate-newthing.ts',
  { scripts: {} }
);
...
const nullPkgMapping = mapAutoEvidenceCommand(
  'node --strip-types scripts/validate-anything.ts',
  null
);
...
const malformedMapping = mapAutoEvidenceCommand(
  'this-is-not-a-node-invocation',
  knownPkg
);
