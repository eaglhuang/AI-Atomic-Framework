...
assert.ok(
  branchQueueDryRun.evidence.writeReadinessHint.blockers.some((entry: any) => entry.code === 'ATM_TASKFLOW_CLOSE_BRANCH_COMMIT_QUEUE_BUSY'),
  'focused branch queue regression must block close before commit tail'
);
