import { generateMapHealthReport } from '../../../core/src/maps/map-health-report.ts';
import { CliError, makeResult, message, parseOptions } from './shared.ts';

export function runHealthReport(argv: string[]) {
  ...
  return makeResult({
    ...
    messages: [
      message(
        ok ? 'info' : 'warn',
        ok ? 'ATM_HEALTH_REPORT_OK' : 'ATM_HEALTH_REPORT_HIGH_RISK',
        ok
          ? `Map health report generated: ${report.atomCount} atoms, ${report.edgeCount} edges, no high-risk atoms.`
          : `Map health report: ${highRiskAtoms.length} high-risk atom(s) detected out of ${report.atomCount}.`,
        { mapId: options.map, highRiskCount: highRiskAtoms.length }
      )
    ],
    evidence: {
      mapId: report.mapId,
