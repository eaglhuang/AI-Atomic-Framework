...
function parseActorArgs(argv: string[]): ParsedActorOptions {
  const state = {
    cwd: process.cwd(),
    action: null as ParsedActorOptions['action'] | null,
...
function requireResolvedActor(inputActorId: string | null): ResolvedActorId {
...
function readGitLocalConfig(cwd: string, key: 'user.name' | 'user.email'): string | null {
  try {
