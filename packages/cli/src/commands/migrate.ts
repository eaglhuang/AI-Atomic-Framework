...
import { CliError, makeResult, makeHelpResult, message, parseArgsForCommand } from './shared.ts';
import { getCommandSpec } from './command-specs.ts';
import { resolveCodemodsForMigration } from '../migration/codemod-registry.ts';

const migrationIndexPath = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  '../migration/migration-index.json'
);
...
function loadMigrationIndex(): MigrationIndex {
  const raw = readFileSync(migrationIndexPath, 'utf8');
  return JSON.parse(raw) as MigrationIndex;
}

function findMigrationEntry(idx: MigrationIndex, fromVersion: string, toVersion: string): MigrationEntry | undefined {
  ...
  for (const rel of workspaceFiles) {
    for (const codemod of codemods) {
      ...
      if (transformed !== null && !affectedFiles.includes(rel)) {
        ...
        if (isUserModified(rel, cwd)) userModifiedFiles.push(rel);
      }
    }
  }
...
function runVerify(options: Record<string, unknown>, cwd: string, root: string): object {
  ...
  if (fixturePath) {
    ...
    for (const rel of beforeFiles) {
      ...
      if (!existsSync(afterFilePath)) { failures.push(`after/${rel} not found`); continue; }
      const afterContent = readFileSync(afterFilePath, 'utf8');
      let transformed = beforeContent;
      for (const codemod of codemods) {
        const r = codemod.apply(transformed, rel);
        if (r !== null) transformed = r;
      }
      if (transformed !== afterContent) failures.push(`${rel}: codemod output does not match after/ expected content`);
    }
