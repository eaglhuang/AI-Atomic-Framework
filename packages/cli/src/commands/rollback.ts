...
export async function runRollback(argv: any) {
  ...
  const targetAtomId = options.targetKind === 'atom' ? options.atomId ?? undefined : undefined;
  const targetMapId = options.targetKind === 'map' ? options.mapId ?? undefined : undefined;
  const targetWorkItemId = options.targetKind === 'atom' ? targetAtomId : targetMapId;

  if (!targetWorkItemId) {
    throw new CliError('ATM_CLI_USAGE', `rollback --target ${options.targetKind} requires an explicit target id`, { exitCode: 2 });
  }
  ...
  const rollbackResult = applyRegistryRollback({
    ...
    mapOwner: options.mapOwner ?? undefined,
  ...
  const evidence = {
    ...
    evidenceKind: 'validation' as const,
    evidenceType: 'rollback-proof' as const,
    summary: `Rollback ${rollbackResult.proof.verificationStatus} for ${options.targetKind} ${targetWorkItemId}.`,
    artifactPaths: [
    ...
    reproducibility: {
      replayable: true,
      replayCommand: ['node', 'scripts/validate-rollback-proof.ts'],
  ...
  await resolveValue(governance.stores.evidenceStore.writeEvidence(targetWorkItemId, evidence));
