...
import { readRuntimeIdentityDefault, writeRuntimeIdentityDefault } from './actor-registry.ts';
import { relativePathFrom } from './shared.ts';

export const runtimeSessionsRootRelativePath = '.atm/runtime/sessions' as const;
...
export function readActorWorkSession(cwd: string, sessionId: string): ActorWorkSessionDocument | null {
  const absolutePath = sessionPathFor(path.resolve(cwd), sessionId);
  return readSessionFile(absolutePath);
}

export function listActorWorkSessions(cwd: string): readonly ActorWorkSessionDocument[] {
  ...
  return readdirSync(absoluteRoot)
    ...
    .filter((entry): entry is ActorWorkSessionDocument => entry !== null)
    .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
}

export function resolveActorWorkSession(cwd: string, criteria: SessionResolveCriteria): ActorWorkSessionDocument | null {
  const root = path.resolve(cwd);
  ...
  const sessions = listActorWorkSessions(root).filter((session) => {
    if (!criteria.includeNonActive && session.status !== 'active') return false;
    if (actorId && session.actorId !== actorId) return false;
    if (taskId && session.taskId !== taskId) return false;
    if (claimLeaseId && session.claimLeaseId !== claimLeaseId) return false;
    return true;
  });
  return sessions[0] ?? null;
}

export function upsertActorWorkSession(input: SessionUpsertInput): {
  ...
  const session: ActorWorkSessionDocument = {
...
export function updateActorWorkSessionState(input: {
  ...
  readonly reason?: string | null;
  readonly timestamp?: string;
}): { readonly session: ActorWorkSessionDocument; readonly sessionPath: string } | null {
  const cwd = path.resolve(input.cwd);
  const existing = resolveActorWorkSession(cwd, {
    sessionId: input.sessionId ?? null,
...
function readSessionFile(filePath: string): ActorWorkSessionDocument | null {
  if (!existsSync(filePath)) return null;
  try {
    const parsed = JSON.parse(readFileSync(filePath, 'utf8')) as Partial<ActorWorkSessionDocument>;
...
function createActorWorkSessionId(cwd: string, actorId: string, taskId: string, timestamp: string) {
...
function normalizeSessionStatus(value: unknown): ActorWorkSessionStatus {
