...
function normalizeSharedPathOrder(paths: readonly string[]): readonly string[] {
  return [...new Set(paths.map((value) => String(value).trim().replace(/\\/g, '/')).filter(Boolean))]
    .sort((left, right) => left.localeCompare(right));
}

export function buildClaimAdmissionDecisionLog(input: ClaimAdmissionDecisionLogInput): ClaimAdmissionDecisionLog {
