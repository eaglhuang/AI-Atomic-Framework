...
export async function runLock(argv: any) {
  ...
  const task: WorkItemRef = existingTask ?? {
  ...
  if (options.action === 'check') {
    const lock = await resolveValue(adapter.stores.lockStore.getLock(options.taskId));
    const ok = lock !== null;
    return makeResult({
      ok,
      command: 'lock',
  ...
  const released = await resolveValue(adapter.stores.lockStore.releaseLock(options.taskId, options.owner));
  return makeResult({
    ok: true,
    command: 'lock',
    cwd: options.cwd,
