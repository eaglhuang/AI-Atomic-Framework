    ...
    return makeResult({
      ...
      messages: [
        ok
          ...
          : message(
            'error',
            verifyReport.status === 'stale' ? 'ATM_INTEGRATION_VERIFY_STALE' : 'ATM_INTEGRATION_VERIFY_DRIFT',
            verifyReport.status === 'stale'
...
function compareManifestParity(installed: InstallManifest, expected: InstallManifest) {
