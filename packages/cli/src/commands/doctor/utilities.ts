...
export function createCheck(name: string, ok: boolean, details: unknown): DoctorCheck {
  return {
    name,
    ok: ok === true,
...
export function createIntegrationDriftRemediation(integrationHealth: Awaited<ReturnType<typeof checkIntegrationHealth>>) {
...
export function readJsonIfExists(filePath: string): Record<string, unknown> | null {
  return existsSync(filePath) ? (JSON.parse(readFileSync(filePath, 'utf8')) as Record<string, unknown>) : null;
}


export function listPackageDirs(root: string): string[] {
...
export function packageDirLabel(root: string, packageDir: string): string { return (readJsonIfExists(path.join(root, packageDir, 'package.json'))?.name as string | undefined) ?? packageDir; }
...
export function checkCharterIntegrity(root: string): { ok: boolean; charterPath: string; charterInvariantsPath: string; charterPresent: boolean; invariantsPresent: boolean; invariantsParseable: boolean; hashField: string | null } {
...
export function checkCharterIntegrityV2(root: string): {
  ok: boolean;
  charterPath: string;
