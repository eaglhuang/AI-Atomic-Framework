...
import { createCheck } from './utilities.ts';

export function createGovernanceEntryReadinessCheck(root: string, repoIdentity: { isFrameworkRepo: boolean }, gitHeadEvidenceCheck: DoctorCheck) {
...
export function createBacklogSyncCheck(root: string, repoIdentity: { isFrameworkRepo: boolean }) {
...
export function parseBacklogRows(markdown: string) {
  return markdown
    .split(/\r?\n/)
...
export function runGitScalar(cwd: string, args: readonly string[]) {
...
export function isProtectedFrameworkBranchTarget(branch: string) {
  return branch === 'main'
    || branch === 'master'
    || branch === 'trunk'
    || /^release\/.+/.test(branch);
}
export function hasRequiredScripts(scripts: Record<string, string> = {}) {
...
export function isFrameworkContractExpected(repoIdentity: { isFrameworkRepo: boolean }) {
