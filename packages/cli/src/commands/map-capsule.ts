...
import {
  exportMapCapsule,
  importMapCapsule,
  convertSpecToMapBundle,
  validateMapCidFormat,
  mapCidToShortId,
  MapCapsuleError
} from '../../../core/src/registry/map-capsule.ts';
import {
  loadMapRegistry,
  saveMapRegistry,
  upsertMapEntry,
  linkMapChain,
  markMapRolledBack,
  getGlobalMapRegistryPath,
  getRepoMapRegistryPath
} from '../../../core/src/registry/map-capsule-registry.ts';
import { loadCapsuleRegistry, getGlobalRegistryPath } from '../../../core/src/registry/capsule-registry.ts';
import { CliError, makeResult, message } from './shared.ts';

interface MapCapsuleOptions {
  cwd: string;
  map?: string;
  cid?: string;
  payload?: string;
...
function parseMapCapsuleArgs(argv: string[]): { action: string; options: MapCapsuleOptions } {
  const options: MapCapsuleOptions = { cwd: process.cwd(), vendor: true };
  let action = '';

  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (!arg.startsWith('--') && !action) { action = arg; continue; }
    if (arg === '--cwd') { options.cwd = argv[++i]; continue; }
    if (arg === '--map') { options.map = argv[++i]; continue; }
...
export async function runMapCapsule(argv: string[]) {
...
async function runMapExport(cwd: string, options: MapCapsuleOptions) {
  const mapId = options.map ?? '';
  if (!mapId) throw new CliError('ATM_CLI_USAGE', 'map-capsule export requires --map <id>', { exitCode: 2 });
  ...
  const atomCidMap: Record<string, string> = {};
  for (const [cid, entry] of Object.entries(capsuleRegistry.entries)) {
    if (entry.atomId) atomCidMap[entry.atomId] = cid;
  }
...
async function runMapImport(cwd: string, options: MapCapsuleOptions) {
  const mapCid = options.cid ?? '';
  const payload = options.payload ?? '';
  if (!mapCid) throw new CliError('ATM_CLI_USAGE', 'map-capsule import requires --cid <map:cid>', { exitCode: 2 });
  if (!payload) throw new CliError('ATM_CLI_USAGE', 'map-capsule import requires --payload <base64>', { exitCode: 2 });
...
async function runMapRollback(cwd: string, options: MapCapsuleOptions) {
  const mapCid = options.cid ?? '';
  if (!mapCid) throw new CliError('ATM_CLI_USAGE', 'map-capsule rollback requires --cid <map:cid>', { exitCode: 2 });
  validateMapCidFormat(mapCid);
  ...
  if (!entry) throw new CliError('ATM_MAP_CAPSULE_NOT_FOUND', `No map registry entry found for: ${mapCid}`, { exitCode: 1 });
  if (!entry.previousMapCid) throw new CliError('ATM_MAP_CAPSULE_NO_PREVIOUS', `No previousMapCid for ${mapCid}`, { exitCode: 1 });
