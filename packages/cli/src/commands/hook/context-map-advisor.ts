...
export function runContextMapAdvisor(cwd: string): AdvisoryReport | null {
  ...
  try {
    ...
    if (Date.now() - startTime > 50) return null;
    ...
    if (Array.isArray(task.scopePaths)) {
      for (const p of task.scopePaths) {
        if (typeof p === 'string') allowedPatterns.add(p);
      }
    }

    // 收集 contextMap 中的 primary, secondary, tests
    if (task.contextMap && typeof task.contextMap === 'object') {
      const categories: ('primary' | 'secondary' | 'tests')[] = ['primary', 'secondary', 'tests'];
    ...
    // 比對 staged files
    const outOfScope: { path: string; suggestedCategory: 'primary' | 'secondary' | 'tests' }[] = [];

    for (const file of stagedFiles) {
      if (Date.now() - startTime > 50) return null; // 效能守門
...
function findCurrentTaskId(root: string): string | null {
...
function isPathMatched(filePath: string, patterns: string[]): boolean {
...
function suggestCategory(filePath: string): 'primary' | 'secondary' | 'tests' {
  const lower = filePath.toLowerCase();
  if (
    lower.includes('.test.ts') ||
...
function printAdvisoryWarning(taskId: string, outOfScope: readonly { path: string; suggestedCategory: string }[]) {
  const lines: string[] = [];
  lines.push('');
  ...
  // 按建議分類分群
  const byCategory: Record<string, string[]> = { primary: [], secondary: [], tests: [] };
