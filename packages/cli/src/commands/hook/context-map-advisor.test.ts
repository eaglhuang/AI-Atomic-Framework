...
import { runContextMapAdvisor } from './context-map-advisor.ts';

// 測試 suggestCategory 啟發式分類
function testSuggestCategory() {
  const suggestCategory = (filePath: string): 'primary' | 'secondary' | 'tests' => {
...
function testIsPathMatched() {
  ...
  const isPathMatched = (filePath: string, patterns: string[]): boolean => {
