...
if (!taskId || !isLockBackedRunnerPublicationEvidence(file, evidence, taskId)) return taskId;
...
} function isVerifiedStandaloneBulkClosureManifest(cwd, filePath) { const normalized = normalizeRelativePath(filePath);
if (!/^atomic_workbench\/evidence\/bulk-closure-manifest-[^/]+\.json$/i.test(normalized)) return false;
const manifest = readJsonFile(path.join(cwd, normalized));
if (manifest?.schemaId !== 'atm.bulkClosureManifest.v1' || typeof manifest?.bundleCommit !== 'string' || !/^[0-9a-f]{7,40}$/i.test(manifest.bundleCommit) || !Array.isArray(manifest?.tasks) || manifest.tasks.length === 0) return false;
try { execFileSync('git', ['merge-base', '--is-ancestor', manifest.bundleCommit, 'HEAD'], { cwd, stdio: 'ignore' }); } catch { return false; }
return manifest.tasks.every((entry) => { const taskId = typeof entry?.taskId === 'string' ? entry.taskId.trim() : '';
if (!taskId || entry?.closurePacket !== `.atm/history/evidence/${taskId}.closure-packet.json` || entry?.bundleManifest !== `.atm/history/evidence/${taskId}.bundle-manifest.json`) return false;
