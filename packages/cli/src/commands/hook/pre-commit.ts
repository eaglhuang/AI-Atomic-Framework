...
export function runPreCommitHook(cwd: string) {
  ...
  const actionableResidueFindings = selectActionableResidueFindings({
    ...
    hasTerminalOwner: (ownerTaskId: string) => {
      try {
        const taskDocument = JSON.parse(readFileSync(path.join(root, '.atm', 'history', 'tasks', `${ownerTaskId}.json`), 'utf8')) as Record<string, unknown>;
...
function isResolutionAuthorizedCurrentTask(cwd: string, taskId: string | null, conflictTaskId: string): boolean {
  ...
  try {
    const artifact = JSON.parse(readFileSync(path.resolve(cwd, artifactPath), 'utf8')) as Record<string, unknown>;
...
function taskLedgerEntryExistsOrIsStaged(cwd: string, taskId: string, stagedSet: ReadonlySet<string>): boolean {
 ...
 */
export function isUnconsumedCloseWindowDeferralSnapshot(cwd: string, relativePath: string): boolean {
  if (!/close-window-governance-dirty-/.test(relativePath)) return false;
  try {
    const snapshot = JSON.parse(readFileSync(path.join(cwd, relativePath), 'utf8')) as Record<string, unknown>;
    return snapshot.schemaId === 'atm.closeWindowGovernanceDirtySnapshot.v1' && snapshot.restoredAt === null;
  } catch {
    // Unreadable snapshot: fail safe, keep it.
    return true;
  }
}
