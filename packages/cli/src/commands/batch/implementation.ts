...
const { options } = parseOptions( action === 'checkpoint' ? stripBatchCheckpointCloseArgs(argv) : action === 'deliver-and-close' ? stripBatchDeliverAndCloseExtras(argv) : argv, 'batch' ); if (action === 'status' || action === 'current') {
