...
import {
  ...
  uniqueSorted
} from './git-governance/commit-scope-policy.ts';
import { buildTaskScopedCommitFileSet, isFileAllowedInTaskBundle as isTaskBundleAllowedByPolicy } from './git-governance/commit-bundle-filter.ts';
import { isActionableManualResidue, isDeferrableForeignGovernanceResidue } from './git-governance/governance-residue-policy.ts';
...
function resolveGitCommitTimeoutMs(explicitTimeoutMs: number | null): number {
...
interface GitCommitAttemptStatus {
  readonly schemaId: 'atm.gitCommitAttemptStatus.v1';
  readonly actorId: string;
  readonly taskId: string | null;
  readonly sessionId: string | null;
  readonly status: 'in-progress' | 'committed' | 'failed' | 'timeout';
  readonly phase: string;
  readonly startedAt: string;
  readonly updatedAt: string;
  readonly commitSha: string | null;
  readonly headShaBeforeCommit: string | null;
  readonly headShaAfterAttempt: string | null;
  readonly headAdvancedDuringAttempt: boolean | null;
...
interface GitCommitBranchQueueStatus {
  readonly schemaId: 'atm.gitCommitBranchQueueStatus.v1';
  readonly status: 'free' | 'busy' | 'stale-or-dead';
  readonly branchRef: string;
  readonly branchName: string;
  readonly lockPath: string;
  readonly lockRecord: BranchCommitQueueLockRecord | null;
  readonly ownerAlive: boolean | null;
  readonly ageMs: number | null;
  readonly currentHeadSha: string | null;
  readonly headMovedSinceAcquire: boolean | null;
  readonly recommendedAction: string;
}

function writeGitCommitAttemptStatus(cwd: string, statusRelativePath: string, status: GitCommitAttemptStatus) {
  try {
...
function readGitCommitAttemptStatus(cwd: string, actorId: string, taskId: string | null): GitCommitAttemptStatus | null {
  const absolutePath = path.join(cwd, gitCommitAttemptStatusRelativePath(actorId, taskId));
  if (!existsSync(absolutePath)) return null;
  try {
    return JSON.parse(readFileSync(absolutePath, 'utf8')) as GitCommitAttemptStatus;
  } catch {
    return null;
  }
}
...
function listCommitAttributionSideEffectPaths(cwd: string): readonly string[] {
  const actorRegistryState = inspectTrackedActorRegistryState(cwd);
...
function isCommitAttributionSideEffectPath(filePath: string): boolean {
...
function shouldStageGovernedGitHeadEvidenceBeforeCommit(stagedFiles: readonly string[]): boolean {
...
function isGovernedLedgerBoundaryPathForGitCommit(filePath: string): boolean {
...
function appendGitHeadEvidenceJsonl(evidenceAbsolute: string, payload: Record<string, unknown>) {
...
export function captureGitHeadEvidencePreparation(cwd: string): GitHeadEvidencePreparationSnapshot {
  const evidenceAbsolute = path.join(cwd, gitHeadEvidencePath);
...
export function rollbackFailedGitHeadEvidencePreparation(snapshot: GitHeadEvidencePreparationSnapshot): boolean {
...
export function reconcileResolvedCrossTaskMutationIncident(cwd: string, taskId: string | null): boolean {
...
function hasMatchingWorktreeGitHeadEvidence(cwd: string, treeSha: string, parentCommitShas: readonly string[]): boolean {
  const evidenceAbsolute = path.join(cwd, gitHeadEvidencePath);
  ...
  for (const line of lines) {
    try {
      const parsed = JSON.parse(line) as { evidence?: Array<{ details?: { git?: { treeSha?: string; parentCommitShas?: unknown } } }> };
...
function readCurrentHeadParentCommitShas(cwd: string): readonly string[] {
  try {
...
function isIgnorableTaskScopedDirtySideEffect(filePath: string): boolean {
 ...
 */
export function resolveActorGitIdentityForCommit(cwd: string, actorId: string): GitIdentityProfile {
...
type FrameworkScopedStagingInspection =
...
function runGitAdmission(options: ParsedGitOptions) {
...
function buildGitAdmissionComposerInput(options: ParsedGitOptions, result: ReturnType<typeof evaluateGitAdmission>) {
 ...
 */
function runGitCommitStatus(options: ParsedGitOptions) {
  const resolvedActor = resolveActorId(options.actorId ?? undefined, options.cwd);
  if (!resolvedActor) {
...
function isRecordCommitAllowedPath(filePath: string): boolean {
  const normalized = normalizeRelativePath(filePath).toLowerCase();
  if (!normalized) return false;
  if (normalized === gitHeadEvidencePaths.legacyJson || normalized === gitHeadEvidencePaths.jsonl) return true;
  if (isCommitAttributionSideEffectPath(normalized)) return true;
  if (normalized.includes('/protected-override') || normalized.startsWith('.atm/history/protected-override-audit/')) return false;
  if (normalized.endsWith('.closure-packet.json')) return false;
  if (normalized.includes('/repair') || normalized.includes('.repair-')) return false;
  if (normalized.startsWith('.atm/history/tasks/') && normalized.endsWith('.json')) return true;
  if (normalized.startsWith('.atm/history/task-events/') && normalized.endsWith('.json')) return true;
  if (normalized.startsWith('.atm/history/reports/task-import/') && normalized.endsWith('.json')) return true;
  if (normalized.startsWith('.atm/history/evidence/historical-batches/') && normalized.endsWith('.json')) return true;
...
function runGitCommit(options: ParsedGitOptions) {
  ...
  try {
    ...
    const nodeChildError = error as { killed?: boolean; signal?: string | null; code?: string } | null;
 ...
 */
function rollbackNewlyStagedLiveIndexResidue(cwd: string, stagedBeforeAttempt: readonly string[]): readonly string[] {
...
function isAllowedGovernanceArtifactPath(cwd: string, filePath: string, taskId: string): boolean {
  const normalized = normalizeRelativePath(filePath);
  const normalizedTaskId = taskId.toLowerCase();
  const lower = normalized.toLowerCase();
...
function isFileAllowedInTaskBundle(cwd: string, filePath: string, taskId: string, declaredScope: readonly string[]): boolean {
...
function listExistingGovernanceFilesRecursively(root: string, relativeDirectory: string): string[] {
...
export function listTaskOwnedProtectedOverrideAuditFiles(cwd: string, taskId: string): readonly string[] {
...
function stageTaskScopedBundleFiles(cwd: string, files: readonly string[], env?: NodeJS.ProcessEnv) {
  const normalizedFiles = uniqueSorted(files.map(normalizeRelativePath).filter(Boolean));
  if (normalizedFiles.length === 0) {
    return;
  }
...
function isIgnorableCommitStagingSideEffect(cwd: string, filePath: string, taskId: string): boolean {
  const normalized = normalizeRelativePath(filePath).toLowerCase();
  const normalizedTaskId = taskId.toLowerCase();
...
function isTaskOwnedProtectedOverrideAuditPath(cwd: string, filePath: string, normalizedTaskId: string): boolean {
  const normalized = normalizeRelativePath(filePath).toLowerCase();
...
function frameworkTempTaskId(actorId: string): string {
  const normalized = actorId.trim().replace(/[^A-Za-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '');
  return `ATM-FRAMEWORK-TEMP-${normalized || 'actor'}`;
}

function readActiveFrameworkClaimFiles(cwd: string, actorId: string): readonly string[] {
  const lockPath = path.join(cwd, '.atm', 'runtime', 'locks', `${frameworkTempTaskId(actorId)}.lock.json`);
  if (!existsSync(lockPath)) {
    return [];
  }
  try {
    const parsed = JSON.parse(readFileSync(lockPath, 'utf8')) as { files?: unknown };
...
function readReleaseGeneratedArtifactPaths(cwd: string): ReadonlySet<string> {
  ...
  for (const manifestPath of [
    ...
    try {
      const parsed = JSON.parse(readFileSync(manifestPath, 'utf8')) as { generatedFiles?: unknown };
...
function isIgnorableFrameworkCommitStagingSideEffect(filePath: string): boolean {
  const normalized = normalizeRelativePath(filePath).toLowerCase();
  if (normalized === gitHeadEvidencePaths.legacyJson || normalized === gitHeadEvidencePaths.jsonl) {
    return true;
  }
...
function autoStageFrameworkClaimFiles(cwd: string, actorId: string, apply = true): readonly string[] {
  ...
  const candidates = uniqueSorted(
    listTaskScopedWorktreeDirtyFiles(cwd).filter((filePath) =>
...
function inspectFrameworkScopedUnstagedCommit(cwd: string, actorId: string): FrameworkScopedStagingInspection | null {
  const claimedFiles = new Set(readActiveFrameworkClaimFiles(cwd, actorId));
  if (claimedFiles.size === 0) {
    return null;
  }
  ...
  const inScopeDirtyFiles = uniqueSorted(dirtyFiles.filter((filePath) =>
    isFrameworkGeneratedArtifactAllowed(filePath, claimedFiles, releaseGeneratedArtifacts)
  ));
  ...
  const skippedExternalDirtyFiles = uniqueSorted(dirtyFiles.filter((filePath) =>
    !isFrameworkGeneratedArtifactAllowed(filePath, claimedFiles, releaseGeneratedArtifacts)
  ));
  return {
    kind: 'staging-required',
...
function inspectCurrentBranchCommitQueueStatus(cwd: string): GitCommitBranchQueueStatus {
  ...
  const status: GitCommitBranchQueueStatus['status'] = ownerAlive === true ? 'busy' : 'stale-or-dead';
  return {
    schemaId: 'atm.gitCommitBranchQueueStatus.v1',
    status,
    branchRef,
...
function isBranchCommitQueueOwnerAlive(ownerPid: unknown): boolean | null {
...
function readBranchCommitQueueLockRecord(lockPath: string): BranchCommitQueueLockRecord | null {
  try {
    const raw = JSON.parse(readFileSync(path.join(lockPath, 'record.json'), 'utf8')) as Record<string, unknown>;
...
function recordBranchCommitQueueStaleCleanup(input: {
  readonly cwd: string;
  readonly lockPath: string;
  readonly record: BranchCommitQueueLockRecord;
  readonly actorId: string;
  readonly currentHeadSha: string | null;
  readonly ownerAlive: boolean | null;
...
function buildUnifiedPatch(filePath: string, beforeContent: string, afterContent: string) {
