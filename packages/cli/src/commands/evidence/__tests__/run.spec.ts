...
const findLatestCommandRun = (manifest: unknown, predicate: (run: Record<string, unknown>) => boolean) => {
  const runs = manifest
    && typeof manifest === 'object'
    && Array.isArray((manifest as { commandRuns?: unknown }).commandRuns)
    ? [...((manifest as { commandRuns: Record<string, unknown>[] }).commandRuns)]
    : [];
...
try {
  ...
  const passRun = findLatestCommandRun(pass.evidence?.bundleManifest, (run) =>
  ...
  assert.ok((passRun?.durationMs as number) >= 0);
  ...
  try {
    ...
    if (previousLaneSessionId === undefined) delete process.env.ATM_LANE_SESSION_ID;
    else process.env.ATM_LANE_SESSION_ID = previousLaneSessionId;
  }
  ...
  const recentRun = findLatestCommandRun(recent.evidence?.bundleManifest, (run) =>
    run.command === 'node -e "process.exit(0)"'
    && Array.isArray(run.validators)
