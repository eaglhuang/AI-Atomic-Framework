...
import { inspectGitWorktreeReadiness, type GitWorktreeReadinessReport } from '../git-worktree-readiness.ts';
...
export type AutoGeneratedResidueVerdict =
  | 'auto-clean-safe'
  | 'block-and-explain'
  | 'manual-review';
...
interface GovernanceHistoryArtifactInfo {
  readonly ownerTaskId: string;
  readonly artifactKind: 'bundle-manifest' | 'closure-packet' | 'close-task-event';
}
...
export function inspectAutoGeneratedResidue(taskId: string | null, candidatePaths: readonly string[]): AutoGeneratedResidueReport {
  const normalizedTaskId = taskId?.trim().toUpperCase() ?? null;
  const findings = uniqueSorted(candidatePaths.map(normalizeRelativePath).filter(Boolean))
    .map((filePath) => classifyAutoGeneratedResidue(normalizedTaskId, filePath))
    .filter((entry): entry is AutoGeneratedResidueFinding => entry !== null);
  return {
...
function classifyAutoGeneratedResidue(taskId: string | null, filePath: string): AutoGeneratedResidueFinding | null {
  const normalized = normalizeRelativePath(filePath);
  const lower = normalized.toLowerCase();
  const normalizedTaskId = taskId?.trim().toUpperCase() ?? null;
  if (lower.startsWith('.atm/history/reports/task-import/') && lower.endsWith('.json')) {
    return {
      path: normalized,
      verdict: 'auto-clean-safe',
      reason: 'task-import report is an ATM-generated transient report and may be removed automatically after the import/claim lane completes.',
      ownerTaskId: normalizedTaskId,
      cleanupAction: 'remove'
    };
  }
  // ATM-BUG-2026-07-07-052 (OPT-14): protected-override-audit events are the
  // durable governance record of an emergency/hook-bypass lane (what was
  // authorized, by whom, and whether it succeeded). They were previously
  // classified as disposable "auto-clean-safe" residue and physically
  // deleted by cleanupAutoGeneratedResidue() the next time any task ran a
  // commit -- so `emergency audit --task <id>` could report the eventPath at
  // authorization time, then find nothing once the first unrelated commit
  // swept the worktree clean. These files must never be silently deleted;
  // isIgnorableCommitStagingSideEffect() now lets them ride along and get
  // staged into whichever commit runs next instead.
  if (lower.startsWith('.atm/history/protected-override-audit/') && lower.endsWith('.json')) {
    return null;
  }
  const governanceArtifact = classifyGovernanceHistoryArtifact(normalized);
  if (governanceArtifact) {
    if (normalizedTaskId && governanceArtifact.ownerTaskId === normalizedTaskId) {
      return null;
    }
    return {
      path: normalized,
      verdict: 'block-and-explain',
      reason: normalizedTaskId
        ? `${governanceArtifact.artifactKind} belongs to another task (${governanceArtifact.ownerTaskId}) and must be reconciled explicitly instead of silently skipped.`
        : `${governanceArtifact.artifactKind} is an ATM-managed governance artifact (${governanceArtifact.ownerTaskId}) but no active task ownership is available to prove it is safe residue.`,
      ownerTaskId: governanceArtifact.ownerTaskId,
      cleanupAction: null
    };
  }
  ...
  const lockCleanupReportMatch = lower.match(/^\.atm\/history\/reports\/lock-cleanup\/[^/]+-([a-z0-9-]+)\.json$/);
  if (lockCleanupReportMatch) {
    const ownerTaskId = lockCleanupReportMatch[1]?.toUpperCase() ?? null;
    return {
      path: normalized,
      verdict: 'auto-clean-safe',
      reason: ownerTaskId === normalizedTaskId
        ? 'lock-cleanup report belongs to the current task checkpoint and may be removed automatically after commit.'
        : `lock-cleanup report belongs to another task/run (${ownerTaskId}) but is a disposable ATM byproduct and may be removed automatically before commit.`,
      ownerTaskId,
      cleanupAction: 'remove'
    };
  }
...
function classifyGovernanceHistoryArtifact(filePath: string): GovernanceHistoryArtifactInfo | null {
  const normalized = normalizeRelativePath(filePath);
  const bundleManifestMatch = normalized.match(/^\.atm\/history\/evidence\/([^/.]+)\.bundle-manifest\.json$/i);
  if (bundleManifestMatch) {
    return { ownerTaskId: bundleManifestMatch[1].toUpperCase(), artifactKind: 'bundle-manifest' };
  }
  const closurePacketMatch = normalized.match(/^\.atm\/history\/evidence\/([^/.]+)\.closure-packet\.json$/i);
  if (closurePacketMatch) {
    return { ownerTaskId: closurePacketMatch[1].toUpperCase(), artifactKind: 'closure-packet' };
  }
  const taskEventMatch = normalized.match(/^\.atm\/history\/task-events\/([^/]+)\/.+(?:close|reconcile|repair-closure).+\.json$/i);
  if (taskEventMatch) {
    return { ownerTaskId: taskEventMatch[1].toUpperCase(), artifactKind: 'close-task-event' };
  }
  return null;
}
...
export async function runFrameworkTempRelease(cwd: string, actor: string | null) {
  ...
  if (stagedFiles.length > 0) {
    releaseMessages.push(
      message('warn', 'ATM_FRAMEWORK_STAGED_RESIDUE_AT_RELEASE', `Warning: Staged files remain in index at session end: ${stagedFiles.join(', ')}. Please commit, stash, or reset them.`, {
        stagedFiles
      })
    );
  }
...
function assertFrameworkGitWorktreeReady(cwd: string) {
  ...
  throw new CliError('ATM_GIT_WORKTREE_READY_REQUIRED', 'Git local config marks this checked-out repository as bare, so framework-mode cannot safely continue until the local worktree setting is repaired.', {
    exitCode: 1,
    details: readiness as unknown as Record<string, unknown>
  });
}
...
export function inspectRunnerSourceDrift(cwd: string) {
...
export function assertSourceFirstRunnerReadOnlyAction(input: {
  ...
  if (entrypoint !== 'atm.dev.mjs' && !entrypoint?.endsWith('/atm.dev.mjs')) return;
...
export function repairClosurePacketForTask(input: {
  ...
  if (!parsed) {
    ...
    let reconstructedPacket: ClosurePacket | null = null;
    const missingSegments: string[] = ['closure-packet'];
  ...
  if (!parsed) {
    parsed = readJsonIfExists(packetAbsolutePath) as ClosurePacket | null;
  }
  if (parsed) {
