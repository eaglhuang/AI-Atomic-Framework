...
export function inspectAutoGeneratedResidue(taskId: string | null, candidatePaths: readonly string[]): AutoGeneratedResidueReport {
  ...
  const findings = uniqueSorted(candidatePaths.map(normalizeRelativePath).filter(Boolean)).map((filePath) =>
    classifyAutoGeneratedResidue(normalizedTaskId, filePath)
  );
  return {
...
function classifyAutoGeneratedResidue(taskId: string | null, filePath: string): AutoGeneratedResidueFinding {
