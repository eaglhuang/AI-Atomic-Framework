...
function classifyAutoGeneratedResidue(taskId: string | null, filePath: string): AutoGeneratedResidueFinding {
  ...
  if (snapshotMatch) {
    ...
    if (ownerTaskId === 'GIT-HEAD' || (normalizedTaskId && ownerTaskId === normalizedTaskId)) {
      return {
        path: normalized,
        verdict: 'auto-clean-safe',
        reason: 'current-operation foreign-staged snapshot is an ATM byproduct and may be removed automatically after the lane completes.',
        ownerTaskId,
    ...
    return {
      ...
      reason: `generated snapshot belongs to another task/run (${ownerTaskId}) and must not be cleaned implicitly.`,
      ownerTaskId,
      cleanupAction: null
    };
  }
  ...
  if (governanceDirtyMatch) {
    ...
    if (ownerTaskId === 'GIT-HEAD' || (normalizedTaskId && ownerTaskId === normalizedTaskId)) {
      return {
        path: normalized,
        verdict: 'auto-clean-safe',
        reason: 'close-window governance-dirty snapshot belongs to the current operation and may be removed automatically after the lane completes.',
        ownerTaskId,
        cleanupAction: 'remove'
      };
    }
    return {
      path: normalized,
      verdict: 'block-and-explain',
      reason: `generated close-window governance-dirty snapshot belongs to another task/run (${ownerTaskId}) and must not be cleaned implicitly.`,
      ownerTaskId,
      cleanupAction: null
    };
  }
