...
| R3 | Single-branch commit ordering | The single `main` branch makes commit landing an inherent global serialization point. This is the **accepted minimal serial core**. Broker batching reduces its frequency, but batching is only appropriate for **related tasks** (same dispatch wave, compatible surface family); unrelated tasks must not share a commit. |
