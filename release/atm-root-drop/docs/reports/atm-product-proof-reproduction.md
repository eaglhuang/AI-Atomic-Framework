...
An earlier measurement the same day reported 9 failures and 1 unexplained. That reading was taken while the GitHub API still showed run 35638310600's Product CI job as in progress with a null conclusion, about ten minutes after every one of its steps had succeeded. `export-github-ci-attempts.ts` writes `unknown` for a null job conclusion and the measure script counts that as an unknown-failure, so a lagging read turns a green run into an unexplained one. Once the job object caught up the same pipeline returned the numbers above.

The lag is not always short. Later the same day run 35648817462 was still being served as `Checkout in_progress` with every subsequent step `pending`, more than twenty-eight minutes after the job had in fact completed successfully in twelve minutes. Treat any unexplained failure as unconfirmed until the run's job conclusions have been queried directly and the export rerun; the instrument was not changed to accommodate the transient reading, because a rule that drops runs is exactly what a subject of this proof should not be adding to it.
