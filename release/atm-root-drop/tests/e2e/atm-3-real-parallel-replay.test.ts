...
assert.equal(evidence.workerReceipts.every((worker) => (worker.commandReceipts?.length ?? 0) > 0), true);
assert.equal(evidence.workerReceipts.every((worker) => worker.commandReceipts?.every((receipt) => receipt.command.includes('broker decision'))), true);
assert.equal(evidence.workerReceipts.every((worker) => worker.sideEffects.some((effect) => effect.startsWith('broker-decision:'))), true);
