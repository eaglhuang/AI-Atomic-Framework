...
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 69);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 69);
assert.equal(report.positiveRows.length, 69);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 69);
