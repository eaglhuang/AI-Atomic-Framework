...
import {
  lineageLogMatchesMap,
  normalizeVersionLineage,
  resolveBackfillTimestamp
} from './registry/lineage-normalization.ts';
...
export function runRegistry(argv: string[] = []) {
  ...
  if (area !== 'lineage' || action !== 'backfill') {
    throw new CliError(
      'ATM_CLI_USAGE',
      'Usage: atm registry lineage backfill --atom <id> --from <version> --to <version> --map <map-id> --lineage-log <path> (--dry-run|--apply)',
      { exitCode: 2, details: { positional: parsed.positional } }
    );
  }
...
function runRegistryLineageBackfill(options: Record<string, unknown>) {
...
function requireBackfillOptions(options: Record<string, unknown>) {
  return {
...
function requireOption(options: Record<string, unknown>, key: string, flag: string) {
...
function validateEvidenceSet(cwd: string, input: {
  ...
  reviewAdvisoryPath: string | null;
...
function createCloseoutReport(input: Record<string, unknown>) {
  return {
...
function buildBackfillRecordId(atomId: unknown, fromVersion: unknown, toVersion: unknown) {
...
function buildCloseoutReportPath(cwd: string, atomId: string, fromVersion: string, toVersion: string) {
...
function normalizeEvidenceRefs(values: readonly (string | null | undefined)[]) {
  return [...new Set(values.map((value) => String(value ?? '').trim()).filter(Boolean))]
    .sort((left, right) => left.localeCompare(right));
}

function normalizeOptionalProjectPath(cwd: string, value: unknown) {
  const normalized = String(value ?? '').trim();
  if (!normalized) return null;
  return toProjectPath(cwd, resolveInputPath(cwd, normalized));
}

function resolveInputPath(cwd: string, value: string) {
  return path.isAbsolute(value) ? path.resolve(value) : path.resolve(cwd, value);
}

function toProjectPath(cwd: string, absolutePath: string) {
...
function readJsonFile(filePath: string, code: string) {
...
function makeBackfillFailure(cwd: string, code: string, text: string, evidence: Record<string, unknown>) {
  return makeResult({
    ok: false,
    command: commandName,
    cwd,
