import { CliError, makeResult, message, parseArgsForCommand, quoteCliValue } from './shared.ts';
import { computeMissingValidatorReport } from './evidence.ts';
import { buildCloseCompletionChecklist, type TaskCloseCompletionChecklist } from './taskflow/close-orchestration.ts';
import { buildResidueDiagnosisEvidence, loadTaskDocumentOrThrow } from './tasks/public-surface.ts';
...
function parseTaskViewOptions(argv: string[]) {
...
function materializeCommandTemplate(command: string, taskId: string, planningCardPath: string | null): string {
  return command
...
function resolveNextSafeCommand(input: {
  ...
  residueNextCommand: string | null;
  evidenceBlockers: readonly TaskViewEvidenceBlocker[];
  partialClose: boolean;
}): string {
  ...
  if (input.evidenceBlockers[0]?.requiredCommand) return input.evidenceBlockers[0].requiredCommand;
...
export function buildTaskViewDashboard(input: {
  ...
  const missingValidators = input.actorId
    ...
    : null;
  const evidenceBlockers: TaskViewEvidenceBlocker[] = (missingValidators?.blockingFindings ?? []).map((entry) => ({
...
export function runTaskView(argv: string[]) {
