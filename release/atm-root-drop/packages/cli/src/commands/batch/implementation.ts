...
let cleanupResult: unknown = null; if (!continuation.pendingCommitTaskId) { try { cleanupResult = await runTasks([ 'lock', 'cleanup', '--cwd', options.cwd, '--task', currentTaskId, '--actor', resolvedActor.actorId, '--reason', 'batch checkpoint cleanup', '--json' ]); } catch { cleanupResult = null; } }
...
: null });
if (updated.status === 'completed') { releaseBatchRun(options.cwd, updated, 'completed'); }
...
const primaryText = isBatchComplete ? 'Batch checkpoint closed the final task and completed the batch run.' : 'Batch checkpoint closed the current task and opened a mandatory commit window before claiming the next queue head.'; return makeResult({ ok: true, command: 'batch', cwd: options.cwd,
...
cleanupResult: (cleanupResult as { evidence?: unknown } | null)?.evidence ?? null, batchRun: updated, nextClaim: null }
});
}
...
if (action === 'resume' && !continuation.mayClaimNext) { throw new CliError('ATM_BATCH_PENDING_COMMIT_REQUIRED', continuation.reason, { exitCode: 1, details: { batchId: repaired.batchId, pendingCommitTaskId: continuation.pendingCommitTaskId, commitCommand: pendingCommitWindow?.commitCommand ?? null, statusCommand: `node atm.mjs batch current --batch ${repaired.batchId} --compact --json` } }); }
...
const arg = argv[index]; if ( arg === '--historical-delivery' || arg === '--historical-delivery-commit' || arg === '--delivery-commit' || arg === '--historical-batch' || arg === '--reason' || arg === '--emergency-approval' ) { index += 1; continue; }
if (arg === '--waiver-out-of-scope-delivery' || arg === '--waive-out-of-scope') { continue; }
stripped.push(arg); }
return stripped; }
function parseBatchHistoricalDeliveryWaiver(argv: readonly string[]): { enabled: boolean; reason: string } | null {
let enabled = false;
let reason: string | null = null;
for (let index = 0; index < argv.length; index += 1) {
const arg = argv[index];
if (arg === '--waiver-out-of-scope-delivery' || arg === '--waive-out-of-scope') { enabled = true; continue; }
if (arg === '--reason' && index + 1 < argv.length) { reason = argv[index + 1]?.trim() || null; index += 1; continue; }
}
if (!enabled) return null;
if (!reason) { throw new CliError('ATM_BATCH_CHECKPOINT_WAIVER_REASON_REQUIRED', 'batch checkpoint --waiver-out-of-scope-delivery requires --reason <text>.', { exitCode: 2 }); }
return { enabled: true, reason };
}
function parseBatchEmergencyApproval(argv: readonly string[]): string | null {
for (let index = 0; index < argv.length; index += 1) {
const arg = argv[index];
if (arg !== '--emergency-approval') continue;
const value = argv[index + 1];
if (!value || value.startsWith('--')) { throw new CliError('ATM_CLI_USAGE', 'batch checkpoint --emergency-approval requires a lease id.', { exitCode: 2 }); }
return value.trim();
}
return null;
}
...
let evidenceOutPath: string | null = null;
...
if (arg === '--evidence-out' && index + 1 < argv.length) { evidenceOutPath = argv[++index]; continue; }
...
for (let index = 0; index < argv.length; index += 1) {
const arg = argv[index];
if (['--wave', '--manifest-digest', '--task', '--delivery-receipt', '--build-receipt', '--projection-receipt', '--evidence-out'].includes(arg) && index + 1 < argv.length) { index += 1; continue; }
if (arg === '--planning-closeback-ok' || arg === '--planning-closeback-blocked') { continue; }
stripped.push(arg);
}
return stripped;
}
function writeCheckpointReceipt(cwd: string, outPath: string, value: unknown): string {
...
const taskLower = taskId.toLowerCase(); return lower === `.atm/history/tasks/${taskLower}.json` || lower === `.atm/history/evidence/${taskLower}.bundle-manifest.json` || lower === `.atm/history/evidence/${taskLower}.closure-packet.json` || lower.startsWith(`.atm/history/task-events/${taskLower}/`); }
...
export interface BatchCheckpointCandidateSeal { readonly schemaId: 'atm.batchCheckpointCandidateSeal.v1'; readonly batchId: string; readonly taskId: string; readonly actorId: string; readonly laneSessionId: string | null; readonly headCommitSha: string | null; readonly headTreeSha: string | null; readonly candidateTreeSha: string | null; readonly candidateDigest: string | null; readonly stagedFiles: readonly string[]; readonly sourceAvailable: boolean; readonly runnerRelationship: 'head-bound' | 'candidate-diff' | 'unavailable'; readonly runnerSourceSeal: { readonly present: boolean; readonly digest: string | null; }; }
export function buildBatchCheckpointCandidateSeal(input: { readonly cwd: string; readonly batchId: string; readonly taskId: string; readonly actorId: string; readonly laneSessionId?: string | null; }): BatchCheckpointCandidateSeal { const cwd = path.resolve(input.cwd); const headCommitSha = readGitHead(cwd); const headTreeSha = headCommitSha ? String(spawnSync('git', ['rev-parse', `${headCommitSha}^{tree}`], { cwd, encoding: 'utf8' }).stdout ?? '').trim() || null : null; const treeResult = spawnSync('git', ['write-tree'], { cwd, encoding: 'utf8' }); const candidateTreeSha = treeResult.status === 0 ? String(treeResult.stdout ?? '').trim() || null : null; const stagedFiles = treeResult.status === 0 ? String(spawnSync('git', ['diff', '--cached', '--name-only'], { cwd, encoding: 'utf8' }).stdout ?? '').split(/\r?\n/).map((entry) => normalizeRelativePath(entry)).filter(Boolean).sort() : []; const releaseManifestPath = path.join(cwd, 'release', 'atm-root-drop', 'release-manifest.json'); let runnerSourceSeal = { present: false, digest: null as string | null }; if (existsSync(releaseManifestPath)) { try { const parsed = JSON.parse(readFileSync(releaseManifestPath, 'utf8')) as { runnerSourceSeal?: { digest?: unknown } }; const digest = typeof parsed.runnerSourceSeal?.digest === 'string' ? parsed.runnerSourceSeal.digest : null; runnerSourceSeal = { present: Boolean(digest), digest }; } catch { } } const candidateDigest = candidateTreeSha ? `sha256:${createHash('sha256').update(JSON.stringify({ candidateTreeSha, stagedFiles })).digest('hex')}` : null; return { schemaId: 'atm.batchCheckpointCandidateSeal.v1', batchId: input.batchId, taskId: input.taskId, actorId: input.actorId, laneSessionId: input.laneSessionId ?? process.env.ATM_LANE_SESSION_ID ?? null, headCommitSha, headTreeSha, candidateTreeSha, candidateDigest, stagedFiles, sourceAvailable: Boolean(candidateTreeSha), runnerRelationship: !candidateTreeSha ? 'unavailable' : candidateTreeSha === headTreeSha ? 'head-bound' : 'candidate-diff', runnerSourceSeal }; }
