...
export function writeBranchCommitQueueLock(repo: string, input: {
  actorId: string;
  taskId?: string | null;
...
export function readBranchRef(repo: string): string {
...
export function writeBrokerRegistry(repo: string, activeIntents: unknown[], options: { currentEpoch?: number } = {}) {
...
export async function makeBrokerCloseFixture(label: string) {
...
export async function makeDualRepoCloseFixture(label: string, options: { closePlanningStatus?: string } = {}) {
