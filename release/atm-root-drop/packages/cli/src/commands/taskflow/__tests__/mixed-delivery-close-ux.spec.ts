...
import {
  buildSharedDeliveryWaiverCommand,
  buildTaskflowCloseWriteReadinessHint,
  prioritizeSharedHistoricalDeliveryBlockers
} from '../write-readiness.ts';
...
assert.equal(
  mixedBlocker!.requiredCommand,
  buildSharedDeliveryWaiverCommand({ taskId, actorId: 'validator', historicalRef })
);
...
assert.equal(
  prioritized[0]?.requiredCommand,
  buildSharedDeliveryWaiverCommand({ taskId, actorId: 'validator', historicalRef })
);
assert.ok(
  !prioritized.some((entry) => entry.code === 'ATM_TASK_CLOSE_DELIVERABLE_DIFF_REQUIRED'),
