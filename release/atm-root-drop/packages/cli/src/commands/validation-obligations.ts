...
export const VALIDATION_OBLIGATION_RULES: readonly ValidationObligationRule[] = [
  {
...
export function resolveValidationObligations(changedPaths: readonly string[]): ValidationObligationResolution {
  const normalizedPaths = [...new Set(changedPaths.map(normalizePath).filter(Boolean))];
  const validators = new Set<string>();
  const matchedRules: ValidationObligationMatch[] = [];
  for (const rule of VALIDATION_OBLIGATION_RULES) {
    for (const pattern of rule.patterns) {
      for (const candidatePath of normalizedPaths) {
        if (!matchesPattern(pattern, candidatePath)) continue;
...
export function createSealedCommitCanaryPlan(options: { commitSha: string; validators?: readonly string[] }): SealedCommitCanaryPlan {
