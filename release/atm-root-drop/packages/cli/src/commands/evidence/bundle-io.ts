...
import { inspectHistoricalDelivery, pathMatchesTaskScope, type TaskHistoricalDeliveryReport } from '../tasks/historical-delivery.ts';
import {
  normalizeValidatorGateName,
  normalizeValidatorToken,
...
import {
  ...
  normalizeRunnerKind,
  inferRunnerKindFromCommand,
  uniqueCommandRuns,
  computeCommandRunCacheKey,
  hashJson,
  uniqueStrings,
  readCurrentCommit,
...
import {
  ...
  buildMissingValidatorFinding,
  computeMissingValidatorReport,
  type MissingValidatorFinding,
  type ValidatorCatalogEntry,
  type MissingValidatorReport
} from './missing-report.ts';
...
export function readEvidenceBundleManifest(cwd: string, taskId: string): EvidenceBundleManifest | null {
...
export function runEvidenceValidators(argv: string[]) {
  let cwd = process.cwd();
  ...
  for (const record of bundle.evidence) {
    ...
    if (Array.isArray(record.validationPasses)) {
      for (const v of record.validationPasses as unknown[]) {
        if (typeof v === 'string' && v.trim()) recordedPasses.add(canonicalizeValidatorIdentity(v.trim()));
      }
    }
...
export function runEvidenceMissing(argv: string[]) {
  let cwd = '.';
  let taskId: string | null = null;
...
export function runEvidenceDiff(argv: string[]) {
...
export function runEvidenceAdd(argv: string[]) {
...
type HistoricalBatchCoverageStatus = 'complete' | 'partial' | 'blocked';
type HistoricalBatchValidatorKind = 'taskSpecific' | 'batchWide' | 'advisory';
type HistoricalBatchFinalizeDisposition = 'keep-diagnostic' | 'abandon' | 'remove-evidence';

interface HistoricalBatchValidatorClaim {
  readonly gate: string;
  readonly kind: HistoricalBatchValidatorKind;
  readonly satisfied: boolean;
  readonly requiredForClose: boolean;
}

interface HistoricalBatchAtomHealthClaim {
  readonly atomOrMapId: string;
  readonly kind: 'owner' | 'map-update';
  readonly generatedByTask: boolean;
  readonly validatorHealthy: boolean;
}
...
interface HistoricalBatchTaskSlice {
  ...
  readonly declaredDeliverables: readonly string[];
  readonly declaredScopeFiles: readonly string[];
  readonly matchedDeliverables: readonly string[];
  readonly missingCoverage: readonly string[];
  readonly coverageStatus: HistoricalBatchCoverageStatus;
  readonly validatorClaims: readonly HistoricalBatchValidatorClaim[];
  ...
  readonly atomHealthClaims: readonly HistoricalBatchAtomHealthClaim[];
...
interface HistoricalBatchFinalizeOptions {
  readonly cwd: string;
  readonly actorId: string | null;
  readonly taskId: string;
  readonly batchRef: string;
  readonly disposition: HistoricalBatchFinalizeDisposition;
  readonly reason: string;
  readonly write: boolean;
}

export function runEvidenceHistoricalBatch(argv: string[]) {
...
export function runEvidenceHistoricalBatchFinalize(argv: string[]) {
  ...
  const batchEnvelope = JSON.parse(readFileSync(batchPath, 'utf8')) as unknown;
  ...
  return makeResult({
    ...
    mode: options.write ? 'write' : 'dry-run',
    messages: [
      message('info', options.write ? 'ATM_HISTORICAL_BATCH_SLICE_FINALIZED' : 'ATM_HISTORICAL_BATCH_SLICE_FINALIZE_READY',
        options.write
          ? `Historical batch slice ${batchId}/${options.taskId} finalized as ${options.disposition}.`
          : `Historical batch slice ${batchId}/${options.taskId} can be finalized as ${options.disposition}.`,
        plan)
    ],
    evidence: {
...
function parseEvidenceHistoricalBatchFinalizeOptions(argv: string[]): HistoricalBatchFinalizeOptions {
  const options = {
    cwd: process.cwd(),
    actorId: null as string | null,
    taskId: '',
    batchRef: '',
    disposition: '' as HistoricalBatchFinalizeDisposition | '',
    reason: '',
    write: false
  };
  for (let index = 0; index < argv.length; index += 1) {
  ...
  if (!options.taskId) throw new CliError('ATM_CLI_USAGE', 'evidence historical-batch-finalize requires --task <id>.', { exitCode: 2 });
  if (!options.batchRef) throw new CliError('ATM_CLI_USAGE', 'evidence historical-batch-finalize requires --batch <id-or-path>.', { exitCode: 2 });
  if (!options.disposition) throw new CliError('ATM_CLI_USAGE', 'evidence historical-batch-finalize requires --disposition keep-diagnostic|abandon|remove-evidence.', { exitCode: 2 });
  if (!options.reason) throw new CliError('ATM_CLI_USAGE', 'evidence historical-batch-finalize requires --reason <text>.', { exitCode: 2 });
  return {
    ...options,
...
function isHistoricalBatchFinalizeDisposition(value: string): value is HistoricalBatchFinalizeDisposition {
...
function evidenceRecordReferencesHistoricalBatch(record: Record<string, unknown>, batchId: string) {
  if (!isRecord(record.details)) return false;
  const details = record.details;
  if (isRecord(details.historicalBatch) && details.historicalBatch.batchId === batchId) return true;
  if (isRecord(details.historicalBatchValidatorAttestation) && details.historicalBatchValidatorAttestation.batchId === batchId) return true;
  if (isRecord(details.historicalBatchDisposition) && details.historicalBatchDisposition.batchId === batchId) return true;
  return false;
}
...
function resolveHistoricalBatchRunValidators(command: string, requestedValidators: readonly string[]) {
  ...
  if (requestedValidators.includes(normalized)) return [normalized];
  if (/^npm(?:\s+run)?\s+test$/i.test(command.trim()) && requestedValidators.includes('test')) return ['test'];
  return [];
}
...
function buildHistoricalBatchTaskSlice(input: {
  ...
  const matchedDeliverables = declaredDeliverables.filter((entry) =>
    matchedFiles.some((filePath) => pathMatchesTaskScope(filePath, entry))
  );
  const missingCoverage = declaredDeliverables.filter((entry) => !matchedDeliverables.includes(entry));
  const coverageStatus: HistoricalBatchCoverageStatus = declaredDeliverables.length === 0
...
function extractHistoricalBatchDeclaredDeliverables(taskDocument: Record<string, unknown> | null, declaredFiles: readonly string[]) {
  if (!taskDocument) return [...declaredFiles];
...
function extractHistoricalBatchDeclaredValidators(taskDocument: Record<string, unknown> | null) {
  if (!taskDocument || !Array.isArray(taskDocument.validators)) return [];
...
function extractHistoricalBatchAtomHealthClaims(input: {
  ...
  if (!input.taskDocument || !isRecord(input.taskDocument.atomizationImpact)) return [] as HistoricalBatchAtomHealthClaim[];
  const atomizationImpact = input.taskDocument.atomizationImpact as Record<string, unknown>;
  ...
  const mapUpdates = Array.isArray(atomizationImpact.mapUpdates)
    ? atomizationImpact.mapUpdates.filter((entry): entry is string => typeof entry === 'string' && entry.trim().length > 0).map((entry) => entry.trim())
    : [];
  ...
  const claims: HistoricalBatchAtomHealthClaim[] = [];
  if (ownerAtomOrMap) {
...
function buildHistoricalBatchValidatorClaims(input: {
  readonly requestedValidators: readonly string[];
  readonly declaredValidators: readonly string[];
  readonly validatorRuns: readonly HistoricalBatchValidatorRun[];
}) {
  const allValidators = uniqueStrings([...input.requestedValidators, ...input.declaredValidators]);
  return allValidators.map((gate): HistoricalBatchValidatorClaim => {
    const requiredForClose = input.declaredValidators.includes(gate);
    const tier = classifyValidatorTier(gate);
    const kind: HistoricalBatchValidatorKind = requiredForClose
      ? 'taskSpecific'
...
function appendHistoricalBatchTaskEvidence(input: {
  ...
  const freshValidatorAttestationRecord: Record<string, unknown> | null = reusableValidationPasses.length > 0 && input.validatorRuns.length > 0
    ? {
      evidenceKind: 'validation',
      evidenceType: 'test',
...
export function runEvidenceVerify(argv: string[]) {
...
export function runGitHeadEvidenceBackfill(argv: string[]) {
...
function hasCommandRunProofForBundle(run: CommandRunEvidenceInput): boolean {
...
function serializeBundleCommandRun(run: CommandRunEvidenceInput): Record<string, unknown> {
  return {
    command: run.command,
    exitCode: run.exitCode,
    stdoutSha256: run.stdoutSha256,
...
function upsertEvidenceBundleManifest(input: {
  ...
  validationPasses: readonly string[];
  commandRuns: readonly CommandRunEvidenceInput[];
...
function writeEvidenceBundleManifest(cwd: string, manifest: EvidenceBundleManifest) {
...
function extractAtomizationTargets(taskDocument: Record<string, unknown> | null) {
  if (!taskDocument || !isRecord(taskDocument.atomizationImpact)) return [] as Array<{ atomOrMapId: string; kind: 'owner' | 'map-update' }>;
  const atomizationImpact = taskDocument.atomizationImpact as Record<string, unknown>;
  const output: Array<{ atomOrMapId: string; kind: 'owner' | 'map-update' }> = [];
  const ownerAtomOrMap = typeof atomizationImpact.ownerAtomOrMap === 'string' ? atomizationImpact.ownerAtomOrMap.trim() : '';
  if (ownerAtomOrMap) {
...
function buildGenericAtomHealthClaims(taskDocument: Record<string, unknown> | null, validationPasses: readonly string[]) {
...
function hasHealthyAtomEvidence(taskDocument: Record<string, unknown> | null, bundle: readonly Record<string, unknown>[]) {
  const targets = extractAtomizationTargets(taskDocument);
  ...
  for (const record of bundle) {
    ...
    for (const entry of candidates) {
      ...
      if (!atomOrMapId || !requiredIds.has(atomOrMapId)) continue;
