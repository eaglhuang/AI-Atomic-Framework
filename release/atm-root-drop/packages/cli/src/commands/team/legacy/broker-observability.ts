...
import { runtimeBackendAdmissionForTeam } from '../team-execution-lane.ts';
import {
  listTeamRuns,
  readTeamRun,
  teamRunsDirectory
} from './team-run-store.ts';
...
export function runTeamBroker(argv: string[], defaultCwd: string) {
...
export function runTeamObservability(argv: string[], defaultCwd: string) {
  const action = String(argv[0] ?? '').toLowerCase();
  if (action !== 'query') {
...
function normalizeAuthorizationResourceKind(value: string | undefined): 'path' | 'atom' | 'surface' | 'range' | undefined {
  const normalized = value?.trim();
