...
import { CliError, makeResult, message, quoteCliValue, readJsonFile } from '../../shared.ts';
import { readTeamHandoffArtifacts, renderTeamHandoffIndex, teamHandoffRuntimeDirectory, verifyTeamHandoffLedger } from '../../../../../core/src/team-runtime/handoff-ledger.ts';
import { normalizeOptionalRuntimeString } from './runtime-governance.ts';
import { buildTeamPatrolFollowUp, buildTeamRunPatrolFindings, summarizePatrolSeverity, suggestedPatrolCommand, teamPatrolFinding } from './patrol-contracts.ts';
import { evaluateLargeScriptRisk } from './crew-decision-policy.ts';
import { findLatestTeamRunForTask, readTeamRun } from './team-run-store.ts';
import { deriveWritePaths, summarizeTask, uniqueStrings } from './team-utils.ts';
...
export function normalizeTeamPatrolMode(value: unknown): TeamPatrolMode {
