...
import { buildTeamObservabilityContract } from '../../../../../core/src/team-runtime/observability.ts';
...
} from '../runtime-bridges.ts';
import { buildRuntimeTierContract } from '../runtime-tier-contract.ts';
import { buildTeamShadowScheduleForPlan } from '../shadow-plan.ts';
import {
  buildAtomizationChecklist,
  buildCaptainDecision,
  buildMinimalTaskCrewBriefingContract
} from './crew-decision-policy.ts';
import {
  mapTeamSizeToLevel,
  projectTeamRecipeForLevel,
  selectTeamImplementer
} from './implementer-selector-policy.ts';
import {
  buildPermissionFinding,
  buildProposalFirstParityFindings,
  buildSuggestedPermissionLeases
} from './permission-lease-policy.ts';
import type {
  PermissionFinding,
...
export function readActiveTaskClaimActorId(cwd: string, taskId: string): string | null {
  try {
    ...
    const task = readJsonFile(taskPath) as Record<string, unknown>;
