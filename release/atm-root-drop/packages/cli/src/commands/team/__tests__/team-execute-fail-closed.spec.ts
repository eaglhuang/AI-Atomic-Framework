...
import { runTeam } from '../../team.ts';
import { resolveTeamStartExecutionLane } from '../team-execution-lane.ts';
...
async function testTeamStartExecuteWithZeroProvidersFailsClosed() {
  ...
  assert.equal((blockedMessage?.data as { providerExecutionCount?: number })?.providerExecutionCount, 0);
  assert.equal((zeroExecution.evidence as { providerOrchestration?: { results?: unknown[] } })?.providerOrchestration?.results?.length, 0);
...
async function testTeamStartWithoutExecuteRemainsOk() {
  ...
  assert.equal((stateOnly.evidence as { providerOrchestration?: { results?: unknown[] } })?.providerOrchestration?.results?.length ?? 0, 0);
