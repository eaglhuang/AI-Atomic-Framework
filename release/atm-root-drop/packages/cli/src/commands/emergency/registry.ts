export type EmergencyPermissionId =
  ...
  | 'backend.tasks.import.write'
  | 'backend.tasks.repairClosure'
  | 'backend.tasks.reset'
  | 'backend.tasks.lockCleanupGlobal'
...
export const emergencyPermissionRegistry: readonly EmergencyPermissionDefinition[] = Object.freeze([
  {
...
export function getEmergencyPermission(id: string): EmergencyPermissionDefinition | null {
  return emergencyPermissionRegistry.find((entry) => entry.id === id) ?? null;
}

export function listEmergencyPermissionIds(): string[] {
