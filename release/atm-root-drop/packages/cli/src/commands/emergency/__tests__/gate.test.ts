...
function writeLease(repo: string, input: {
...
function readLease(repo: string, leaseId: string): { usedCount: number } {
  const filePath = path.join(repo, '.atm', 'runtime', 'emergency', 'leases', `${leaseId}.json`);
  return JSON.parse(readFileSync(filePath, 'utf8')) as { usedCount: number };
}
...
// lease for each, and must still refuse when none of them covers this surface.
{
  ...
  try {
    ...
    try {
      ...
      const details = error.details as Record<string, unknown>;
      const supplied = details.suppliedLeases as Array<{ leaseId: string; permission: string }> | undefined;
