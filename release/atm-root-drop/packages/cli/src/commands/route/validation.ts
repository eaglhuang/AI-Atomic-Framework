import type { RouteContext, RouteResourceSet } from '../../../../core/src/routing/index.ts';
import { unique } from './files.ts';
import type { RouteContextValidation } from './types.ts';

export function parseResourceSet(input: string[]): RouteResourceSet {
  return {
    files: unique(input),
    atomCids: [],
    virtualAtomCids: [],
...
export function validateRouteContext(value: unknown): RouteContextValidation {
  const errors: string[] = [];
