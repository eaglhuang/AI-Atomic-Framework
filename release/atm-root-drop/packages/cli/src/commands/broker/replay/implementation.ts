...
export async function runFrozenParallelReplay(input: {
  ...
  readonly minimumWorkerObservationMs?: number;
...
export function selectRuntimeDogfoodTasks(input: {
  ...
  const candidates = readdirSync(taskRoot)
    ...
    .filter((candidate) => ['open', 'planned', 'ready', 'running', 'done'].includes(candidate.status))
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (exitCode) => {
      ...
      const finishedAtMs = Math.max(
        commandFinishedAtMs,
        startedAtMs + (input.minimumWorkerObservationMs ?? 3000)
      );
