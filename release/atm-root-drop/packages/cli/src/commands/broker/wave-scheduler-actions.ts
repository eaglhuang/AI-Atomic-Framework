...
import {
  ...
  planWaveBrokerBatch
} from '../../../../core/src/broker/wave-broker-scheduler.ts';
import type { ParsedBrokerOptions } from './parser.ts';
import type { BrokerCommandContext } from './types.ts';

function readScheduler(pathName: string) {
  if (!existsSync(pathName)) return createEmptyWaveBrokerSchedulerDocument();
...
function writeScheduler(pathName: string, document: unknown): void {
  mkdirSync(path.dirname(pathName), { recursive: true });
  writeFileSync(pathName, `${JSON.stringify(document, null, 2)}\n`, 'utf8');
}

export function handleBrokerWaveScheduler(options: ParsedBrokerOptions, context: BrokerCommandContext) {
  if (options.action !== 'schedule') return null;
  ...
  if (action === 'enqueue') {
    ...
    if (!result.replayed) writeScheduler(context.waveSchedulerPath, result.document);
    return makeResult({
      ok: true,
      command: 'broker',
      cwd: options.cwd,
