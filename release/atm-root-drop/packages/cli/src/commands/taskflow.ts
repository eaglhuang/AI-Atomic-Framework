...
function buildTaskflowOpenDryRunCommand(input: {
  ...
  if (input.profilePath) parts.push(`--profile ${quoteCliValue(input.profilePath)}`);
  if (input.taskId) parts.push(`--task-id ${quoteCliValue(input.taskId)}`);
  if (input.outputPath) parts.push(`--output ${quoteCliValue(input.outputPath)}`);
  if (input.title) parts.push(`--title ${quoteCliValue(input.title)}`);
  parts.push('--json');
  return parts.join(' ');
}
...
export async function runTaskflow(argv: string[] = []) {
  ...
  const result = makeResult({
    ...
    messages: [
      message(
        hostPolicyDecision?.familyDrift ? 'warn' : openerMode === 'delegated-governed' ? 'info' : 'warn',
