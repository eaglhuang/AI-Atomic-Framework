...
import { readStagedFiles, readStagedJsonFile } from './git-index-transaction.ts';

import { isCommitAttributionSideEffectPath } from './git-process-port.ts';

import { taskImportReportReferencesTask } from './task-scope-staging.ts';

type LegacyValue = ReturnType<typeof JSON.parse>;



export function isRecordCommitAllowedPath(filePath: LegacyValue) {
  ...
  if (
    ...
    return true;
  if (isCommitAttributionSideEffectPath(normalized)) return true;
  if (
    normalized.includes("/protected-override") ||
    normalized.startsWith(".atm/history/protected-override-audit/")
  )
    return false;
  if (normalized.endsWith(".closure-packet.json")) return false;
  ...
  if (
    ...
    return true;
  if (
    normalized.startsWith(".atm/history/reports/task-import/") &&
    normalized.endsWith(".json")
  )
    return true;
  if (
    normalized.startsWith(".atm/history/evidence/historical-batches/") &&
    normalized.endsWith(".json")
  )
    return true;
  if (
    /^\.atm\/history\/evidence\/[^/]+(?:\.bundle-manifest)?\.json$/.test(
      normalized,
    )
  )
    return true;
...
export function extractRecordCommitTaskOwner(filePath: LegacyValue) {
  const normalized = normalizeRelativePath(filePath);
  const match = normalized.match(
    /^\.atm\/history\/(?:tasks|task-events|evidence)\/([^/.]+)/i,
  );
  return match ? match[1].toUpperCase() : null;
}

export function assertRecordCommitSingleTaskOwner(cwd: LegacyValue, stagedFiles: LegacyValue) {
  ...
  for (const filePath of stagedFiles) {
    ...
    if (
      !existsSync(
        path.join(cwd, ".atm", "history", "tasks", `${ownerTaskId}.json`),
      )
    )
      continue;
  ...
  const conflicts = Array.from(taskOwnerFiles.entries())
    ...
    .map(([conflictTaskId, conflictFiles]: LegacyValue) => ({
      conflictTaskId,
      conflictFiles: uniqueSorted(conflictFiles),
      owner: conflictTaskId,
      surface: "task-history",
    }));
  const conflictFiles = uniqueSorted(
    conflicts.flatMap((conflict: LegacyValue) => conflict.conflictFiles),
  );
  throw new CliError(
    "ATM_CROSS_TASK_MUTATION_BLOCKED",
    `git record-commit cannot combine low-risk record files owned by multiple tasks. File(s): ${conflictFiles.join(", ")}.`,
    {
      exitCode: 1,
      details: {
        taskId: null,
        ...
        conflicts,
        recoveryLane:
          "Split record maintenance by task owner, or use the governed closeback/reconcile lane that owns the full cross-task packet.",
      },
    },
  );
}

export function inspectMirrorSyncOnlyStagedArtifacts(cwd: LegacyValue, taskId: LegacyValue) {
  ...
  for (const file of stagedFiles) {
    ...
    if (
      lower.startsWith(`.atm/history/task-events/${taskId.toLowerCase()}/`) &&
    ...
    if (
      lower.startsWith(".atm/history/reports/task-import/") &&
      lower.endsWith(".json") &&
...
export function inspectHistoricalLedgerRestoreStagedArtifacts(cwd: LegacyValue, taskId: LegacyValue) {
  const stagedFiles = readStagedFiles(cwd);
  if (stagedFiles.length === 0) {
    return { ok: false, taskId, stagedFiles, reason: "no-staged-files" };
  }
  ...
  const expectedEvidencePath =
    `.atm/history/evidence/${taskId}.json`.toLowerCase();
  const expectedClosurePacketPath =
    `.atm/history/evidence/${taskId}.closure-packet.json`.toLowerCase();
  ...
  const taskDocument = readStagedJsonFile(
    cwd,
  ...
  const evidence = readStagedJsonFile(
    cwd,
    `.atm/history/evidence/${taskId}.json`,
  );
  for (const eventPath of stagedFiles.filter((file: LegacyValue) =>
  ...
  if (!hasEvidenceBundle) {
    ...
    const repairEventPaths = stagedFiles.filter((file: LegacyValue) =>
      normalizeRelativePath(file)
        .toLowerCase()
        .startsWith(`.atm/history/task-events/${normalizedTaskId}/`),
    );
    const repairOnly =
      repairEventPaths.length > 0 &&
      repairEventPaths.every((eventPath: LegacyValue) => {
        const event = readStagedJsonFile(cwd, eventPath);
