...
import { readHeadBranchRef, readHeadCommitSha } from './push-command.ts';
...
export function branchCommitQueueLockPath(cwd: LegacyValue, branchRef: LegacyValue) {
  ...
  return path.join(
    ...
    "runtime",
    "locks",
    `git-commit-queue-${safeName}.lock`,
  );
}

export function inspectCurrentBranchCommitQueueStatus(cwd: LegacyValue) {
  ...
  if (!existsSync(lockPath)) {
    return {
      ...
      recommendedAction:
        "No branch commit queue lock is present; it is safe to start a new governed commit if the worktree is otherwise ready.",
    };
  }
  ...
  const ownerAlive = record
    ...
    : null;
  const headMovedSinceAcquire = Boolean(
...
export function isBranchCommitQueueOwnerAlive(ownerPid: LegacyValue) {
  if (
...
export function readBranchCommitQueueLockRecord(lockPath: LegacyValue) {
  try {
    const raw = JSON.parse(
      readFileSync(path.join(lockPath, "record.json"), "utf8"),
    );
    ...
    return {
      ...
      branchRef:
        typeof raw.branchRef === "string" ? raw.branchRef : "detached-head",
      branchName:
        typeof raw.branchName === "string" ? raw.branchName : "detached-head",
      headShaAtAcquire:
        typeof raw.headShaAtAcquire === "string" ? raw.headShaAtAcquire : null,
...
export function recordBranchCommitQueueStaleCleanup(input: LegacyValue) {
  const auditPath = path.join(
    input.cwd,
    ".atm",
    "runtime",
    "locks",
    "branch-commit-queue-stale-cleanup.jsonl",
  );
  mkdirSync(path.dirname(auditPath), { recursive: true });
  appendFileSync(
    auditPath,
    `${JSON.stringify({ schemaId: "atm.branchCommitQueueStaleCleanup.v1", cleanedAt: new Date().toISOString(), actorId: input.actorId, lockPath: relativePathFrom(input.cwd, input.lockPath), lockRecord: input.record, currentHeadSha: input.currentHeadSha, ownerAlive: input.ownerAlive, ageMs: input.ageMs, reason: input.reason })}\n`,
    "utf8",
  );
}
...
export function withBranchCommitQueueLock(input: LegacyValue, operation: LegacyValue) {
  ...
  while (true) {
    try {
      ...
      writeFileSync(
        path.join(lockPath, "record.json"),
...
export function sleepMs(ms: LegacyValue) {
