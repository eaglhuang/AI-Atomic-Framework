...
export function buildHostGitCompatibilityGuidance(input: LegacyValue) {
  ...
  if (combined.includes('enametoolong') || combined.includes('argument list too long')) {
    lines.push(
      'Windows pathspec fallback: the pre-commit hook reads ATM_COMMIT_ACTOR_ID, ATM_COMMIT_TASK_ID, ATM_COMMIT_CLAIM_ID, ATM_COMMIT_SESSION_ID, and ATM_COMMIT_LANE_SESSION_ID; ATM_TASK_ID is not a substitute. Preserve the exact values from the governed commit attempt.',
    );
  }
  if (
