...
try {
  ...
  assert.equal(
    resolution.roots.some((entry) => entry.startsWith(`${stale}${path.sep}`)),
    false,
    'derivative sibling planning root must be excluded when canonical exists'
  );
...
// coversAcceptance: ACC-4, ACC-5
{
  ...
  try {
    const makePlanningRepo = (name: string, opts: { hasSeriesRegistry: boolean; activeSeriesCount: number }): string => {
      ...
      if (opts.hasSeriesRegistry) {
        ...
        writeFileSync(
          path.join(planningRoot, 'series-registry.json'),
          JSON.stringify({
    ...
    assert.equal(
      selection.resolvedRoots[0],
      path.resolve(regRepo, 'docs', 'ai_atomic_framework'),
      'must select the registered planning authority over unregistered candidate'
    );
