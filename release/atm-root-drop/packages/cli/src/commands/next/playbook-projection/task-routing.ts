...
import { quoteCliValue, toTaskCandidateView, uniqueSorted } from '../view-projections.ts';
...
export function detectActiveTaskDivergence(
  ...
  const outsidePromptPaths = promptPaths.filter((entry) => {
    if (isPathAllowedByScope(entry, activeScope)) return false;
...
function canonicalPromptPath(value: string): string {
