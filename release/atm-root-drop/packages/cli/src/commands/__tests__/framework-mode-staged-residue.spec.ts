...
async function testCrossFileConsistency() {
  ...
  try {
    ...
    const unrelatedResult = await runHook(['pre-commit', '--cwd', tempRoot]) as any;
    assert.equal(
      unrelatedResult.evidence.blockingFindings.some(
        (finding: any) => finding.code === 'ATM_PRE_COMMIT_CROSS_FILE_INCONSISTENCY'
      ),
      false,
      'a staged change that does not touch the import must not be blocked by cross-file consistency'
    );
