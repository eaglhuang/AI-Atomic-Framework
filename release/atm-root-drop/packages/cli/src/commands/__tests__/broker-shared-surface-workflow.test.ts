...
import { evaluateBrokerQueueAdmission } from '../next/broker-queue-admission.ts';
import type { WriteIntent } from '../../../../core/src/broker/types.ts';

function intent(taskId: string, actorId: string, files: readonly string[]): WriteIntent {
  return {
...
async function register(cwd: string, value: WriteIntent) {
...
async function testQueueFreezeAckReleaseWorkflow() {
  ...
  try {
    ...
    const queued = await register(cwd, waiter) as any;
    ...
    await assert.rejects(
      () => runBroker(['acknowledge', '--cwd', cwd, '--task', 'TASK-WAITER', '--actor', 'waiter-agent', '--freeze-id', freezeId]),
      (error: { code?: string }) => error.code === 'ATM_BROKER_FREEZE_ACK_FORBIDDEN'
    );
...
async function testBaseMismatchFailsClosed() {
  ...
  try {
    ...
    await assert.rejects(
      () => register(cwd, mismatched),
      (error: { code?: string }) => error.code === 'ATM_BROKER_SHARED_QUEUE_BLOCKED'
    );
...
async function testWaitingTaskCanAbandonWithoutLeavingGhostQueueEntry() {
  ...
  try {
    ...
    assert.deepEqual(queueDocument.queues[0].entries.map((entry: { taskId: string }) => entry.taskId), ['TASK-OWNER']);
...
async function testDisjointFilesRemainParallel() {
  ...
  try {
    ...
    const result = await register(cwd, intent('TASK-RIGHT', 'right-agent', ['src/right.ts'])) as any;
...
async function testNextClaimExcludesQueuedPathsFromDirectionLock() {
  ...
  try {
    ...
    for (const entry of queueDocument.queues[0].entries) entry.baseHash = fixtureHead;
...
function taskDocument(taskId: string, status: string, scopePaths: readonly string[]) {
  return {
