...
export function createSkipMatcher(skips: readonly string[], cwd: string) {
...
export function normalizeActiveCaptains(values: readonly string[]): readonly string[] {
...
export function readActiveReleaseCaptainsFromEnv(stewardActorId: string): readonly string[] {
...
export function runNodeAtm(cwd: string, args: readonly string[]): CommandRunRecord {
  return runCommand(cwd, process.execPath, ['atm.mjs', ...args]);
}

export function runCommand(cwd: string, command: string, args: readonly string[]): CommandRunRecord {
...
export function readGitScalar(cwd: string, args: readonly string[]) {
  const result = spawnSync('git', args, { cwd, encoding: 'utf8' });
  return !result.error && result.status === 0 ? result.stdout.trim() : null;
}

export function readGitDirtyFiles(cwd: string): readonly string[] {
...
export function requireValue(argv: readonly string[], index: number, flag: string) {
