...
export function checkOnboardingLifecycle(root: string, runtime: ReturnType<typeof detectGovernanceRuntime>) {
  ...
  const standaloneInitialization = adapter
    ...
    && (adapter as Record<string, unknown>).mode === 'standalone'
    && (adapter as Record<string, unknown>).implemented === false;
...
export function createVersionSummaryMessages(versionSummary: ReturnType<typeof createATMVersionSummary>) {
  const messages = [];
...
export function readATMChartFrontmatter(filePath: string): Record<string, unknown> | null {
  try {
