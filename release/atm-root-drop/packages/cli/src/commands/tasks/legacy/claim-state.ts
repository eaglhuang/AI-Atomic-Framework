 ...
 */
export function hasProtectedActiveClaim(
...
export function hasPreservableClaimState(
  document: Record<string, unknown> | null,
): boolean {
  if (!document) return false;
  if (hasProtectedActiveClaim(document)) return true;
  const claim = document.claim;
  if (!claim || typeof claim !== "object" || Array.isArray(claim)) return false;
  const state = String((claim as Record<string, unknown>).state ?? "").trim();
