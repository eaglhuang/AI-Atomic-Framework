...
export interface SingleCardContractValidationResult {
  readonly parsed: ParsedPlanResult;
  readonly contractRecovery: ContractImportRecoveryManifest | null;
}
...
export function applySingleCardContractValidation(input: {
  readonly cwd: string;
  readonly parsed: ParsedPlanResult;
  readonly causalFrontmatter?: Record<string, unknown> | null;
  readonly planText: string;
  readonly planPath: string;
}): SingleCardContractValidationResult {
  ...
  const dependencyValidation = validateHardCausalDependencyImport({
    taskId: task.workItemId,
    taskDocument: parsed.tasks[0] as unknown as Record<string, unknown>,
    cwd: input.cwd
  });
