...
export const canonicalTaskIdAtStartPattern = new RegExp(
  `^${CANONICAL_TASK_ID_SOURCE}${CANONICAL_TASK_ID_RIGHT_BOUNDARY_SOURCE}`,
  'i'
);

export const canonicalTaskIdAnywherePattern = new RegExp(
  `(?:^|[^A-Z0-9-])(${CANONICAL_TASK_ID_SOURCE})${CANONICAL_TASK_ID_RIGHT_BOUNDARY_SOURCE}`,
  'i'
);

export function extractCanonicalTaskIdAtStart(value: string): string | null {
  return canonicalTaskIdAtStartPattern.exec(value)?.[0] ?? null;
}

export function extractCanonicalTaskId(value: string): string | null {
  const match = canonicalTaskIdAnywherePattern.exec(value);
  return match?.[1] ?? null;
}

export function isCanonicalTaskIdDeclaration(value: string): boolean {
