...
export type ClaimRepairIssueKind =
  | 'valid-active-claim'
  | 'expired-claim'
  | 'stale-running-without-claim'
  | 'stale-claim-released'
  | 'dangling-governance-lock'
...
function readGovernanceLock(cwd: string, taskId: string): Record<string, unknown> | null {
...
function isLockReleased(lock: Record<string, unknown> | null): boolean {
  if (!lock) return true;
  return lock.released === true || lock.status === 'released';
}

function readLockActorId(lock: Record<string, unknown> | null): string | null {
  if (!lock) return null;
  const embedded = lock.taskDirectionLock;
  if (embedded && typeof embedded === 'object' && !Array.isArray(embedded)) {
    const actorId = (embedded as Record<string, unknown>).actorId;
...
function readLaneSessionId(value: unknown): string | null {
  ...
  if (laneSession && typeof laneSession === 'object' && !Array.isArray(laneSession)) {
    return normalizeOptionalString((laneSession as Record<string, unknown>).laneSessionId)
      ?? normalizeOptionalString((laneSession as Record<string, unknown>).laneId);
  }
...
export function diagnoseClaimRepairState(cwd: string, taskId: string, actorId?: string | null): ClaimRepairDiagnosis {
