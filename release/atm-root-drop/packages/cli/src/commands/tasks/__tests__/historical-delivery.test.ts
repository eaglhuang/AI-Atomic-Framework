...
import {
  ...
  readPlanningCardDeliveryCommit
} from '../historical-delivery.ts';
import { runEvidence, verifyTaskEvidence } from '../../evidence.ts';
...
try {
  ...
  const record = taskEvidence.evidence?.find((entry: any) => entry.summary === `Historical batch evidence ${batchResult.evidence.batchId} for TASK-HIST.`);
  ...
  const freshAttestation = taskEvidence.evidence?.find((entry: any) => entry.summary === `Fresh validator attestation slice ${batchResult.evidence.batchId} for TASK-HIST.`);
...
try {
  ...
  assertEqual(
    readPlanningCardDeliveryCommit(planningCardRepo, 'docs/tasks/TASK-HIST.task.md'),
    'abc123def',
    'readPlanningCardDeliveryCommit must parse quoted delivery_commit frontmatter'
  );
} finally {
