...
import { CliError, makeResult, message, relativePathFrom, type CommandResult } from '../shared.ts';
import { writeTaskDocumentWithTransition } from './close-helpers/task-transition-writer.ts';
import { applyClaimRepairWrite, buildRepairClaimCommand, diagnoseClaimRepairState } from './claim-repair-diagnostics.ts';
...
export async function runTasksRepairClaim(argv: string[]): Promise<CommandResult> {
