...
import { PLANNING_REPO_ROOT_ENV, resolvePlanningRepoRootConfig } from './planning-repo-root.ts';

const PLAN_COMMAND = 'plan';
const SERIES_REGISTRY_FILE = 'series-registry.json';
const ATM_PLANNING_ROOT_ENV = 'ATM_PLANNING_ROOT';

interface PlanSeriesEntry {
  readonly series: string;
  readonly prefix: string;
  readonly familyDir: string;
  readonly planDocs: readonly string[];
  readonly status: 'active' | 'archived' | 'reserved';
  readonly approvedBy: string;
  readonly approvedAt: string;
  readonly createdByCommand?: string;
  readonly creationSeal?: PlanningCreationSeal;
}

interface PlanSeriesRegistry {
  readonly schemaId: 'atm.seriesRegistry.v1';
  readonly generatedAt: string;
  readonly baseDir: string;
  readonly series: readonly PlanSeriesEntry[];
}

interface PlanningCreationSeal {
  readonly schemaId: 'atm.planningCreationSeal.v1';
...
interface ParsedPlanArgs {
  ...
  readonly familyDir: string | null;
...
export async function runPlan(argv: string[]) {
...
function runSeriesRegister(options: ParsedPlanArgs) {
...
function runDocCreate(options: ParsedPlanArgs) {
  const planningRoot = resolvePlanningRoot(options);
...
function runCardCreate(options: ParsedPlanArgs) {
  const planningRoot = resolvePlanningRoot(options);
  const title = required(options.title, '--title');
...
function parsePlanArgs(argv: string[]): ParsedPlanArgs {
...
function resolvePlanningRoot(options: ParsedPlanArgs): string {
...
function readSeriesRegistry(planningRoot: string): PlanSeriesRegistry {
  const registryPath = path.join(planningRoot, SERIES_REGISTRY_FILE);
  ...
  const parsed = JSON.parse(readFileSync(registryPath, 'utf8')) as PlanSeriesRegistry;
...
function writeSeriesRegistry(registryPath: string, registry: PlanSeriesRegistry, dirs: readonly string[] = []): readonly string[] {
...
function upsertSeries(registry: PlanSeriesRegistry, entry: PlanSeriesEntry, planningRoot: string): PlanSeriesRegistry {
...
function findSeries(registry: PlanSeriesRegistry, rawSeries: string | null): PlanSeriesEntry | null {
...
function resolveFamilyDirFromSeries(planningRoot: string, rawSeries: string | null): string {
  if (!rawSeries) throw usage('doc create requires either --family-dir or --series.');
  const entry = findSeries(readSeriesRegistry(planningRoot), rawSeries);
  if (!entry) throw new CliError('ATM_PLAN_SERIES_NOT_REGISTERED', `series is not registered: ${rawSeries}`, { exitCode: 1 });
  return entry.familyDir;
}

function maybeAddPlanDocToRegistry(planningRoot: string, rawSeries: string, planDoc: string, write: boolean): Record<string, unknown> | null {
  const registry = readSeriesRegistry(planningRoot);
  const entry = findSeries(registry, rawSeries);
  if (!entry) return { status: 'series-not-registered', series: rawSeries };
  ...
  if (write) writeSeriesRegistry(path.join(planningRoot, SERIES_REGISTRY_FILE), nextRegistry);
  return { status: write ? 'updated' : 'dry-run', series: entry.series, planDoc };
}

function nextTaskId(planningRoot: string, entry: PlanSeriesEntry): string {
...
function renderPlanDoc(input: { title: string; familyDir: string; planningRoot: string; relativePath: string; command: string }): string {
  const withoutSeal = [
    '---',
    `doc_id: pending`,
    `title: ${input.title}`,
...
function withCreationSeal(content: string, seal: PlanningCreationSeal): string {
  return `${content.trimEnd()}\n\n<!-- atmPlanningCreationSeal ${JSON.stringify(seal)} -->\n`;
}

function buildCreationSeal(input: { command: string; planningRoot: string; relativePath: string; content: string }): PlanningCreationSeal {
  return {
...
function writeTextNewFile(absolutePath: string, content: string): readonly string[] {
...
function required(value: string | null, flag: string): string {
  if (!value) throw usage(`missing required ${flag}`);
  return value;
}
...
function normalizeTaskPrefix(value: string): string {
  const raw = value.trim().toUpperCase();
  if (!raw) throw usage('task prefix is required');
  if (/^(TASK|ATM)-[A-Z0-9]+$/.test(raw)) return raw;
  if (/^[A-Z0-9]+$/.test(raw)) return `TASK-${raw}`;
...
function normalizeFamilyDir(value: string): string {
...
function normalizeSeriesStatus(value: string | null): PlanSeriesEntry['status'] {
