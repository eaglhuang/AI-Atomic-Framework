...
import { classifyBlockLifecycleRecordBundle, isRecordCommitBlockBridgeAuthorized, recordOnlyClaimScopeExemptCovers, RECORD_COMMIT_BLOCK_BRIDGE_AUTH_DIR, RECORD_COMMIT_BLOCK_BRIDGE_AUTH_ENV } from '../../git-governance/record-only-block-lifecycle-bridge.js';
...
export function authorizeBlockLifecycleRecordBridge(root, crossTaskBlock) {
    ...
    if (nonce.length > 0) {
        ...
        if (existsSync(authPath)) {
            try {
                authorization = JSON.parse(readFileSync(authPath, 'utf8'));
            }
            catch {
                authorization = null;
            }
    ...
    const sha256 = (filePath) => createHash('sha256').update(readFileSync(path.join(root, filePath))).digest('hex');
...
export function raiseCrossTaskMutationBlock(input) {
    ...
    const ownershipState = crossTaskBlock.conflicts.find((entry) => entry.surface === 'task-history')?.ownershipState ?? null;
    ...
    throw new CliError('ATM_CROSS_TASK_MUTATION_BLOCKED', `Cross-task mutation incident detected: files owned by ${ownershipSummary} are mutated. File(s): ${crossTaskBlock.conflictFiles.join(', ')}. Recovery: ${recovery}`, {
        exitCode: 1,
        details: {
