...
export function runPreCommitHook(cwd) {
    ...
    const autoCleanedResidue = cleanupAutoGeneratedResidue(root, reconciledResidueReport.autoCleanSafe);
    const stagedFiles = readStagedFiles(root).filter((entry) => entry !== gitHeadEvidencePaths.legacyJson && entry !== gitHeadEvidencePaths.jsonl);
    const actionableResidueFindings = selectActionableResidueFindings({
        findings: [...reconciledResidueReport.blockAndExplain, ...reconciledResidueReport.manualReview.filter((entry) => isActionableManualResidue(entry.path))],
        stagedFiles,
