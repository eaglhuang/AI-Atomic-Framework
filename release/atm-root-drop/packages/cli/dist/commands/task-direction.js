...
export function assertTaskCloseAllowedByDirection(cwd, taskId, actorId) {
...
function readTaskQueue(cwd, queueId) {
...
function dedupeDirectionLocks(locks) {
    const seen = new Set();
    const output = [];
...
function readGovernanceDirectionLockForTask(cwd, taskId) {
    const lockPath = path.join(cwd, '.atm', 'runtime', 'locks', `${taskId}.lock.json`);
    if (!existsSync(lockPath))
        return null;
    try {
...
function matchesPlanningMirrorPath(filePath, mirrorPath) {
    if (!mirrorPath)
        return false;
