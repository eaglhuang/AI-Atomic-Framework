...
import { ATM_INDEX_FOREIGN_ACTIVE_STAGED, authorizeGitIndexOverrideLease, buildForeignActiveStagedDiagnostic, consumeGitIndexOverrideLease, inspectGitIndexOwnership, parkGitIndexLease, restoreGitIndexLease, } from "../git-index-ownership.js";
...
function deferStagedFilePaths(cwd, taskId, filesInput) {
    ...
    writeFileSync(path.join(cwd, snapshotPath), `${JSON.stringify({ schemaId: "atm.foreignStagedSnapshot.v1", taskId, createdAt: new Date().toISOString(), files }, null, 2)}\n`, "utf8");
...
function maybeCleanupStaleBranchCommitQueueLock(input) {
    ...
    const headMoved = Boolean(record.headShaAtAcquire &&
