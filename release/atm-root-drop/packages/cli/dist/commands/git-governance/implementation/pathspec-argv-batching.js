 ...
 */
export function resolvePathspecArgvBudget(platform = process.platform) {
...
export function estimateArgvBytes(args) {
    let total = 0;
...
export function planPathspecBatches(input) {
    ...
    for (const filePath of paths) {
        ...
        if (fixedBytes + pathBytes > budgetBytes) {
            throw new CliError('ATM_GIT_PATHSPEC_ARGV_BUDGET_EXCEEDED', 'A single pathspec exceeds the platform argument-vector budget, so no batching can make this Git invocation runnable.', {
                exitCode: 1,
                details: {
                    schemaId: PATHSPEC_ARGV_BUDGET_SCHEMA_ID,
                    path: filePath,
 ...
 */
export function forEachPathspecBatch(input, invoke) {
