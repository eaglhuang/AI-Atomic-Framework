...
export function classifyPostPushRecoveryKind(input) {
...
export function buildPostPushRecoveryRecommendation(input) {
    const defaultRecommendation = String(input.defaultRecommendation ?? '').trim();
    if (/ATM_WRITE_TICKET_HISTORICAL_ATTESTATION_REQUIRED/i.test(defaultRecommendation) ||
...
export function isHeadRaceCommitFailure(stderr) {
    return (/cannot lock ref 'HEAD'/i.test(stderr) &&
