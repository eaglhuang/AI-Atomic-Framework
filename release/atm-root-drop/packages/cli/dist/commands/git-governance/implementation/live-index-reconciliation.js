...
import { forEachPathspecBatch, planPathspecBatches } from './pathspec-argv-batching.js';
import { isIndexLockContention, withIndexLockRetry } from './live-index-lock-retry.js';
...
const LS_FILES_ARGS = ['ls-files', '-s', '--'];
const ADD_ARGS = ['add', '-A', '-f', '--'];
...
function collectIndexEntries(entries, output) {
...
export function readIndexEntries(cwd, paths, env, budgetBytes) {
    const entries = Object.fromEntries(paths.map((filePath) => [filePath, null]));
...
export function readTreeEntries(cwd, treeish, paths, budgetBytes) {
...
function readHeadEntries(cwd, paths, budgetBytes) {
    return readTreeEntries(cwd, 'HEAD', paths, budgetBytes);
}
export function applyLiveIndexHeadEntry(cwd, filePath, target, retry = {}) {
...
export function readWorktreeEntries(cwd, paths, headEntries, budgetBytes) {
    ...
    try {
        ...
        const stageablePaths = paths.filter((filePath) => headEntries[filePath] !== null || existsSync(path.join(cwd, filePath)));
...
export function captureLiveIndexSnapshot(cwd, pathsInput, options = {}) {
...
export function reconcileLiveIndexAfterCommitAttempt(input) {
    ...
    try {
        ...
        if (!headAdvanced)
            return report({ headAdvanced: false });
 ...
 * governance forbids.
 */
export function recordLiveIndexReconciliation(cwd, taskId, reconciliation) {
    if (!taskId || reconciliation.clean !== false)
        return null;
    ...
    if (existsSync(absolutePath)) {
        try {
            existing = JSON.parse(readFileSync(absolutePath, 'utf8'));
        }
        catch {
    ...
    if (existing?.schemaId === LIVE_INDEX_RECONCILIATION_SCHEMA_ID
    ...
    writeFileSync(absolutePath, `${JSON.stringify({ ...reconciliation, retainedPaths, taskId, createdAt: new Date().toISOString() }, null, 2)}\n`, 'utf8');
    return relativePath;
}
/**
 ...
 */
function stampFirstUnreconciledCommit(cwd, retainedPaths, existing) {
    if (retainedPaths.length === 0)
        return retainedPaths;
    const known = new Map();
    const prior = Array.isArray(existing?.retainedPaths)
        ? existing.retainedPaths
        : [];
...
function resolveCommitSha(cwd, commitSha) {
    try {
        return runGitCommand(cwd, ['rev-parse', '--verify', `${commitSha}^{commit}`], QUIET_STDIO).trim();
    }
    catch {
        throw new CliError('ATM_LIVE_INDEX_RECOVERY_COMMIT_INVALID', 'Historical live-index recovery requires a resolvable commit object.', { exitCode: 1, details: { commitSha } });
    }
}
function resolveParentSha(cwd, commitSha) {
    try {
        ...
        throw new CliError('ATM_LIVE_INDEX_RECOVERY_PARENT_MISSING', 'Historical live-index recovery requires the named commit to have a parent tree as the proven pre-state.', { exitCode: 1, details: { commitSha } });
    }
}
function assertNamedCommitIsCurrentOrAncestor(cwd, commitSha, headSha) {
    ...
    throw new CliError('ATM_LIVE_INDEX_RECOVERY_HEAD_MISMATCH', 'Historical live-index recovery requires HEAD to equal the named commit, or to be a descendant that has not rewritten those paths.', { exitCode: 1, details: { commitSha, headSha } });
}
function listCommitPaths(cwd, parentSha, commitSha) {
    const output = runGitCommand(cwd, ['diff-tree', '--no-commit-id', '--name-only', '-r', parentSha, commitSha], QUIET_STDIO);
    return output
        .split(/\r?\n/)
...
function recordHistoricalLiveIndexRecovery(cwd, recovery) {
    const relativePath = `.atm/history/evidence/live-index-reconciliation.${recovery.commitSha}.json`;
    const absolutePath = path.join(cwd, relativePath);
    mkdirSync(path.dirname(absolutePath), { recursive: true });
    writeFileSync(absolutePath, `${JSON.stringify({ ...recovery, evidencePath: relativePath, createdAt: new Date().toISOString() }, null, 2)}\n`, 'utf8');
    return relativePath;
}
/**
 ...
 */
export function attachLiveIndexReconciliation(error, reconciliation) {
...
export function readLiveIndexReconciliationFromError(error) {
    if (error === null || typeof error !== 'object')
        return null;
    const carried = error[RECONCILIATION_ON_ERROR];
    return carried ?? null;
}
