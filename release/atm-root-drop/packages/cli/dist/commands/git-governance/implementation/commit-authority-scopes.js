import { pathMatchesTaskScope, uniqueSorted } from '../commit-scope-policy.js';
import { parseTaskClaim } from './identity-check-command.js';
function stringScopes(value) {
    return Array.isArray(value)
        ? uniqueSorted(value.filter((entry) => typeof entry === 'string' && entry.trim().length > 0))
        : [];
}
/**
 ...
 */
function resolveCommitAuthorityScopes(input) {
    ...
    const fileGrant = matchingTicket && Array.isArray(matchingTicket.grants)
        ? matchingTicket.grants.find((grant) => grant?.kind === 'file-write')
        : null;
...
function evaluatePath(filePath, authority) {
    const decision = (ok, code, reason) => ({ schemaId: 'atm.commitPathAuthorityDecision.v1', ok, code, reason });
    if (!authority.enforced)
        return decision(true, 'legacy-inspection', 'No claim exists; later mutation admission remains authoritative.');
    if (authority.claimScopes.length === 0)
        return decision(false, 'claim-not-active', 'No active claim scope authorizes source bytes.');
...
export function createCommitAuthorityPolicy(input) {
