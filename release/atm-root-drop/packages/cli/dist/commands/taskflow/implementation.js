...
function buildRosterSyncCommand(input) { const parts = ['node atm.mjs tasks roster update', `--index ${input.indexPath}`, `--from ${input.fromPath}`]; if (input.dryRun) {
    parts.push('--dry-run');
} parts.push('--json'); return parts.join(' '); }
async function runRosterSyncFollowUp(input) {
    let result = null;
    try {
        result = await runTasksRosterUpdate(['--cwd', input.cwd, '--index', input.indexPath, '--from', input.fromPath]);
    }
    catch (error) {
        const caught = error instanceof Error ? { message: error.message, name: error.name } : { message: String(error), name: 'UnknownError' };
        result = makeResult({ ok: false, command: 'tasks roster update', cwd: input.cwd, mode: 'standalone', messages: [message('error', 'ATM_TASK_ROSTER_SYNC_FOLLOWUP_EXCEPTION', `Roster follow-up command failed before writing: ${caught.message}`, caught)],
            evidence: { indexPath: input.indexPath, fromPath: input.fromPath, followUpCommand: input.command, syncMode: 'follow-up-command', failure: caught } });
    }
    if (!result.ok) {
    ...
    return { mode: 'follow-up-command', command: input.command, result };
}
function buildTasksImportCommand(input) { return `node atm.mjs tasks import --from ${quoteCliValue(input.fromPath)} --write --json`; }
...
function readPackageJsonForAutoEvidence(cwd) {
...
function buildOrchestrationPlan(input) {
    ...
    if (rosterSyncPolicy === 'follow-up-command' && rosterIndexPath && resolvedOutputPath) {
        rosterFollowUpCommand = buildRosterSyncCommand({ indexPath: rosterIndexPath, fromPath: resolvedOutputPath });
    ...
    return {
        generationSurface: 'tasks-new', wouldInvokeTasksNew: true, wouldInvokeTasksImport: Boolean(resolvedOutputPath), tasksNewCommand: buildTasksNewCommand({ taskId: resolvedTaskId, outputPath: resolvedOutputPath, template: input.template, title: input.title }), tasksImportCommand: resolvedOutputPath ? buildTasksImportCommand({
            fromPath: input.outputRoot ? resolveOutputAbsolute(input.outputRoot, resolvedOutputPath) : resolvedOutputPath
        }) : null, hostOpenerInvocation: input.delegationContract.displayHint, planningCard: resolvedOutputPath, targetRepository: input.profile?.ownerRepo ?? 'adopter-repo', nextDryRunCommand: buildTaskflowOpenDryRunCommand({ profilePath: input.profilePath, taskId: resolvedTaskId, outputPath: resolvedOutputPath,
            title: input.title }), nextImportCommand: resolvedOutputPath ? buildTasksImportCommand({ fromPath: input.outputRoot ? resolveOutputAbsolute(input.outputRoot, resolvedOutputPath) : resolvedOutputPath }) : null, rosterSyncPolicy, rosterIndexPath, rosterFollowUpCommand, followUpRequired: input.openerMode === 'template-only-fallback' || !resolvedTaskId || !resolvedOutputPath
            || (rosterSyncPolicy === 'follow-up-command' && Boolean(rosterFollowUpCommand)), followUpSteps, targetRepo: input.profile?.ownerRepo ?? 'adopter-repo', outputRepoRoot: input.outputRoot ?? null, profileRepoLabel: input.profile?.repoLabel ?? 'adopter-repo', policyDecision: { allocateTaskId: input.delegationContract.policy.allocateTaskId,
            resolveCanonicalOutputPath: input.delegationContract.policy.resolveCanonicalOutputPath, rosterSyncPolicy, rosterSyncIndexPath: rosterIndexPath, fallbackBehavior: input.delegationContract.policy.fallbackBehavior }, hostPolicyDecision: input.hostPolicyDecision ?? null
    };
}
...
function buildWriteReadinessHint(input) {
    ...
    const summary = status === 'fallback'
        ? 'taskflow open --write will fail closed in template-only-fallback mode. Configure the listed prerequisites, or use tasks new as the explicit low-level generator surface.' : 'taskflow open --write prerequisites are incomplete. Resolve the listed items before retrying --write.';
    return { schemaId: 'atm.taskflowOpenWriteReadinessHint.v1', status, summary, missingPrerequisites: missing, nextCommand: null,
        operatorLane: 'taskflow open', fallbackSurface: 'tasks new (low-level generator)' };
}
...
function collectWaiverOutOfScopeDelivery(parsed) {
    ...
    const reason = typeof parsed.options.reason === 'string' && parsed.options.reason.trim() ? parsed.options.reason.trim() : null;
    if (waiverOutOfScopeDelivery && !reason) {
        throw new CliError('ATM_TASKFLOW_CLOSE_WAIVER_REASON_REQUIRED', 'taskflow close --waiver-out-of-scope-delivery requires --reason <text>.', { exitCode: 2 });
    }
    return { waiverOutOfScopeDelivery, waiverReason: reason };
}
...
function loadHistoricalBatchMatchedCommits(cwd, taskId, batchRef) {
    ...
    if (!batchPath || !existsSync(batchPath)) {
        throw new CliError('ATM_TASKFLOW_CLOSE_HISTORICAL_BATCH_NOT_FOUND', `Historical batch evidence not found for ${batchRef}.`, { exitCode: 1,
    ...
    if (!rawSlice) {
        throw new CliError('ATM_TASKFLOW_CLOSE_HISTORICAL_BATCH_TASK_NOT_FOUND', `Historical batch ${batchRef} does not contain task ${taskId}.`, { exitCode: 1, details: { taskId, batchRef, batchPath: relativePathFrom(cwd, batchPath) } });
    }
    if (rawSlice.okToCloseTask !== true) {
        throw new CliError('ATM_TASKFLOW_CLOSE_HISTORICAL_BATCH_NOT_CLOSE_READY', `Historical batch ${batchRef} task slice for ${taskId} is not close-ready.`, { exitCode: 1, details: { taskId, batchRef, batchPath: relativePathFrom(cwd, batchPath), coverageStatus: rawSlice.coverageStatus ?? null, okToRecordEvidence: rawSlice.okToRecordEvidence === true,
                okToCloseTask: rawSlice.okToCloseTask === true, diagnosticOnly: rawSlice.diagnosticOnly === true, missingCoverage: Array.isArray(rawSlice.missingCoverage) ? rawSlice.missingCoverage : [] } });
    }
    return Array.isArray(rawSlice.matchedCommits) ? rawSlice.matchedCommits.filter((entry) => typeof entry === 'string' && entry.trim().length > 0) : [];
}
...
function uniqueSorted(values) { return [...new Set(values.map((value) => value.replace(/\\/g, '/')).filter(Boolean))].sort((a, b) => a.localeCompare(b)); }
function readCurrentGitHead(cwd) { try {
    return execFileSync('git', ['rev-parse', 'HEAD'], { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim() || null;
}
catch {
    return null;
} }
const runnerSyncSurfaces = ['release/atm-onefile/atm.mjs', 'release/atm-root-drop', 'packages/cli/dist'];
const runnerGateBuildInputPatterns = ['packages/', 'scripts/', 'templates/', 'schemas/', 'atomic_workbench/', 'package.json', 'package-lock.json', 'tsconfig.json', 'tsconfig.build.json'];
function isRunnerGateBuildInput(filePath) { const normalized = filePath.replace(/\\/g, '/').replace(/^\.\//, ''); return runnerGateBuildInputPatterns.some((pattern) => pattern.endsWith('/') ? normalized.startsWith(pattern) : normalized === pattern); }
function buildTaskflowRunnerGateDecision(filePaths) { const scopeClass = deriveAtmScopeClass(filePaths); const runnerGateIntersectingFiles = uniqueSorted(scopeClass.classifications.filter((entry) => entry.scopeClass.includes('code') && isRunnerGateBuildInput(entry.path)).map((entry) => entry.path)); const runnerGateDecision = scopeClass.hasCode ? 'required' : 'skipped-non-code'; return { schemaId: 'atm.taskflowRunnerGateDecision.v1', runnerGateDecision, runnerGateIntersectingFiles, sharedSurface: runnerGateDecision === 'required' ? 'runner-sync' : null, scopeClass }; }
...
const runnerGateBuildInputTreePaths = RUNNER_INPUT_TREE_PATHS;
...
export function buildCloseOwnedDirtyPendingBlocker(input) { const blockingDirty = new Set(uniqueSorted(input.dirtyGuard.blockingTrackedDirtyFiles ?? [])); const closeOwnedFiles = uniqueSorted([...input.previewCommitBundle.targetRepo.stageFiles, ...input.previewCommitBundle.targetGovernanceFiles, ...input.previewCommitBundle.planningRepo.stageFiles, ...input.previewCommitBundle.planningFiles]); const pendingFiles = closeOwnedFiles.filter((file) => blockingDirty.has(file)); if (pendingFiles.length === 0)
    return null; const preview = pendingFiles.slice(0, 4).join(', '); const suffix = pendingFiles.length > 4 ? ` (+${pendingFiles.length - 4} more)` : ''; return { code: 'ATM_TASKFLOW_CLOSE_OWNED_DIRTY_PENDING', summary: `taskflow close owns blocking dirty file(s) (${preview}${suffix}); resolve the listed delivery residue before close --write.`, requiredCommand: `node atm.mjs taskflow pre-close --task ${input.taskId} --actor ${quoteCliValue(input.actorId || '<actor>')} --json`, files: pendingFiles, multiTaskCloseRecipe: 'Resolve or reconcile the listed blocking files through their governed delivery route, then rerun taskflow pre-close.' }; }
...
function tryGitScalar(cwd, args) {
    try {
        return execFileSync('git', [...args], { cwd, encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'pipe'] }).trim() || null;
    }
    catch {
        return null;
    }
}
function readGitRoot(startPath) { const probe = existsSync(startPath) && statSync(startPath).isDirectory() ? startPath : path.dirname(startPath); const root = tryGitScalar(probe, ['rev-parse', '--show-toplevel']); return root ? path.resolve(root) : null; }
function resolveProfileRepoRoot(profilePath, fallbackCwd) { if (!profilePath)
    return fallbackCwd; const resolvedProfilePath = path.resolve(profilePath); return readGitRoot(resolvedProfilePath) ?? path.dirname(resolvedProfilePath); }
...
async function runTaskflowClose(parsed, cwd, surface = 'close') {
    ...
    const commitMode = writeRequested ? noCommitRequested ? 'stage-only' : 'auto-commit' : 'dry-run';
    ...
    const historicalBatchMatchedCommits = historicalBatchRef ? loadHistoricalBatchMatchedCommits(cwd, taskId, historicalBatchRef) : [];
    const historicalDeliveryRefs = uniqueSorted([...explicitHistoricalDeliveryRefs, ...historicalBatchMatchedCommits]);
    if (!taskId) {
    ...
    const profileRepoRoot = profilePath && profileData
        ? resolveProfileRepoRoot(profilePath, cwd) : null;
    const closebackPathResolution = resolveClosebackPlanningPath({ cwd, taskId, taskDocument, profile: profileData, profileRepoRoot, delegationContract });
    if (profileData || writeRequested) {
        assertClosebackPlanningPathReady(closebackPathResolution, { profileSupplied: Boolean(profileData), requirePlanningPath: true });
    }
    const diagnosis = buildResidueDiagnosisEvidence(cwd, taskId, taskDocument);
    const enrichedDiagnosis = closebackPathResolution.planningMirrorPath ? { ...diagnosis, triangulation: { ...diagnosis.triangulation, planningFrontmatter: { status: closebackPathResolution.planningStatus ?? diagnosis.triangulation.planningFrontmatter.status, source: closebackPathResolution.planningMirrorPath } } } : diagnosis;
    const planningAuthorityDeliveryGate = inspectPlanningAuthorityDelivery({ cwd, taskDocument, historicalDeliveryRefs, resolvedPlanningMirrorPath: closebackPathResolution.planningMirrorPath });
    if (planningAuthorityDeliveryGate.required && historicalDeliveryRefs.length > 0 && !planningAuthorityDeliveryGate.ok) {
        throw new CliError('ATM_TASKFLOW_CLOSE_PLANNING_DELIVERY_INVALID', 'taskflow close could not verify the supplied planning-repo delivery commit against the task deliverables.', { exitCode: 1, details: { taskId, planningAuthorityDeliveryGate, historicalDeliveryRefs } });
    }
    const closebackPlan = buildClosebackPlan({ taskId, actorId: actorId || '<actor>', historicalDeliveryRefs, historicalBatchRef,
        waiverOutOfScopeDelivery: waiver.waiverOutOfScopeDelivery, waiverReason: waiver.waiverReason, planningAuthorityDeliveryGate, delegationContract, diagnosis: { bucket: enrichedDiagnosis.bucket, truth: enrichedDiagnosis.truth, residue: enrichedDiagnosis.residue, reason: enrichedDiagnosis.reason, nextCommand: enrichedDiagnosis.nextCommand, triangulation: enrichedDiagnosis.triangulation }, closebackPathResolution, cwd, taskDocument, targetFiles: [...resolveTaskflowDeclaredFiles(cwd, taskId, taskDocument)], planningFiles: closebackPathResolution.planningMirrorPath ? [closebackPathResolution.planningMirrorPath] : [] });
    const diagnostics = buildTaskflowCloseDiagnostics({ closeMode: closebackPlan.closeMode, writeRequested, actorSupplied: actorId.length > 0, taskIdSupplied: taskId.length > 0 });
    const previewCommitBundle = buildTaskflowCommitBundle({ cwd, taskId, actorId: actorId || null, commitMode, planningMirrorPath: closebackPlan.writerBoundary.planningMirrorPath,
    ...
    const rawHistoricalClosePreflight = buildTaskflowClosePreflight({ cwd, taskId, actorId: actorId || '<actor>', taskDocument, previewCommitBundle, historicalDeliveryRefs, deferForeignStaged, deferGovernanceDirty, waiverOutOfScopeDelivery: waiver.waiverOutOfScopeDelivery, waiverReason: waiver.waiverReason });
    ...
    const autoEvidencePlan = actorId ? buildAutoEvidencePlan({ cwd, taskId, actorId, mode: writeRequested && autoEvidenceRequested ? 'execute' : 'dry-run', commandMapper: (declared) => mapAutoEvidenceCommand(declared, packageJsonForAutoEvidencePlan).command }) : null;
    const preCloseWriteBlocked = writeReadinessHint.status === 'blocked';
    const preCloseMessageLevel = historicalClosePreflight.ok && !preCloseWriteBlocked ? 'info' : 'warn';
    const preCloseMessageCode = !historicalClosePreflight.ok ? 'ATM_TASKFLOW_PRECLOSE_BLOCKED' : preCloseWriteBlocked ? 'ATM_TASKFLOW_PRECLOSE_WRITE_NOT_READY' : 'ATM_TASKFLOW_PRECLOSE_READY';
    const preCloseMessageText = !historicalClosePreflight.ok ? `taskflow pre-close found ${historicalClosePreflight.blockers.length} blocker(s) for ${taskId}; resolve them before taskflow close --write.` : preCloseWriteBlocked ? `taskflow pre-close found no preflight blockers for ${taskId}, but close --write is not ready; inspect writeReadinessHint.blockers before leaving this lane.` : `taskflow pre-close found no blockers for ${taskId}; inspect writeRollbackSummary before --write.`;
    ...
    if (writeRequested && writeSupport.allowed) {
        ...
        if (autoEvidenceRequested) {
            ...
            autoEvidenceExecution = executeAutoEvidencePlan({ cwd, taskId, actorId, commandMapper: (declared) => mapAutoEvidenceCommand(declared, packageJson).command });
            if (!autoEvidenceExecution.ok) {
                throw new CliError('ATM_TASKFLOW_AUTO_EVIDENCE_FAILED', `Auto-evidence could not satisfy declared validators for ${taskId}.`, { exitCode: 1, details: { taskId, failedValidator: autoEvidenceExecution.failedValidator, remediationCommand: autoEvidenceExecution.remediationCommand, autoEvidenceExecution } });
            }
        }
        if (previewCommitBundle.targetDeliveryFiles.length > 0 && commitMode !== 'auto-commit') {
            throw new CliError('ATM_TASKFLOW_CLOSE_DELIVERY_COMMIT_REQUIRED', 'taskflow close --write --no-commit cannot close dirty source deliverables because backend close requires a delivery commit first. Rerun without --no-commit or commit through the governed taskflow close operator lane.', { exitCode: 1, details: { taskId, governedCommitBundle: previewCommitBundle,
        ...
        const planningCardSnapshot = capturePlanningCardSnapshot({ cwd,
            planningMirrorPath: closebackPlan.writerBoundary.planningMirrorPath });
        const expectedCloseWindowStageFiles = uniqueSorted([...previewCommitBundle.targetRepo.stageFiles, ...previewCommitBundle.targetDeliveryFiles, ...previewCommitBundle.targetGovernanceFiles]);
        ...
        try {
            ...
            const preCloseDeliveryCommit = await commitTaskflowDeliveryFiles({ bundle: previewCommitBundle, actorId, taskId, deferForeignStaged });
            const effectiveHistoricalDeliveryRefs = preCloseDeliveryCommit?.commitSha ? uniqueSorted([...historicalDeliveryRefs, preCloseDeliveryCommit.commitSha]) : historicalDeliveryRefs;
            ...
            const rollbackSnapshot = buildCloseWriteRollbackSnapshot({ cwd, taskId, previousTaskContent, backendEvidence: backendResult.evidence, planningCard: planningCardSnapshot,
                closeWindowStagedIndexLockActive: closeWindowLock?.ok === true, preCloseStagedFiles });
            const planningCardCloseback = closebackPlan.backendSurface === 'tasks-close' && backendResult.ok ? applyPlanningCardCloseback({ cwd, planningMirrorPath: closebackPlan.writerBoundary.planningMirrorPath, actorId, historicalDeliveryRefs: effectiveHistoricalDeliveryRefs }) : null;
            const rollbackSnapshotWithPlanning = planningCardCloseback?.transitionPath && rollbackSnapshot.planningCard ? { ...rollbackSnapshot, planningCard: { ...rollbackSnapshot.planningCard, transitionPath: planningCardCloseback.transitionPath }, stagedArtifacts: [...rollbackSnapshot.stagedArtifacts, planningCardCloseback.transitionPath] } : rollbackSnapshot;
            let rosterCloseback = null;
            const closeMessages = [];
            const planningRosterPaths = closebackPlan.writerBoundary.rosterClosebackCommand && closebackPlan.writerBoundary.rosterIndexPath && closebackPlan.writerBoundary.planningMirrorPath ? resolvePlanningRosterPaths({ cwd, planningMirrorPath: closebackPlan.writerBoundary.planningMirrorPath, rosterIndexPath: closebackPlan.writerBoundary.rosterIndexPath }) : null;
            if (closebackPlan.writerBoundary.rosterClosebackCommand && closebackPlan.writerBoundary.rosterSyncPolicy === 'inline' && closebackPlan.writerBoundary.rosterIndexPath && closebackPlan.writerBoundary.planningMirrorPath) {
                if (!planningRosterPaths?.repoRoot || !planningRosterPaths?.indexPath || !planningRosterPaths?.fromPath) {
                    throw new CliError('ATM_TASKFLOW_CLOSE_PLANNING_ROSTER_UNRESOLVED', planningRosterPaths?.reason ?? 'taskflow close could not resolve planning roster paths.', { exitCode: 1, details: { closebackPlan } });
                }
                rosterCloseback = { mode: 'inline', command: closebackPlan.writerBoundary.rosterClosebackCommand, result: await runTasksRosterUpdate(['--cwd', planningRosterPaths.repoRoot,
                        '--index', planningRosterPaths.indexPath, '--from', planningRosterPaths.fromPath]) };
            }
            else if (closebackPlan.writerBoundary.rosterClosebackCommand && closebackPlan.writerBoundary.rosterSyncPolicy === 'follow-up-command') {
                if (!planningRosterPaths?.repoRoot || !planningRosterPaths.indexPath || !planningRosterPaths.fromPath) {
                    ...
                    rosterCloseback = { mode: 'follow-up-command', command: closebackPlan.writerBoundary.rosterClosebackCommand };
                }
                else {
                    const command = buildRosterSyncCommand({ indexPath: planningRosterPaths.indexPath, fromPath: planningRosterPaths.fromPath });
                    rosterCloseback = await runRosterSyncFollowUp({ command, cwd: planningRosterPaths.repoRoot, indexPath: planningRosterPaths.indexPath, fromPath: planningRosterPaths.fromPath, messages: closeMessages });
                }
            }
            const planningIndexAdvisory = buildTaskflowPlanningIndexAdvisory({ taskId,
                planningRosterPaths, rosterCloseback, rosterCommand: closebackPlan.writerBoundary.rosterClosebackCommand ?? null });
            if (planningIndexAdvisory) {
                closeMessages.push(message(planningIndexAdvisory.status === 'follow-up-required' ? 'warn' : 'info', planningIndexAdvisory.status === 'follow-up-required' ? 'ATM_TASKFLOW_CLOSE_PLANNING_INDEX_FOLLOWUP_REQUIRED' : 'ATM_TASKFLOW_CLOSE_PLANNING_INDEX_SYNCED', planningIndexAdvisory.status === 'follow-up-required' ? 'taskflow close succeeded; planning roster/index sync still needs an explicit follow-up command.' : 'taskflow close synced the planning roster/index and recorded the closeback fields.', planningIndexAdvisory));
            }
            promoteTaskRuntimeHandoffs(cwd, taskId, 'completed');
            const commitBundleInput = buildTaskflowCommitBundle({ cwd, taskId, actorId, commitMode,
                planningMirrorPath: closebackPlan.writerBoundary.planningMirrorPath, rosterIndexPath: closebackPlan.writerBoundary.rosterSyncPolicy === 'inline' ? closebackPlan.writerBoundary.rosterIndexPath : null, extraPlanningStageFiles: planningCardCloseback?.transitionPath ? [planningCardCloseback.transitionPath] : [], planningStageContent: planningCardCloseback?.stagedContent && planningCardCloseback.relativePath ? { [planningCardCloseback.relativePath]: planningCardCloseback.stagedContent } : undefined, backendResult: backendResult,
                historicalDeliveryRefs: effectiveHistoricalDeliveryRefs, historicalBatchRef, planningAuthorityDeliveryOk: planningAuthorityDeliveryGate.ok });
            const { bundle: governedCommitBundle, transaction: closeWriteTransaction } = backendResult.ok ? await executeCloseWriteCommitPhase({ cwd, taskId, actorId, snapshot: rollbackSnapshotWithPlanning, commit: () => finalizeTaskflowCommitBundle({ bundle: commitBundleInput, actorId,
                    taskId }) }) : { bundle: commitBundleInput, transaction: { schemaId: 'atm.closeWriteTransaction.v1', taskId, phase: 'pending', ok: false, failureStep: 'backend-close', failureCode: 'ATM_TASKFLOW_CLOSE_WRITE_FAILED', failureReason: 'backend close did not complete', rolledBackArtifacts: [], recoveryCommand: diagnosis.nextCommand, backendCloseApplied: false, commitBundleApplied: false } };
            const writeOk = backendResult.ok && closeWriteTransaction.ok && !governedCommitBundle.failClosed;
            const brokerRuntimeCleanup = writeOk ? clearBrokerRuntimeStateForTask({ cwd, taskId }).runtimeCleanup ?? null : null;
            ...
            closeMessages.push(message(writeOk ? 'info' : 'error', writeOk ? 'ATM_TASKFLOW_CLOSE_WRITE_ORCHESTRATED' : closeWriteTransaction.phase === 'rolled_back' ? 'ATM_TASKFLOW_CLOSE_WRITE_ROLLED_BACK' : 'ATM_TASKFLOW_CLOSE_WRITE_FAILED', writeOk
                ? `taskflow close orchestrated ${closebackPlan.backendSurface} for ${taskId}.` : closeWriteTransaction.phase === 'rolled_back' ? `taskflow close --write rolled back ${taskId} after a commit-bundle failure; ledger close state was restored.` : `taskflow close write failed for ${taskId}.`, { closeMode: closebackPlan.closeMode, backendSurface: closebackPlan.backendSurface }));
            const releasedCloseWindowLock = releaseCloseWindowStagedIndexLock({ cwd, taskId, actorId, outcome: writeOk ? 'committed' : closeWriteTransaction.phase === 'rolled_back' ? 'rolled_back' : 'aborted' });
            ...
            return { ...makeResult({ ok: writeOk,
                    command: 'taskflow close', cwd, mode: 'write', messages: closeMessages, evidence: { closeMode: closebackPlan.closeMode, writeSupport, commitMode, runnerGateDecision: runnerGateEvidence.runnerGateDecision, runnerGateIntersectingFiles: runnerGateEvidence.runnerGateIntersectingFiles, runnerGateScopeClass: runnerGateEvidence.scopeClass, delegationContract, diagnostics, closebackPlan, backendResult, preCloseDeliveryCommit, planningCardCloseback, rosterCloseback, governedCommitBundle, closeWriteTransaction, closeWindowLock, releasedCloseWindowLock, brokerRuntimeCleanup, deferredGovernanceDirty,
                        residueDiagnosis: enrichedDiagnosis, residueAdvisory, planningIndexAdvisory, closebackPathResolution, ...(autoEvidenceExecution ? { autoEvidenceExecution, autoEvidencePlan: autoEvidenceExecution.plan } : {}), ...(profileData ? { profile: profileData } : {}) } }), schemaId: 'atm.taskflowCloseResult.v1', writeEnabled: true };
        }
        finally {
            if (closeWindowLock?.ok && !closeWindowLockReleased) {
                releaseCloseWindowStagedIndexLock({ cwd, taskId, actorId, outcome: 'aborted' });
            }
...
export async function runTaskflow(argv = []) {
    ...
    const openOutputRoot = resolveTaskflowOpenOutputRoot({ profilePath, profile: profileData, cwd });
    const prerequisiteInput = { profile: profileData, taskIdSupplied: taskId !== null, outputPathSupplied: outputPath !== null, writeRequested };
    const delegationContract = buildDelegationContract(profileData);
    ...
    if (profileData && canResolveHostOpenerPolicy({ cwd: openOutputRoot, profile: profileData, delegationContract, taskId, outputPath, title })) {
        try {
            hostPolicyDecision = resolveHostOpenerPolicyDecision({ cwd: openOutputRoot, profile: profileData, delegationContract, taskId, outputPath, title });
    ...
    if (writeRequested && writeSupport.allowed) {
        ...
        try {
            const runtimeImportResult = await withTaskflowOperatorLane(() => runTasks(['import', '--cwd', cwd, '--from', targetAbsolute, '--write']));
            runtimeImport = { command: buildTasksImportCommand({ fromPath: targetAbsolute }), result: runtimeImportResult };
        }
        catch (error) {
        ...
        if (delegationContract.policy.rosterSyncPolicy === 'inline' && effectiveRosterIndex) {
            ...
            rosterSync = { mode: 'inline',
                command: buildRosterSyncCommand({ indexPath: effectiveRosterIndex, fromPath: resolved.outputPath }), result: rosterResult };
        }
    ...
    const result = makeResult({
        ok: true, command: 'taskflow open', cwd, mode: 'dry-run', messages: [message(hostPolicyDecision?.familyDrift ? 'warn' : openerMode === 'delegated-governed' ? 'info' : 'warn', hostPolicyDecision?.familyDrift ? 'ATM_TASK_ID_FAMILY_DRIFT' : openerMode === 'delegated-governed' ? 'ATM_TASKFLOW_OPEN_ORCHESTRATION_READY' : 'ATM_TASKFLOW_OPEN_TEMPLATE_ONLY_FALLBACK', hostPolicyDecision?.familyDrift
                ? hostPolicyDecision.familyDrift.message : openerMode === 'delegated-governed' ? 'taskflow open dry-run orchestration plan is ready for delegated governed entry.' : 'taskflow open is in template-only-fallback mode. --write will fail closed; see writeReadinessHint for the exact missing prerequisites. tasks new (low-level generator surface) remains the explicit non-governed escape hatch.', { cwd, openerMode, writeReadinessHintStatus: writeReadinessHint.status })], evidence: { openerMode, writeSupport, writeReadinessHint, delegationContract, diagnostics, orchestrationPlan, hostPolicyDecision, fallbackBehavior: delegationContract.policy.fallbackBehavior, ...(profileData ? { profile: profileData } : {}) }
    });
    return { ...result, schemaId: 'atm.taskflowOpenResult.v1', writeEnabled: false,
        writeReadinessHint };
}
export function buildTaskflowCloseResidueAdvisory(diagnosis) {
    ...
    const advisoryBuckets = new Set(['planning-mirror-only', 'stale-import', 'ledger-ahead-of-planning-mirror'
    ]);
    if (!bucket || !advisoryBuckets.has(bucket))
        return null;
    return { schemaId: 'atm.taskflowCloseResidueAdvisory.v1', bucket, truth: typeof diagnosis.truth === 'string' ? diagnosis.truth : null, residue: typeof diagnosis.residue === 'string' ? diagnosis.residue : null, reason: typeof diagnosis.reason === 'string' ? diagnosis.reason : null, recoveryCommand: typeof diagnosis.nextCommand === 'string'
            ? diagnosis.nextCommand : 'node atm.mjs tasks status --task <id> --residue --json', severity: 'warning', closeSucceeded: true };
}
export function buildTaskflowPlanningIndexAdvisory(input) {
    if (!input.rosterCommand && !input.planningRosterPaths?.indexPath)
        return null;
    const result = input.rosterCloseback?.result;
    const evidence = result && typeof result === 'object' && !Array.isArray(result) ? result.evidence : null;
    const evidenceRecord = evidence && typeof evidence === 'object' && !Array.isArray(evidence) ? evidence : {};
    const diff = evidenceRecord.diff && typeof evidenceRecord.diff === 'object' && !Array.isArray(evidenceRecord.diff) ? evidenceRecord.diff : null;
    const ok = result && typeof result === 'object' && !Array.isArray(result)
        ? result.ok === true : false;
    const unchanged = evidenceRecord.unchanged === true || (diff && typeof diff.before === 'string' && typeof diff.after === 'string' && diff.before === diff.after);
    return { schemaId: 'atm.taskflowPlanningIndexAdvisory.v1', taskId: input.taskId, status: ok ? (unchanged ? 'already-current' : 'updated') : 'follow-up-required',
        repoRoot: input.planningRosterPaths?.repoRoot ?? null, indexPath: input.planningRosterPaths?.indexPath ?? null, planningCardPath: input.planningRosterPaths?.fromPath ?? null, frontmatterFields: ['status', 'completed_by_agent', 'closedByActor', 'closedByCommand', 'lastTransitionId', 'delivery_commit'], requiredCommand: input.rosterCommand
            ?? (input.planningRosterPaths?.indexPath && input.planningRosterPaths?.fromPath ? buildRosterSyncCommand({ indexPath: input.planningRosterPaths.indexPath, fromPath: input.planningRosterPaths.fromPath }) : 'node atm.mjs tasks roster update --index <tasks/README.md> --from <task-card.md> --json'), diff: diff ? { before: diff.before ?? null, after: diff.after ?? null } : null };
}
