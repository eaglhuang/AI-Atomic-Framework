...
function parseNumericSuffixFormat(format) {
    const match = /^(.*?)(N+)$/.exec(format.trim());
    if (!match) {
        throw new CliError('ATM_TASKFLOW_HOST_POLICY_AMBIGUOUS_NUMBERING', `Host opener numbering format "${format}" is ambiguous; expected a trailing N-run such as PREFIX-NNNN.`, { exitCode: 1 });
    }
    return { prefix: match[1], width: match[2].length };
}
function formatTaskIdFromNumber(format, numericSuffix) {
...
function collectExistingNumericSuffixes(cwd, directory, format) {
    const parsed = parseNumericSuffixFormat(format);
    ...
    const fileNamePattern = new RegExp(`^${escapeRegex(parsed.prefix)}(\\d{${parsed.width}})(?:\\.task\\.md)?$`, 'i');
    const idPattern = new RegExp(`^${escapeRegex(parsed.prefix)}(\\d{${parsed.width}})$`, 'i');
...
function allocateTaskIdFromPolicy(cwd, profile, policy) {
    if (policy.allocateTaskId.mode !== 'host-opener') {
        throw new CliError('ATM_TASKFLOW_HOST_POLICY_NUMBERING_UNSUPPORTED', 'Host opener numbering is not configured; supply --task-id explicitly.', { exitCode: 1 });
    }
