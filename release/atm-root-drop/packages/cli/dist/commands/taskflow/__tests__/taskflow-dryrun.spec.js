...
assert.ok(res2.evidence.diagnostics.messages.some((entry) => entry.includes('describe-only')));
const governedProfilePath = path.join(rootDir, 'fixtures/taskflow-profile/governed-invocable.profile.json');
const res3 = await runTaskflow(['open', '--dry-run', '--profile', governedProfilePath]);
...
async function makeBrokerCloseFixture(label) {
...
async function makeDualRepoCloseFixture(label, options = {}) {
...
assert.ok(brokerConfirmedDryRun.evidence.writeReadinessHint.blockers.some((entry) => entry.code === 'ATM_TASKFLOW_CLOSE_BROKER_CONFIRMED_CONFLICT'), 'confirmed broker conflict must block taskflow close --write readiness');
...
assert.ok(brokerInsufficientDryRun.evidence.writeReadinessHint.blockers.every((entry) => entry.code !== 'ATM_TASKFLOW_CLOSE_BROKER_CONFIRMED_CONFLICT'), 'insufficient mutation intent must remain advisory');
...
const externalStoredPlanPath = path.join(externalStoredPathFixture.planningRepo, 'docs', 'ai_atomic_framework', 'atm-agent-first-operability', 'tasks', `${externalStoredPathFixture.taskId}.task.md`);
...
assert.ok(externalStoredPathDryRun.evidence.governedCommitBundle.planningRepo.stageFiles.includes(`docs/ai_atomic_framework/atm-agent-first-operability/tasks/${externalStoredPathFixture.taskId}.task.md`));
