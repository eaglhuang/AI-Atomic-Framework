...
await assert.rejects(() => finalizeTaskflowCommitBundle({
    bundle: blockedMutexBundle,
    actorId: 'validator',
    taskId: blockedMutexTaskId
}), /close transaction mutex is already held/, 'only one closer may own the auto-commit close transaction window');
