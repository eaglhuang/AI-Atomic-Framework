...
writeJson(path.join(targetRepo, '.atm/history/tasks/TASK-BUNDLE-0001.json'), { workItemId: 'TASK-BUNDLE-0001', deliverables: ['src/app.ts'], scopePaths: ['src/app.ts'], source: { planPath } });
writeText(path.join(targetRepo, 'src/app.ts'), 'export const value = 1;\n');
const bundle = buildTaskflowCommitBundle({ cwd: targetRepo, taskId: 'TASK-BUNDLE-0001',
    actorId: 'validator', commitMode: 'dry-run', planningMirrorPath: planPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false });
...
writeJson(path.join(targetRepo, '.atm/history/evidence/TASK-BUNDLE-0001.closure-packet.json'), { schemaId: 'atm.closurePacket.v1', taskId: 'TASK-BUNDLE-0001' });
writeJson(path.join(targetRepo, '.atm/history/evidence/TASK-BUNDLE-0001.json'), { schemaId: 'atm.taskEvidence.v1', taskId: 'TASK-BUNDLE-0001' });
writeJson(path.join(targetRepo, '.atm/history/task-events/TASK-BUNDLE-0001/import.json'), { schemaId: 'atm.taskTransition.v1', taskId: 'TASK-BUNDLE-0001', transitionId: 'import-1', action: 'import',
    command: 'node atm.mjs tasks import --from docs/tasks/TASK-BUNDLE-0001.task.md --write --json' });
writeJson(path.join(targetRepo, '.atm/history/task-events/TASK-BUNDLE-0001/claim.json'), { schemaId: 'atm.taskTransition.v1', taskId: 'TASK-BUNDLE-0001', transitionId: 'claim-1', action: 'claim', command: 'node atm.mjs next --claim --task TASK-BUNDLE-0001 --actor validator --json' });
const backendBundle = buildTaskflowCommitBundle({ cwd: targetRepo, taskId: 'TASK-BUNDLE-0001', actorId: 'validator', commitMode: 'dry-run', planningMirrorPath: planPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false, backendResult: { evidence: { taskPath: '.atm/history/tasks/TASK-BUNDLE-0001.json', closurePacketPath: '.atm/history/evidence/TASK-BUNDLE-0001.closure-packet.json',
...
writeJson(path.join(autoTargetRepo, '.atm/history/tasks/TASK-BUNDLE-0002.json'), { workItemId: autoCommitTaskId, title: `${autoCommitTaskId} fixture`, status: 'running', claim: { actorId: 'validator',
        leaseId: 'lease-bundle-0002', state: 'active' }, deliverables: ['src/app.ts'], scopePaths: ['src/app.ts'], source: { planPath: autoPlanPath } });
writeJson(path.join(autoTargetRepo, '.atm/history/evidence/TASK-BUNDLE-0002.json'), { taskId: autoCommitTaskId, schemaId: 'atm.taskEvidence.v1' });
writeJson(path.join(autoTargetRepo, '.atm/history/evidence/TASK-BUNDLE-0002.closure-packet.json'), { taskId: autoCommitTaskId,
    schemaId: 'atm.closurePacket.v1' });
const autoBundleManifestPath = `.atm/history/evidence/${autoCommitTaskId}.bundle-manifest.json`;
writeJson(path.join(autoTargetRepo, autoBundleManifestPath), { taskId: autoCommitTaskId, schemaId: 'atm.evidenceBundleManifest.v1', summary: 'stale pre-standard manifest' });
execFileSync('git', ['add', autoBundleManifestPath], { cwd: autoTargetRepo, stdio: 'ignore' });
writeJson(path.join(autoTargetRepo, autoBundleManifestPath), { taskId: autoCommitTaskId, schemaId: 'atm.evidenceBundleManifest.v1', summary: 'fresh standard manifest' });
...
writeJson(path.join(autoTargetRepo, '.atm', 'runtime', 'sessions', 'session-bundle-0002.json'), { schemaId: 'atm.actorWorkSession.v1', specVersion: '0.1.0', sessionId: 'session-bundle-0002', actorId: 'validator', taskId: autoCommitTaskId, claimLeaseId: 'lease-bundle-0002', status: 'active', createdAt: '2026-06-20T00:00:00.000Z', updatedAt: '2026-06-20T00:00:00.000Z', heartbeatAt: '2026-06-20T00:00:00.000Z',
    taskPath: `.atm/history/tasks/${autoCommitTaskId}.json`, sourcePrompt: null, batchId: null, guidanceSessionId: null, editor: 'codex', gitName: 'ATM Validator', gitEmail: 'validator@example.invalid' });
...
const autoBundle = buildTaskflowCommitBundle({ cwd: autoTargetRepo, taskId: autoCommitTaskId, actorId: 'validator', commitMode: 'auto-commit', planningMirrorPath: autoPlanPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false });
const autoFinal = await finalizeTaskflowCommitBundle({ bundle: autoBundle,
    actorId: 'validator', taskId: autoCommitTaskId });
...
assert.equal(existsSync(closeTransactionMutexPath(autoTargetRepo, autoCommitTaskId)), false, 'auto-commit close transaction mutex must release after commit');
...
writeJson(path.join(blockedMutexTargetRepo, '.atm/history/tasks/TASK-BUNDLE-MUTEX.json'), { workItemId: blockedMutexTaskId,
    title: `${blockedMutexTaskId} fixture`, status: 'running', claim: { actorId: 'validator', leaseId: 'lease-bundle-mutex', state: 'active' }, deliverables: ['src/app.ts'], scopePaths: ['src/app.ts'], source: { planPath: blockedMutexPlanPath } });
writeJson(path.join(blockedMutexTargetRepo, '.atm/history/evidence/TASK-BUNDLE-MUTEX.json'), { taskId: blockedMutexTaskId, schemaId: 'atm.taskEvidence.v1' });
writeJson(path.join(blockedMutexTargetRepo, '.atm/history/evidence/TASK-BUNDLE-MUTEX.closure-packet.json'), { taskId: blockedMutexTaskId, schemaId: 'atm.closurePacket.v1' });
...
writeJson(blockedMutexPath, {
    schemaId: 'atm.closeTransactionMutexLease.v1', taskId: blockedMutexTaskId, actorId: 'other-captain', leaseId: 'held-by-other', acquiredAt: '2026-07-15T00:00:00.000Z', expiresAt: '2999-01-01T00:00:00.000Z', lockPath: blockedMutexPath
});
const blockedMutexBundle = buildTaskflowCommitBundle({ cwd: blockedMutexTargetRepo, taskId: blockedMutexTaskId, actorId: 'validator', commitMode: 'auto-commit',
    planningMirrorPath: blockedMutexPlanPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false });
await assert.rejects(() => finalizeTaskflowCommitBundle({ bundle: blockedMutexBundle, actorId: 'validator', taskId: blockedMutexTaskId }), /close transaction mutex is already held/, 'only one closer may own the auto-commit close transaction window');
rmSync(blockedMutexPath, { force: true });
writeJson(blockedMutexPath, { schemaId: 'atm.closeTransactionMutexLease.v1', taskId: blockedMutexTaskId, actorId: 'crashed-captain', leaseId: 'close-TASK-BUNDLE-MUTEX-1784080000000-99999999', ownerPid: 99999999, acquiredAt: '2026-07-15T00:00:00.000Z', expiresAt: '2999-01-01T00:00:00.000Z', lockPath: blockedMutexPath });
const recoveredMutexFinal = await finalizeTaskflowCommitBundle({ bundle: blockedMutexBundle,
    actorId: 'validator', taskId: blockedMutexTaskId });
assert.equal(recoveredMutexFinal.targetRepo.status, 'committed');
assert.equal(existsSync(blockedMutexPath), false, 'dead-owner close transaction mutex must be cleaned and released after the recovered commit');
...
writeJson(path.join(sameRepo, '.atm/history/tasks/TASK-BUNDLE-0003.json'), { workItemId: sameRepoTaskId, title: `${sameRepoTaskId} fixture`, status: 'running', claim: { actorId: 'validator', leaseId: 'lease-bundle-0003',
        state: 'active' }, deliverables: ['src/app.ts'], scopePaths: ['src/app.ts'], source: { planPath: sameRepoPlanPath } });
writeJson(path.join(sameRepo, '.atm/history/evidence/TASK-BUNDLE-0003.json'), { taskId: sameRepoTaskId, schemaId: 'atm.taskEvidence.v1' });
writeJson(path.join(sameRepo, '.atm/history/evidence/TASK-BUNDLE-0003.closure-packet.json'), { taskId: sameRepoTaskId, schemaId: 'atm.closurePacket.v1' });
writeJson(path.join(sameRepo, '.atm/history/task-events/TASK-BUNDLE-0003/close.json'), { taskId: sameRepoTaskId, action: 'close' });
...
const sameRepoBundle = buildTaskflowCommitBundle({ cwd: sameRepo, taskId: sameRepoTaskId, actorId: 'validator',
    commitMode: 'auto-commit', planningMirrorPath: sameRepoPlanPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false, backendResult: { evidence: { taskPath: `.atm/history/tasks/${sameRepoTaskId}.json`, closurePacketPath: `.atm/history/evidence/${sameRepoTaskId}.closure-packet.json`, transitionPath: `.atm/history/task-events/${sameRepoTaskId}/close.json` } } });
const sameRepoFinal = await finalizeTaskflowCommitBundle({ bundle: sameRepoBundle, actorId: 'validator', taskId: sameRepoTaskId });
...
writeJson(path.join(amendedTargetRepo, '.atm/history/tasks/TASK-BUNDLE-0004.json'), { workItemId: amendedTaskId, title: `${amendedTaskId} fixture`, status: 'running', claim: { actorId: 'validator',
        leaseId: 'lease-bundle-0004', state: 'active', files: ['packages/**'] }, taskDirectionLock: { allowedFiles: ['packages/**'] }, deliverables: ['docs/guide.md'], scopePaths: ['docs/**', 'packages/**'], targetAllowedFiles: ['docs/**'], source: { planPath: amendedPlanPath } });
writeText(path.join(amendedTargetRepo, 'packages/cli/src/runtime-scope.ts'), 'export const runtimeScoped = true;\n');
const amendedBundle = buildTaskflowCommitBundle({ cwd: amendedTargetRepo, taskId: amendedTaskId, actorId: 'validator', commitMode: 'dry-run', planningMirrorPath: amendedPlanPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false });
...
writeJson(path.join(historicalScratchTargetRepo, '.atm/history/tasks/TASK-BUNDLE-0004B.json'), { workItemId: historicalScratchTaskId, title: `${historicalScratchTaskId} fixture`, status: 'running', deliverables: ['packages/**'], scopePaths: ['packages/**'], targetAllowedFiles: ['packages/**'], source: { planPath: historicalScratchPlanPath } });
...
const historicalScratchBundle = buildTaskflowCommitBundle({ cwd: historicalScratchTargetRepo, taskId: historicalScratchTaskId, actorId: 'validator',
    commitMode: 'dry-run', planningMirrorPath: historicalScratchPlanPath, rosterIndexPath: null, historicalDeliveryRefs: [historicalScratchDeliverySha], planningAuthorityDeliveryOk: false });
...
writeJson(path.join(externalTargetRepo, '.atm/config.json'), { schemaVersion: 'atm.config.v0.1', taskLedger: { planningRoots: ['../planning-external/docs/ai_atomic_framework'] } });
...
writeJson(path.join(externalTargetRepo, '.atm/history/tasks', `${externalTaskId}.json`), { workItemId: externalTaskId, title: `${externalTaskId} fixture`, status: 'running', claim: { actorId: 'validator', leaseId: 'lease-bundle-0005', state: 'active', files: [
            'atm-agent-first-operability/tasks/TASK-BUNDLE-0005.task.md', 'src/runtime.txt'
        ] }, taskDirectionLock: { allowedFiles: ['src/runtime.txt'], planningReadOnlyPaths: ['atm-agent-first-operability/tasks/TASK-BUNDLE-0005.task.md'], planningMirrorPaths: ['atm-agent-first-operability/tasks/TASK-BUNDLE-0005.task.md'] }, deliverables: ['src/runtime.txt'], scopePaths: ['src/runtime.txt'],
...
const externalBundle = buildTaskflowCommitBundle({ cwd: externalTargetRepo, taskId: externalTaskId, actorId: 'validator', commitMode: 'dry-run', planningMirrorPath: 'atm-agent-first-operability/tasks/TASK-BUNDLE-0005.task.md',
    rosterIndexPath: null, planningAuthorityDeliveryOk: false });
...
writeJson(path.join(proseTargetRepo, '.atm/history/tasks/TASK-BUNDLE-0006.json'), { workItemId: proseTaskId, title: `${proseTaskId} fixture`, status: 'running', claim: { actorId: 'validator', leaseId: 'lease-bundle-0006', state: 'active', files: ['src/runtime.txt'] }, taskDirectionLock: { allowedFiles: ['src/runtime.txt'] }, deliverables: [
        'Documentation explaining how adopter repositories extend atom health checks without forking ATM core.', 'src/runtime.txt'
    ], scopePaths: ['src/runtime.txt'], source: { planPath: prosePlanPath } });
writeText(path.join(proseTargetRepo, 'src/runtime.txt'), 'prose deliverable fallback\n');
const proseBundle = buildTaskflowCommitBundle({ cwd: proseTargetRepo, taskId: proseTaskId, actorId: 'validator',
    commitMode: 'dry-run', planningMirrorPath: prosePlanPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false });
...
try {
    ...
    writeJson(path.join(identityTargetRepo, '.atm/runtime/identity/actors', `${identityActorId}.json`), { schemaId: 'atm.identityDefault.v1', specVersion: '0.1.0', actorId: identityActorId,
        gitName: 'OPT-09 Actor', gitEmail: 'opt09-actor@example.invalid', editor: null, provider: null, activeSessionId: null, updatedAt: '2026-07-08T00:00:00.000Z' });
    ...
    writeJson(path.join(identityTargetRepo, '.atm/history/tasks/TASK-BUNDLE-0007.json'), { workItemId: identityTaskId, title: `${identityTaskId} fixture`, status: 'running', claim: { actorId: identityActorId, leaseId: 'lease-bundle-0007', state: 'active' }, deliverables: ['src/app.ts'], scopePaths: ['src/app.ts'], source: { planPath: identityPlanPath } });
    writeJson(path.join(identityTargetRepo, '.atm/history/evidence/TASK-BUNDLE-0007.json'), { taskId: identityTaskId, schemaId: 'atm.taskEvidence.v1' });
    writeJson(path.join(identityTargetRepo, '.atm/history/evidence/TASK-BUNDLE-0007.closure-packet.json'), { taskId: identityTaskId, schemaId: 'atm.closurePacket.v1' });
    writeText(path.join(identityTargetRepo, 'src/app.ts'), 'export const value = 7;\n');
    const identityBundle = buildTaskflowCommitBundle({ cwd: identityTargetRepo, taskId: identityTaskId, actorId: identityActorId, commitMode: 'auto-commit', planningMirrorPath: identityPlanPath, rosterIndexPath: null, planningAuthorityDeliveryOk: false });
    const identityFinal = await finalizeTaskflowCommitBundle({ bundle: identityBundle, actorId: identityActorId, taskId: identityTaskId });
...
{
    assert.equal(isDeferrableGovernanceDirtyFile('.atm/history/evidence/git-head.jsonl', 'TASK-AAO-0194'), true, 'git-head evidence remains deferrable');
    assert.equal(isDeferrableGovernanceDirtyFile('.atm/history/evidence/TASK-AAO-0194.bundle-manifest.json', 'TASK-AAO-0194'), true, 'current-task bundle-manifest is deferrable');
    assert.equal(isDeferrableGovernanceDirtyFile('.atm/history/evidence/TASK-AAO-0192.bundle-manifest.json', 'TASK-AAO-0194'), false, 'foreign-task bundle-manifest must not be deferred/restored during another close');
    assert.equal(isDeferrableGovernanceDirtyFile('.atm/history/evidence/TASK-AAO-0192.bundle-manifest.json', null), false, 'without task binding, only git-head is safe');
    ...
    assert.deepEqual(deferred.files.map((entry) => entry.file), [currentManifest], 'defer must keep foreign-task dirty manifests untouched');
    assert.equal(JSON.parse(readFileSync(path.join(dirtyRepo, foreignManifest), 'utf8')).dirty, 'foreign', 'foreign dirty content must remain after current-task defer');
    assert.equal(JSON.parse(readFileSync(path.join(dirtyRepo, currentManifest), 'utf8')).dirty, false, 'current-task manifest should be restored to HEAD during defer');
    const missingSnapshotReport = restoreDeferredGovernanceDirtyFiles(dirtyRepo, { schemaId: 'atm.deferredGovernanceDirty.v1', requested: true, restored: false, files: [{ file: currentManifest,
                snapshotPath: '.atm/runtime/snapshots/missing-close-window-snapshot.json', originalSha256: 'deadbeef', restoredAt: null }] });
    assert.equal(missingSnapshotReport.restored, true);
    assert.deepEqual(missingSnapshotReport.skippedMissingSnapshots, ['.atm/runtime/snapshots/missing-close-window-snapshot.json']);
