 ...
 */
async function removeFixtureWithRetry(target) {
    let lastError = null;
...
async function makeKillAfterTargetCommitFixture() {
    ...
    const laneSessionId = claim.evidence.laneSession?.laneSessionId;
    if (typeof laneSessionId !== 'string')
        throw new Error('claim must mint an owner lane capability');
...
const stagedAudit = stagedReport.statusReport.entries.find((entry) => entry.path === `.atm/history/task-events/${abandonedTaskId}/audit.json`);
...
assert.equal(existsSync(path.join(repo, `.atm/history/task-events/${abandonedTaskId}/audit.json`)), true, 'staged audit evidence must remain on disk');
...
try {
    ...
    try {
        ...
        assert.equal(error.code, 'ATM_TASK_CLOSE_TRANSACTION_FAILED');
    }
    finally {
    ...
    const taskAfterFailure = JSON.parse(readFileSync(taskPath, 'utf8'));
...
try {
    ...
    child.stdout?.on('data', (chunk) => { driverStdout += chunk; });
    child.stderr?.on('data', (chunk) => { driverStderr += chunk; });
    try {
    ...
    assert.notEqual(targetCommitAfterCrash, git(killFixture.targetRepo, ['rev-parse', 'HEAD~1']), 'target commit must exist before the injected kill');
    ...
    assert.match(git(killFixture.planningRepo, ['show', `HEAD:${planningCardRelativePath}`]), /status:\s*done/, 'taskflow close can converge the dirty planning closeback without changing the target deliverable');
}
finally {
