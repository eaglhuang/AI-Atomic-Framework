...
import { type CrossAuthorityClosebackPlan } from './cross-authority-closeback.ts';
...
export interface TaskflowCloseRecoveryLane {
    disposition: 'execute-now' | 'queue' | 'recover' | 'wait' | 'human-required';
    owner: 'runner-sync' | 'pre-push' | 'evidence' | 'close' | 'planning' | 'target' | 'human' | 'global';
    command: string | null;
    reason: string;
...
export interface TaskflowClosebackPlan {
    ...
    authorityReconciliation?: CrossAuthorityClosebackPlan;
    legalRecoveryLanes: TaskflowCloseRecoveryLane[];
    nextLegalRecoveryLane: TaskflowCloseRecoveryLane;
    forbiddenActions: string[];
}
export declare function buildAuthorityReconciliation(input: {
    cwd: string;
    taskId: string;
    actorId: string;
