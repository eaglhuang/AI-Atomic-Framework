...
export function isTargetTaskPlanPath(planRelativePath) {
...
export function isPlanningFamilyTaskId(taskId) {
    return PLANNING_FAMILY_PATTERN.test(taskId.trim());
}
function listTaskCardCandidates(tasksDirectory, taskId) {
...
function walkTaskDirectories(root, out, depth = 0) {
    ...
    for (const entry of entries) {
        if (entry === 'node_modules' || entry === '.git' || entry === '.atm')
            continue;
...
export function inspectPlanningRootAuthorship(input) {
    ...
    const applies = Boolean(isFrameworkRepo
    ...
    const findings = familyTaskIds.map((taskId) => {
