...
import { auditTasks, isRunnerSyncRequired, runnerStaleWarningMessage } from '../../framework-development.js';
...
async function runTasksCreate(argv) { const options = parseCreateOptions(argv); assertLocalTaskLedgerEnabled(options.cwd, 'create'); const resolvedActor = resolveActorId(options.actorId ?? undefined, options.cwd); if (!resolvedActor) {
...
async function runTasksMirror(argv) { const options = parseMirrorOptions(argv); assertLocalTaskLedgerEnabled(options.cwd, 'mirror'); const resolvedActor = resolveActorId(options.actorId ?? undefined, options.cwd); if (!resolvedActor) {
...
async function runTasksMetadataRepairDeliverables(argv) { const options = parseMetadataRepairDeliverablesOptions(argv); const resolvedActor = resolveActorId(options.actorId ?? undefined, options.cwd); if (!resolvedActor) {
...
async function runTasksScopeAdd(argv) { const options = parseScopeAddOptions(argv); const resolvedActor = resolveActorId(options.actorId ?? undefined, options.cwd); if (!resolvedActor) {
