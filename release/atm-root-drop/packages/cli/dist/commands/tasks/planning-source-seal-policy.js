...
export function isBenignSealUpgrade(input) {
    ...
    if (driftKinds.length !== 1 || driftKinds[0] !== 'commit')
        return false;
    if (sealed.planningCommitSha !== null)
        return false;
    if (typeof current.planningCommitSha !== 'string' || current.planningCommitSha.trim().length === 0)
        return false;
    ...
    if (current.repoIdentity !== sealed.repoIdentity)
        return false;
    if (current.taskCardPath !== sealed.taskCardPath)
        return false;
    return true;
}
/**
...
export function classifyPlanningSourceSeal(input) {
    ...
    const status = governedAmendment ? 'governed-amendment' : 'drift';
    return {
        schemaId: 'atm.planningSourceSealClassification.v1',
