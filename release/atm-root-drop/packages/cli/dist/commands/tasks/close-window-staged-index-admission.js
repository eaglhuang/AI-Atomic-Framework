export function evaluateCloseWindowStagedIndexAdmission(input) {
    ...
    if (input.deferForeignStaged && provenResidueFiles.length > 0) {
        return { ok: false, blockedCode: 'ATM_CLOSE_WINDOW_UNRECONCILED_RESIDUE', blockedSummary: `Close window cannot defer ${provenResidueFiles.length} staged path(s) that are this repository's own unreconciled commits; draining the recorded reconciliation debt is required${input.residueDrainCommand ? ` (${input.residueDrainCommand})` : ''} before close.` };
    }
    if (input.unexpectedStagedFiles.length > 0 && !input.deferForeignStaged) {
        ...
        if (foreignStagedFiles.length === 0 && residue.size > 0) {
            return { ok: false, blockedCode: 'ATM_CLOSE_WINDOW_UNRECONCILED_RESIDUE', blockedSummary: `Close window blocked by ${residue.size} staged path(s) that are this repository's own unreconciled commits, not foreign work; drain the recorded reconciliation debt${input.residueDrainCommand ? ` (${input.residueDrainCommand})` : ''} rather than deferring it.` };
        }
