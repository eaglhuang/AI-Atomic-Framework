...
function buildMetadataRepairNoClaimMessage(precondition) {
    return [
...
async function runTasksMetadataRepairDeliverables(argv) {
    ...
    if (rawDeliverableViolations.length > 0 || candidateDeliverables.length !== rawDeliverables.length) {
        ...
        const violations = rawDeliverableViolations.length > 0
            ...
            : droppedEntries.map((entry) => ({ entry, reason: 'not-path-shaped', severity: 'error' }));
    ...
    const previousDeliverables = sanitizeTaskDirectionAllowedFiles(Array.isArray(taskDocument.deliverables) ? taskDocument.deliverables : []);
...
async function runTasksScopeRemove(argv) {
    const options = parseScopeAddOptions(argv.map((value) => value === '--remove' ? '--add' : value));
    const resolvedActor = resolveActorId(options.actorId ?? undefined, options.cwd);
    if (!resolvedActor)
        throw new CliError('ATM_ACTOR_ID_MISSING', 'tasks scope remove requires --actor or ATM_ACTOR_ID.', { exitCode: 2 });
    const actorId = resolvedActor.actorId;
    const precondition = inspectScopeAmendmentPreconditions(options.cwd, options.taskId, actorId);
    ...
    const deliverables = sanitizeTaskDirectionAllowedFiles(Array.isArray(taskDocument.deliverables) ? taskDocument.deliverables : []);
    ...
    const lockPath = path.join(options.cwd, '.atm', 'runtime', 'locks', `${options.taskId}.lock.json`);
    if (!existsSync(lockPath))
        throw new CliError('ATM_SCOPE_AMENDMENT_NO_ACTIVE_LOCK', buildScopeAmendmentNoClaimMessage(precondition), { exitCode: 1, details: precondition });
    const outerLock = JSON.parse(readFileSync(lockPath, 'utf8'));
    const embeddedLockRecord = outerLock.taskDirectionLock;
    if (!embeddedLockRecord || Array.isArray(embeddedLockRecord))
        throw new CliError('ATM_SCOPE_AMENDMENT_NO_ACTIVE_LOCK', `Lock file for ${options.taskId} does not contain an embedded taskDirectionLock.`, { exitCode: 1 });
    const existingAllowed = sanitizeTaskDirectionAllowedFiles(Array.isArray(embeddedLockRecord.allowedFiles) ? embeddedLockRecord.allowedFiles : []);
...
function parseSingleCard(input) {
    ...
    const ownerAtomOrMap = normalizeOptionalString(frontMatter.data.ownerAtomOrMap
        ?? frontMatter.data.owner_atom_or_map
        ?? atomizationImpactFrontMatter.ownerAtomOrMap
        ?? atomizationImpactFrontMatter.owner_atom_or_map);
    return {
...
function buildMechanicalSplitScopeDiagnostics(input) {
    ...
    if (fileLike.length > 1 || broadPatterns.length > 0)
        return [];
...
function reconcileMirrorOnlyTaskDocument(existingDocument, task) {
    const merged = {
        ...existingDocument,
        source: task.source,
        importedAt: task.importedAt,
    ...
    if (typeof existingDocument.status === 'string')
        merged.status = existingDocument.status;
    if ('claim' in existingDocument)
        merged.claim = existingDocument.claim;
    if ('closedAt' in existingDocument)
        merged.closedAt = existingDocument.closedAt;
    if ('closedByActor' in existingDocument)
        merged.closedByActor = existingDocument.closedByActor;
    if ('closurePacket' in existingDocument)
        merged.closurePacket = existingDocument.closurePacket;
    if ('closeReason' in existingDocument)
        merged.closeReason = existingDocument.closeReason;
    if ('lastTransitionId' in existingDocument)
        merged.lastTransitionId = existingDocument.lastTransitionId;
    return merged;
}
