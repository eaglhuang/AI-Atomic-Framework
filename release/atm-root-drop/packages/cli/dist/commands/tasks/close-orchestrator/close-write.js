...
function isAbandonedTaskOwnedPath(taskId, filePath) {
...
export function applyAbandonedResidueDisposition(input) {
    ...
    for (const rel of [
        ...
        if (existsSync(path.join(input.cwd, rel)))
            keptAuditTrail.push(rel);
    }
    ...
    const disposition = removed.length > 0 ? 'remove-evidence' : 'keep-diagnostic';
...
export async function executeCloseWrites(input) {
    ...
    const closeWriteResult = await executeTaskCloseTransaction({
        ...
        runWrites: () => {
            ...
            let abandonResidueDispositionPath = null;
