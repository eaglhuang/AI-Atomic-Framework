 ...
 * point of the module.
 */
const GOVERNANCE_FIELD_CONTRACTS = [
    {
 ...
 */
const GOVERNANCE_NAMESPACE_PATTERNS = [
    /^causal[A-Z_]/,
    /^causal_/,
    /^atomization[A-Z_]/,
    /^exam[A-Z_]/,
...
function isGovernanceNamespaceKey(key) {
...
function readAlias(record, keys) {
    if (!record)
        return undefined;
    for (const key of keys) {
        if (Object.prototype.hasOwnProperty.call(record, key) && isDeclared(record[key]))
            return record[key];
    }
    return undefined;
}
 ...
 */
function findMissingListEntries(declared, represented) {
    if (!Array.isArray(declared))
        return [];
    const projected = new Set((Array.isArray(represented) ? represented : [])
...
export function collectRawFrontmatterKeys(planText) {
    const match = /^---\s*\r?\n([\s\S]*?)\r?\n---\s*(?:\r?\n|$)/.exec(planText);
    if (!match)
        return [];
    const keys = [];
...
export function inspectTaskFrontmatterFidelity(input) {
    ...
    for (const [key, value] of Object.entries(frontmatter ?? {})) {
        if (contractedKeys.has(key) || !isGovernanceNamespaceKey(key) || !isDeclared(value))
            continue;
        checkedFields.push(key);
        if (isRepresented(input.record[key]))
            continue;
        findings.push({
            code: TASK_FRONTMATTER_FIDELITY_LOSS_CODE,
            severity: 'error',
