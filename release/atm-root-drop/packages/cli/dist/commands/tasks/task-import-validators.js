...
export function buildAtomMapProjectionScopeDiagnostic(input) {
    ...
    if (!normalized.includes(projection))
        return null;
    const hasOwnerShard = normalized.some((entry) => entry === 'atomic_workbench/atomization-coverage/path-to-atom-map-shards'
...
// ─── ATM-GOV-0406: bounded cohesive extractions ──────────────────────────────
// The two units below moved out verbatim so this module stays inside the
// physical line budget. They are re-exported here because this file is the
// stable import path every caller already uses; moving the symbols must not
// move the callers.
export { ATOMIZATION_DEFAULT_MAX_LINES, EXTRACTION_FIRST_LINE_BUDGET, buildExtractionFirstPatrolDiagnostics, parseContextMap, resolveAtomizationLinePolicy } from "./task-atomization-policy-import.js";
export { parseCausalValidatorCardFields, parseTddCardFields, validateCausalValidatorContractImport, validateTddCardImport } from "./task-card-contract-import.js";
