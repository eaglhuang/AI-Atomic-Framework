...
try {
    ...
    const updated = JSON.parse(readFileSync(taskPath, 'utf8'));
...
function readdirFirstJson(directory) {
...
{
    const { extractFrontMatter } = await import('../task-import-validators.js');
    ...
    const candidates = front.data.atomizationImpact.extractionCandidates;
...
{
    const { buildExtractionFirstPatrolDiagnostics, EXTRACTION_FIRST_LINE_BUDGET } = await import('../task-import-validators.js');
    const lineCounts = {
        'packages/cli/src/commands/big-module.ts': EXTRACTION_FIRST_LINE_BUDGET + 1,
        'packages/cli/src/commands/small-module.ts': 42
    };
    const resolveLineCount = (relativePath) => lineCounts[relativePath] ?? null;
...
// ATM-BUG-2026-07-13-178 — abandoned ledger reopen must not require --force/emergency.
{
    ...
    try {
        ...
        const reopened = JSON.parse(readFileSync(abandonedTaskPath, 'utf8'));
