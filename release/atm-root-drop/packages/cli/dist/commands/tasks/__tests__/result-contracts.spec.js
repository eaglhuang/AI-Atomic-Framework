 ...
 */
import { KNOWN_TASK_RESULT_CONTRACT_SCHEMA_IDS, TASK_DELIVERABLE_GATE_SCHEMA_ID, TASK_IMPORT_MANIFEST_SCHEMA_ID, TASK_LEGACY_LEDGER_MIGRATION_SCHEMA_ID, TASK_RESULT_CONTRACT_SPEC_VERSION, TASK_VERIFY_REPORT_SCHEMA_ID, isKnownTaskResultContract } from '../result-contracts.js';
...
for (const id of expected) {
    assert(KNOWN_TASK_RESULT_CONTRACT_SCHEMA_IDS.includes(id), `KNOWN_TASK_RESULT_CONTRACT_SCHEMA_IDS must continue to include ${id}`);
}
assert(KNOWN_TASK_RESULT_CONTRACT_SCHEMA_IDS.length === expected.length, 'KNOWN_TASK_RESULT_CONTRACT_SCHEMA_IDS must not grow silently — add a new schema to the spec when extending');
