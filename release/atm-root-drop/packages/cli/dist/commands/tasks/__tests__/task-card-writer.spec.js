...
import { writeImportEvidence, writeTaskFiles } from '../task-card-writer.js';
import { parsePlanMarkdown } from '../legacy-impl.js';
...
try {
    ...
    assert.ok(splitCard.tasks[0].importDiagnostics?.some((entry) => entry.code === 'ATM_TASK_IMPORT_MECHANICAL_SPLIT_SCOPE_CHECKLIST'), 'mechanical split cards with one declared file should surface a one-shot scope checklist');
}
finally {
