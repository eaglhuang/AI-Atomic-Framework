import { buildPlanningMirrorClosebackExpectation, classifyPlanningMirrorPreEdit } from '../planning-mirror-close-diagnostics.js';
...
assert(classifyPlanningMirrorPreEdit({
    relativePath: 'docs/tasks/TASK-AAO-0143.task.md',
    fileContent: correctCard,
    expectation
}) === 'correct-pre-edit', 'matching closeback frontmatter must classify as correct pre-edit');
assert(classifyPlanningMirrorPreEdit({
    relativePath: 'docs/tasks/TASK-AAO-0143.task.md',
    fileContent: incorrectCard,
    expectation
}) === 'incorrect-pre-edit', 'wrong delivery_commit must classify as incorrect pre-edit');
assert(classifyPlanningMirrorPreEdit({
    relativePath: 'docs/tasks/TASK-AAO-0143.task.md',
    fileContent: correctCard.replace('status: done', 'status: in-progress'),
    expectation
}) === 'not-applicable', 'non-done status must not count as closeback pre-edit');
