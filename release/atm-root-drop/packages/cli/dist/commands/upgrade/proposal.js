...
import { CliError, makeResult, message, readJsonFile, resolveValue } from '../shared.js';
import { collectJsonFiles, requireOptionValue } from './path-helpers.js';
// ─── Option parsing ────────────────────────────────────────────────────────
export function parseUpgradeOptions(argv) {
    const options = {
        cwd: process.cwd(),
        propose: false,
        scan: false,
...
export async function evaluateUpgradeContextBudget(options, inputDocuments) {
    ...
    const estimatedTokens = estimateContextBudgetTokens(qualityComparisonInput.document, renderQualityReportMarkdown(qualityComparisonInput.document));
    ...
    const evaluation = governanceEnabled
        ? await resolveValue(createLocalGovernanceAdapter({ repositoryRoot: options.cwd }).stores.contextBudgetGuard.evaluateBudget(evaluationInput))
...
async function materializeUpgradeHardStop(cwd, atomId, qualityReportPath, evaluation, generatedAt) {
