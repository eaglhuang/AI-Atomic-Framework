...
function categorizeCheckpointCloseFailure(closeResult, taskId, actorId) {
    ...
    const missingValidationPasses = Array.isArray(errorMsg?.data?.missingValidationPasses) ? errorMsg.data.missingValidationPasses : [];
    const blockingFindings = Array.isArray(errorMsg?.data?.blockingFindings) ? errorMsg.data.blockingFindings : [];
    if (code === 'ATM_TASK_CLOSE_EVIDENCE_REQUIRED' || code === 'ATM_TASK_CLOSE_CLOSURE_PACKET_INVALID') {
        return { category: 'missing-evidence', reason: tldr ?? `Task ${taskId} lacks required command-backed evidence or a valid closure packet.`,
    ...
    if (code === 'ATM_TASK_CLOSE_DELIVERABLE_DIFF_REQUIRED') {
        return { category: 'missing-deliverable', reason: `Task ${taskId} has no real non-.atm deliverable diff; implement the required files first.`, requiredCommand: null, tldr, missingValidationPasses, blockingFindings };
    }
    if (code === 'ATM_TASK_CLOSE_FRAMEWORK_DIFF_ACTIVE' || code === 'ATM_TASK_CLOSE_FRAMEWORK_GATE_FAILED') {
        const requiredCommand = typeof errorMsg?.data?.requiredCommand === 'string' ? errorMsg.data.requiredCommand : null;
        return { category: 'framework-gate-failed', reason: tldr ?? `Task ${taskId} cannot close due to ATM framework delivery window or gate blocker.`, requiredCommand, tldr, missingValidationPasses, blockingFindings };
    }
    if (code === 'ATM_TASK_CLOSE_ACTIVE_CLAIM_REQUIRED') {
        return { category: 'no-active-claim', reason: `Task ${taskId} has no active claim owned by ${actorId}.`, requiredCommand: `node atm.mjs next --claim --actor ${actorId} --prompt "${taskId}" --json`, tldr, missingValidationPasses, blockingFindings };
    }
    if (code === 'ATM_TASK_CLOSE_OWNER_MISMATCH') {
        return { category: 'owner-mismatch', reason: `Task ${taskId} is owned by a different actor; use takeover or correct --actor.`, requiredCommand: `node atm.mjs tasks takeover --task ${taskId} --actor ${actorId} --json`, tldr, missingValidationPasses, blockingFindings };
    }
