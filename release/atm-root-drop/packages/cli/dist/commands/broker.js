...
import { enqueueSharedSurface, planSharedSurfaceAcquisition, removeSharedSurfaceEntry } from '../../../core/dist/broker/shared-surface-queue.js';
import { cleanupRunnerSyncStewardQueue, emptyRunnerSyncStewardQueue, enqueueRunnerSyncStewardRequest, explainRunnerSyncStewardPosition, releaseRunnerSyncStewardQueue } from '../../../core/dist/broker/runner-sync-steward-queue.js';
import { cleanupGeneratedProjectionSteward, emptyGeneratedProjectionSteward, enqueueGeneratedProjectionRebuild } from '../../../core/dist/broker/generated-projection-steward.js';
import { acknowledgeFreeze, createFreezeSignal, resolveFreezeDecision } from '../../../core/dist/broker/freeze.js';
...
export async function runBroker(argv) {
    ...
    if (options.action === 'steward') {
        ...
        if (options.stewardAction === 'plan') {
            ...
            return makeResult({
                ...
                cwd: options.cwd,
                messages: [
                    message(planResult.ok ? 'info' : 'error', planResult.ok ? 'ATM_BROKER_STEWARD_PLANNED' : 'ATM_BROKER_STEWARD_PLAN_BLOCKED', planResult.ok
                        ? `Steward plan '${planResult.plan.mergePlanId}' is ready to apply.`
                        : 'Steward plan blocked by validation issues.', { mergePlanId: mergePlan.mergePlanId, issueCount: planResult.plan.issues.length })
                ],
                evidence: {
                    action: 'steward-plan',
                    stewardId,
        ...
        if (options.stewardAction === 'apply') {
            ...
            return makeResult({
                ...
                cwd: options.cwd,
                messages: [
                    message(ok ? 'info' : 'error', ok ? 'ATM_BROKER_STEWARD_APPLIED' : 'ATM_BROKER_STEWARD_APPLY_BLOCKED', ok
    ...
    if (options.action === 'runtime') {
        ...
        return makeResult({
            ...
            cwd: options.cwd,
            messages: [
                message(applyResult.ok ? 'info' : 'error', applyResult.ok ? 'ATM_BROKER_RUNTIME_SCOPED_WRITE_APPLIED' : 'ATM_BROKER_RUNTIME_SCOPED_WRITE_BLOCKED', applyResult.ok
    ...
    if (options.action === 'compose') {
        ...
        return makeResult({
            ...
            cwd: options.cwd,
            messages: [
                message(blocked ? 'error' : composeResult.mergePlan.verdict === 'needs-steward' ? 'warn' : 'info', blocked ? 'ATM_BROKER_COMPOSE_BLOCKED' : 'ATM_BROKER_COMPOSE_PLANNED', blocked
    ...
    if (options.action === 'plan-batch') {
        const requestPaths = [...options.requestFiles.map((file) => path.resolve(options.cwd, file))];
        ...
        return makeResult({
            ...
            cwd: options.cwd,
            messages: [
                message(ok ? 'info' : 'warn', 'ATM_BROKER_PLAN_BATCH', `Planned ${plan.batches.length} batch(es) over ${requests.length} request(s); queued=${plan.queued.length}, blocked=${plan.blocked.length}${options.apply ? `, applied=${applied}` : ''}.`, { planId: plan.planId, batchCount: plan.batches.length, casMismatchCount: casMismatches.length })
            ],
            evidence: {
                action: 'plan-batch',
                explicitInputs,
...
function updateSharedSurfaceQueues(input) {
    ...
    if (!input.shouldQueue)
        return { queues, newlyQueued: [] };
    const activeOwnersByPath = new Map();
    for (const active of input.registry.activeIntents) {
        if (active.taskId === input.intent.taskId)
            continue;
        for (const filePath of active.resourceKeys.files) {
            const normalized = filePath.replace(/\\/g, '/');
            if (!input.intent.targetFiles.map((value) => value.replace(/\\/g, '/')).includes(normalized))
                continue;
    ...
    for (const surfacePath of sharedPaths) {
        ...
        for (const owner of activeOwnersByPath.get(surfacePath) ?? []) {
            ...
            if (!seeded.ok)
                throw new CliError('ATM_BROKER_SHARED_QUEUE_BLOCKED', seeded.reason, { exitCode: 1, details: { surfacePath, code: seeded.code } });
            queue = seeded.queue;
        }
        ...
        if (!result.ok)
            throw new CliError('ATM_BROKER_SHARED_QUEUE_BLOCKED', result.reason, { exitCode: 1, details: { surfacePath, code: result.code } });
        const queueHead = (activeOwnersByPath.get(surfacePath) ?? [])[0];
        if (result.position && result.position > 1 && queueHead)
            newlyQueued.push({ surfacePath, queueHead });
        if (index >= 0)
            next[index] = result.queue;
        else
            next.push(result.queue);
    }
    writeSharedSurfaceQueues(input.queuePath, next);
    return { queues: next, newlyQueued };
}
function createSharedSurfaceFreezeRecords(input) {
...
function markReleasedSharedSurfaceFreezes(input) {
    const now = new Date().toISOString();
    return input.records.map((record) => {
        if (record.signal.taskId !== input.releasedTaskId || record.status === 'released')
            return record;
        ...
        if (!waitingIsHead)
            return record;
        return { ...record, status: 'released', updatedAt: now };
    });
}
function sharedQueueEntry(owner, surfacePath, reason) {
    return {
        taskId: owner.taskId,
...
function shouldQueueSharedSurface(decision) {
...
function resolveSharedSurfaceQueueAdmission(input) {
...
function replaceIntentLane(registry, taskId, lane) {
    return {
        ...registry,
...
function relativeStorePath(cwd, storePath) {
