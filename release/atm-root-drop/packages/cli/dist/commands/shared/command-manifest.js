 ...
 */
export const TASK_SCOPED_SUBCOMMANDS_REQUIRING_TASK_ID = {
...
export const EXECUTABLE_AGGREGATE_STATUS_COMMANDS = [
 ...
 */
export function parseAtmCommandToManifest(command) {
 ...
 */
export function inspectCommandExecutability(command) {
    const trimmed = command.trim();
    if (trimmed.length === 0)
        return { ok: false, reason: 'empty-command', taskScoped: false };
    ...
    const hasTaskId = tokens.some((token, index) => token === '--task' && typeof tokens[index + 1] === 'string' && tokens[index + 1].length > 0 && !tokens[index + 1].startsWith('--'));
    return hasTaskId
