...
function parseGuideArgs(argv = []) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--intent') {
            const learnedIntent = requireOptionValue(argv, index, '--intent');
        ...
        if (arg === '--status') {
            const status = requireOptionValue(argv, index, '--status');
        ...
        if (arg === '--target') {
            const target = requireOptionValue(argv, index, '--target');
...
function buildLearnGuide(cwd, phrase, learnedIntent, reason, status) {
