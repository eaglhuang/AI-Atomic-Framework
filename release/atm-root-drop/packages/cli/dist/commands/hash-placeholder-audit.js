...
export function runHashPlaceholderAudit(options = {}) {
    ...
    for (const filePath of files) {
        ...
        if (repeatedDigitDigestPattern.test(content))
            findings.push({ file: relativePath, issue: 'placeholder-repeated-sha256-digest' });
        if (/placeholder digest|dummy digest/i.test(content))
            findings.push({ file: relativePath, issue: 'placeholder-digest-text' });
    }
