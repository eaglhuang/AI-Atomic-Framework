...
export function handleBrokerParallelAdmissionPolicy(options, _context) {
    ...
    if (action === 'set') {
        ...
        if (options.policyMode)
            patch.mode = options.policyMode;
        if (options.policyFallbackMode)
            patch.fallbackMode = options.policyFallbackMode;
        if (options.policyCircuitBreaker !== null)
            patch.circuitBreakerEnabled = options.policyCircuitBreaker;
        if (options.scopeFiles.length > 0)
            patch.rolloutScope = options.scopeFiles;
...
function buildPolicyResult(options, action, policy, code, text) {
