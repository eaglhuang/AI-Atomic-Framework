...
import { cleanupRunnerSyncStewardQueue, enqueueRunnerSyncStewardRequest, explainRunnerSyncStewardPosition, releaseRunnerSyncStewardQueue } from '../../_vendor/core/dist/broker/runner-sync-steward-queue.js';
...
import { cleanupGeneratedProjectionSteward, enqueueGeneratedProjectionRebuild } from '../../_vendor/core/dist/broker/generated-projection-steward.js';
...
import { runRunnerSyncTakeoverPublication } from './publication-takeover-admission.js';
export function handleBrokerStewardQueues(options, context) {
    ...
    if (options.action === 'runner-sync') {
        ...
        if (options.runnerSyncAction === 'release') {
            const missing = [!options.task ? '--task <task-id>' : null, !options.stewardWorkId ? '--steward-work-id <id>' : null, !options.receiptRef && !options.receiptDigest ? '--receipt-ref <path> or --receipt-digest <sha256>' : null].filter((entry) => Boolean(entry));
            if (missing.length > 1)
                throw new CliError('ATM_CLI_USAGE', `broker runner-sync release is missing ${missing.join(', ')}.`, { exitCode: 2, details: { missing, requiredCommand: 'node atm.mjs broker runner-sync release --task <task-id> --steward-work-id <id> --receipt-ref <path> --json' } });
            if (!options.task)
        ...
        if (options.runnerSyncAction === 'reconcile-receipt') {
            ...
            try {
                ...
                return makeResult({
                    ...
                    cwd: options.cwd,
                    messages: [
                        message('info', 'ATM_BROKER_RUNNER_SYNC_RECEIPT_RECONCILED', reconciliation.decision === 'deleted-untracked-orphan'
                            ...
                            : `Restored the verified stale runner receipt ${reconciliation.legacyReceiptPath} through the receipt-only recovery route.`, reconciliation)
                    ],
                    evidence: {
                        runnerSyncStewardQueuePath: '.atm/runtime/runner-sync-steward-queue.json',
                        reconciliation,
...
function resumeRunnerSyncCommand(stewardWorkId, sealedSourceSha) {
