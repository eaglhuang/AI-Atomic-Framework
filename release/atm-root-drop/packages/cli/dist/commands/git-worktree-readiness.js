...
export function inspectGitWorktreeReadiness(cwd) {
    ...
    if (isBareRepository === true && markerRoot) {
        return {
            ...
            isBareRepository,
            isInsideWorkTree,
            reason: firstReason(root.stderr, inside.stderr, bare.stderr, root.stdout, inside.stdout, bare.stdout, 'Git reports this path as a bare repository even though a worktree-local .git marker exists.'),
...
function findNearestGitMarkerRoot(start) {
...
function resolveGitDirPath(cwd, gitDir) {
    if (!gitDir)
        return null;
