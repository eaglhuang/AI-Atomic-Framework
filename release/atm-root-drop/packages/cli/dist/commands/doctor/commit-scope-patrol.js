...
function taskIdsFromPaths(paths) {
    const ids = new Set();
    for (const filePath of paths) {
        for (const match of filePath.matchAll(TASK_ID_PATTERN))
            ids.add(normalizeTaskId(match[0]));
    }
    return [...ids].sort();
}
export function inspectHistoricalCommitScopePatrol(root, limit = 200) {
