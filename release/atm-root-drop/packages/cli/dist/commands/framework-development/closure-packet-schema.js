...
function classifyGovernanceHistoryArtifact(filePath) {
    const normalized = normalizeRelativePath(filePath);
...
async function runFrameworkTempClaimWithAutoReconcile(input) {
    ...
    let lock;
    let autoReconcileEvidence = null;
    try {
...
function canAutoReconcileCompletedSelfTempLock(staleLock, actorId) {
...
export function auditTasks(cwd) {
    ...
    const annotatedFindings = findings.map((entry) => {
        if (entry.level !== 'warning')
            return entry;
        if (!baseline.acknowledgedFindings.has(taskAuditFindingKey(entry)))
            return entry;
                ...
                if (targetCommit) {
                    ...
                    reconstructedPacket = {
                        ...
                        validationPasses: Array.isArray(closureInfo.validationPasses)
                            ? closureInfo.validationPasses.filter((entry) => typeof entry === 'string')
                            : [],
                        ...
                        requiredGates: Array.isArray(closureInfo.requiredGates)
                            ? closureInfo.requiredGates.filter((entry) => typeof entry === 'string')
                            : [],
                        requiredGatesSnapshot: closureInfo.requiredGatesSnapshot && typeof closureInfo.requiredGatesSnapshot === 'object' && !Array.isArray(closureInfo.requiredGatesSnapshot)
                            ? closureInfo.requiredGatesSnapshot
...
function readTaskAuditWarningBaseline(root) {
...
function taskAuditFindingKey(finding) {
...
function readTrackedChangeStatuses(cwd) {
    const statuses = new Map();
    for (const args of [['diff', '--name-status'], ['diff', '--cached', '--name-status']]) {
