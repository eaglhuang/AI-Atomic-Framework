...
import { adoptLaneSession, classifyLaneSessionTtl, hashHandoffToken, mintLaneSession, readLaneSession, runtimeLaneSessionsRootRelativePath } from '../store.js';
...
try {
    ...
    assert.equal(classifyLaneSessionTtl({ now: '2026-07-16T00:00:30.000Z', expiresAt: minted.session.expiresAt, graceMs: 10_000 }), 'fresh');
    assert.equal(classifyLaneSessionTtl({ now: '2026-07-16T00:01:05.000Z', expiresAt: minted.session.expiresAt, graceMs: 10_000 }), 'grace');
    assert.equal(classifyLaneSessionTtl({ now: '2026-07-16T00:01:11.000Z', expiresAt: minted.session.expiresAt, graceMs: 10_000 }), 'expired');
