...
const OPERATION_TO_LANE_COMMAND = {
...
export function laneCommandClassForOperation(operation) {
 ...
 */
export function evaluateLaneMutation(input) {
    ...
    const capabilityDecision = authorizeMutationCapability({
        ...
        now: input.now
    }, capabilitySnapshot, capabilityPolicy);
    const blockedBy = !laneDecision.allowed
        ? 'lane'
