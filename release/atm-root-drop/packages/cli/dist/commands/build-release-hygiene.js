...
export function shouldRetainReleaseArtifacts() {
...
export function restoreTrackedReleaseArtifacts(repoRoot) {
    if (!isGitRepository(repoRoot)) {
        return [];
    }
    const existing = trackedReleaseManifestPaths.filter((relativePath) => existsSync(path.join(repoRoot, relativePath)));
...
export function finalizeBuildReleaseHygiene(repoRoot) {
    ...
    if (restored.length > 0) {
        console.log(`[build-release-hygiene] restored tracked release manifests to HEAD (${restored.join(', ')}). `
            + `Set ${policy.retainEnvVar}=1 before build when runner sync must keep generated release mirrors.`);
        return;
    }
    console.log(`[build-release-hygiene] no tracked release manifests required cleanup. `
