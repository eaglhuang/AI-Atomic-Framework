...
function toPlanningRootRelativeFromDocPath(storedPath) {
    const normalized = normalizeStoredPlanningPath(storedPath);
    if (!isPlanningRootDocStoredPath(normalized))
        return null;
    if (normalized === PLANNING_ROOT_RELATIVE_SUFFIX.replace(/\\/g, '/'))
        return '';
...
export function looksLikePlanningRootRelativePath(storedPath) {
    const normalized = normalizeStoredPlanningPath(storedPath);
    if (!normalized || normalized.startsWith('../'))
        return false;
    if (TARGET_REPO_ROOT_PREFIXES.some((prefix) => normalized.startsWith(prefix)))
        return false;
    if (normalized.startsWith('docs/'))
        return false;
    return normalized.includes('/');
}
export function readPlanningRootEnv() {
    const raw = process.env[PLANNING_REPO_ROOT_ENV]?.trim();
    if (!raw)
        return null;
    return path.resolve(raw);
}
export function readConfiguredPlanningRoots(cwd) {
    const configPath = path.join(cwd, '.atm', 'config.json');
    if (!existsSync(configPath))
        return [];
    try {
        const parsed = parseJsonText(readFileSync(configPath, 'utf8'));
        const taskLedger = parsed.taskLedger && typeof parsed.taskLedger === 'object' && !Array.isArray(parsed.taskLedger)
            ? parsed.taskLedger
            : {};
        return readStringArray(taskLedger.planningRoots
...
export function resolveStoredPlanningPath(cwd, storedPath) {
...
export function normalizeStoredPlanningPathForIdentity(cwd, storedPath) {
    const resolved = resolveStoredPlanningPath(cwd, storedPath);
    if (resolved.planningRelativePath)
        return resolved.planningRelativePath;
    return resolved.storedPath;
}
export function resolvePlanningPathFromStored(cwd, storedPath) {
    if (!storedPath) {
...
export function resolvePlanAbsoluteFromStored(cwd, storedPath) {
    return resolveStoredPlanningPath(cwd, storedPath).absolutePath;
}
export function buildPlanningRootMissingDiagnostic(cwd) {
...
export function shouldReportPlanningRootMissing(input) {
    if (!input.taskScopeMentioned || input.matchedTaskCount > 0)
        return null;
...
function normalizeStoredPlanningPath(value) {
