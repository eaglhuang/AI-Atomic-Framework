...
import { run as runEvidenceDiagnose } from './evidence/verbs/diagnose.js';
...
import { run as runWaveExitObserver } from './evidence/verbs/wave-exit-observer.js';
...
export async function runEvidence(argv) {
    ...
    if (action === 'diagnose')
        return runEvidenceDiagnose(argv.slice(1));
    ...
    if (action === 'wave-exit-observer')
        return runWaveExitObserver(argv.slice(1));
