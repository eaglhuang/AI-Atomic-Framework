...
function buildApplyReadyPacket(queueRecord) {
    ...
    const rollbackInstructions = Array.isArray(queueRecord.proposal?.rollbackInstructions)
        ? queueRecord.proposal.rollbackInstructions.filter((entry) => typeof entry === 'string')
        : [];
    return {
        proposalId: queueRecord.proposalId,
...
function buildRolloutReadyPacket(cwd, queueRecord) {
...
function loadActualPatchEvidence(cwd, proposalId) {
...
function splitLegacyTarget(legacyTarget) {
    if (!legacyTarget) {
        return [null, null];
    }
...
function runCheckProgression(cwd, mapId, forcePause) {
    if (!mapId) {
    ...
    return makeResult({
        ...
        cwd,
        messages: [
            message(result.canPromote ? 'info' : 'warn', result.canPromote ? 'ATM_PROGRESSION_CAN_PROMOTE' : 'ATM_PROGRESSION_BLOCKED', result.canPromote
                ? `Progression check passed: ${mapId} can advance from ${result.currentLane} to ${result.nextLane}.`
                : `Progression blocked for ${mapId}: ${result.blockedReasons[0] ?? 'unknown reason'}.`, { mapId, canPromote: result.canPromote, blockedReasons: result.blockedReasons })
        ],
        evidence: {
