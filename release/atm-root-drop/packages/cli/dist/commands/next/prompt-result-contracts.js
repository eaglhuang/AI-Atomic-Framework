...
import { buildAgentPackHint, buildChannelPlaybook, buildGovernanceReadinessHint, buildNextMessages, buildTaskDeliveryPrinciple, embedTeamRecommendation } from './playbook-projection.js';
import { findActiveBatchRunForIntent, findActiveTaskQueueForIntent } from './route-resolution.js';
...
export function buildPlanningCardImportRequirement(task) {
    if (!task)
        return null;
...
export function isReadOnlyPromptScopeMiss(taskIntent) {
    if (!taskIntent)
        return false;
...
export function buildPromptScopeQueueResult(input) {
    ...
    if (claimReadiness?.primaryBlocker) {
        ...
        return makeResult({
            ...
            command: 'next',
            cwd: input.cwd,
            messages: buildNextMessages(nextAction, null, input.integrationBootstrap, input.runtimeAdapterReadiness, message('error', blocker.blockerCode, blocker.blockerSummary, {
