...
import { evaluateRuntimeBudget, fileSize, isKnowledgeShardFile, isRuntimePrunableCache, resolveBudgetOptions, resolveKnowledgeOutputs, walkFiles } from './runtime-utils.js';
import { normalizePath, normalizeWhitespace, stripMarkdown } from './text-utils.js';
export function buildKnowledgeIndex(cwd, scope) {
...
export function buildKnowledgeStats(cwd, options) {
...
export function inspectKnowledgeShard(cwd, file) {
...
export function buildIndexEntry(cwd, file) {
    const relativePath = normalizePath(path.relative(cwd, file));
    const body = readFileSync(file, 'utf8');
...
export function extractMetadata(body, relativePath) {
    const title = body.match(/^#\s+(.+)$/m)?.[1]?.trim() || path.basename(relativePath);
    const metadata = {
