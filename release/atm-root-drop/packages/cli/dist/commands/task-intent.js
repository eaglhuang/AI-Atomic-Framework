 ...
 */
export function classifyTaskDelivery(input) {
    ...
    if (closureAuthority)
        diagnostics.push(`closure-authority:${closureAuthority}`);
    if (targetRepo)
        diagnostics.push(`target-repo:${targetRepo}`);
    if (planningRepo)
        diagnostics.push(`planning-repo:${planningRepo}`);
    if (closureAuthority === 'planning_repo' && !targetIsCurrentRepo) {
        ...
        if (statusDivergence || (sourceStatus ?? '').toLowerCase() === 'done') {
            recommendedActions.push(`node atm.mjs tasks import --from ${quoteIfNeeded(sourcePath)} --write --force --json`);
            recommendedActions.push('Inspect the refreshed ledger mirror to confirm the source-card status (e.g. `done`) is preserved.');
        }
        else {
            recommendedActions.push(`Do not implement deliverables for ${task.workItemId} from this repo; the task is owned by ${targetRepo ?? planningRepo ?? 'a different repo'} via planning_repo authority.`);
            recommendedActions.push(`node atm.mjs tasks import --from ${quoteIfNeeded(sourcePath)} --dry-run --json`);
        }
        return {
...
function readSourceTaskCardStatus(cwd, sourcePlanPath, workItemId) {
    ...
    try {
        ...
        for (const rawLine of block.split(/\r?\n/)) {
            ...
            if (taskIdMatch)
                blockTaskId = taskIdMatch[1].trim().replace(/^['"]|['"]$/g, '');
            const statusMatch = /^status\s*:\s*(.+)$/.exec(rawLine);
            if (statusMatch)
                blockStatus = statusMatch[1].trim().replace(/^['"]|['"]$/g, '');
        }
        if (blockTaskId && blockTaskId.toUpperCase() !== workItemId.toUpperCase())
            return null;
        return blockStatus;
    }
    catch {
...
function matchesCurrentRepoIdentityLite(cwd, targetRepo) {
