...
import { createAtomBundle, exportAtomCapsule, importAtomCapsule, validateCidFormat, cidToShortId, AtomCapsuleError } from '../_vendor/core/dist/registry/atom-capsule.js';
import { loadCapsuleRegistry, saveCapsuleRegistry, upsertCapsuleEntry, linkCapsuleChain, markCapsuleRolledBack, listAdvisoryCids, syncRegistries, getGlobalRegistryPath, getRepoRegistryPath } from '../_vendor/core/dist/registry/capsule-registry.js';
...
export async function runAtomCapsule(argv) {
    ...
    switch (action) {
        ...
        default:
            throw new CliError('ATM_CLI_USAGE', `Unknown action "${action}". Usage: atm atom-capsule <export|import|rollback|advisories> [options]`, { exitCode: 2 });
    }
}
async function runExport(cwd, options) {
...
async function runImport(cwd, options) {
...
async function runRollback(cwd, options) {
    const cid = options.cid ?? '';
    if (!cid) {
...
async function runAdvisories(cwd, _options) {
    const globalRegistryPath = getGlobalRegistryPath();
    const globalRegistry = loadCapsuleRegistry(globalRegistryPath);
