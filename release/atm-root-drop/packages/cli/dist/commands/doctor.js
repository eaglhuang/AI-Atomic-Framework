...
export async function runDoctor(argv) {
    ...
    const hasTsNoCheck = listFiles(path.join(root, 'packages'))
        ...
        .filter((filePath) => !filePath.endsWith(path.join('packages', 'cli', 'src', 'commands', 'doctor.ts')))
...
function resolveDoctorPolicy(options) {
...
function checkOnboardingLifecycle(root, runtime) {
...
function createVersionSummaryMessages(versionSummary) {
    const messages = [];
...
function isFrameworkContractExpected(repoIdentity) {
...
function checkCharterIntegrity(root) {
...
function checkCharterIntegrityV2(root) {
    const charterPath = path.join(root, '.atm', 'charter', 'atomic-charter.md');
