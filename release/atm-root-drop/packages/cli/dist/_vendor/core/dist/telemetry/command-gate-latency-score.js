...
export function unionIntervalsMs(intervals) {
    const normalized = intervals.map((interval) => {
        ...
        if (interval.endMs < interval.startMs)
            throw new Error('interval.endMs must be >= interval.startMs');
        return interval;
...
function exclusiveSpanMs(span, spans) {
...
function sampleTiming(sample) {
    ...
    if (spans.length === 0)
        return { inclusiveMs: sample.durationMs, exclusiveMs: sample.durationMs };
...
function validateLatencySample(sample) {
    if (!sample.sampleId || !sample.key || !sample.command || !sample.gate)
        throw new Error('latency sample identity is required');
    assertFiniteNonNegative(sample.durationMs, 'sample.durationMs');
    const frequency = sample.frequency ?? 1;
    if (!Number.isFinite(frequency) || frequency <= 0)
        throw new Error('sample.frequency must be positive when provided');
    if (sample.taskInterval) {
        ...
        if (sample.taskInterval.endMs < sample.taskInterval.startMs)
            throw new Error('taskInterval.endMs must be >= taskInterval.startMs');
    }
}
export function buildCommandGateLatencyReport(input) {
    ...
    if (inventoryByKey.size !== input.inventory.length)
        throw new Error('latency inventory keys must be unique');
    for (const sample of input.samples) {
        validateLatencySample(sample);
        if (!inventoryByKey.has(sample.key))
            throw new Error(`sample ${sample.sampleId} is not present in inventory: ${sample.key}`);
    }
    if (input.measurementOverheadMs !== undefined && input.measurementOverheadMs !== null)
        assertFiniteNonNegative(input.measurementOverheadMs, 'measurementOverheadMs');
    const grouped = new Map();
    for (const sample of input.samples)
        grouped.set(sample.key, [...(grouped.get(sample.key) ?? []), sample]);
    const scores = input.inventory.map((entry) => {
...
export function validateCommandGateLatencyReport(report) {
    const findings = [];
    if (report.schemaId !== 'atm.commandGateLatencyScore.v1')
        findings.push('schemaId mismatch');
    if (report.observedCount > report.inventoryCount)
        findings.push('observedCount exceeds inventoryCount');
    if (report.coveragePct < 0 || report.coveragePct > 100)
        findings.push('coveragePct must be between 0 and 100');
    if (report.unknownKeys.some((key) => !report.uncoveredKeys.includes(key)))
        findings.push('unknown keys must be uncovered');
    if (report.scores.some((score) => !score.measured && (score.p50Ms !== null || score.p95Ms !== null || score.cumulativeTaskWaitingMs !== null)))
        findings.push('unmeasured score cannot contain fabricated timing');
    if (report.hotspots.some((score) => !score.measured))
        findings.push('hotspots must contain measured scores only');
    return findings;
}
export function compareCommandGateLatencyReports(baseline, candidate) {
...
export function buildCommandGateLatencyMarkdown(report) {
