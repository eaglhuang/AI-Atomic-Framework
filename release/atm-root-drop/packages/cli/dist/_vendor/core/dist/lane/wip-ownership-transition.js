 ...
 */
export function planWipTransition(request, snapshot, policy = {}) {
    ...
    const build = (allowed, transitionClass, errorCode, nextOwnerLaneId, recoveryCommand, reason) => {
    ...
    switch (request.kind) {
        case 'release': {
            if (dirtyCount === 0) {
                return build(true, 'release-clean', null, null, reclaimCommand(request.taskId), 'Release with no in-scope dirty WIP; nothing to own.');
            }
            ...
            return build(true, 'release-wip-retained', null, request.requestingLaneId, reclaimCommand(request.taskId), 'Release retains in-scope dirty WIP ownership on the original lane; resume via reclaim.');
        }
        case 'reclaim': {
            ...
            if (dirtyCount > 0 && owner && owner !== request.requestingLaneId) {
                return build(false, 'blocked-foreign-wip', 'ATM_CLAIM_FOREIGN_UNSTAGED_WIP', owner, repairCommand(request.taskId), 'Reclaim refused: in-scope dirty WIP belongs to a different recorded owner lane. Resume as that lane or seal a handoff.');
            }
            return build(true, 'reclaim-resume', null, dirtyCount > 0 ? request.requestingLaneId : null, reclaimCommand(request.taskId), 'Reclaim resumes the recorded WIP under the original owner lane.');
        }
        case 'handoff': {
            const handoff = request.handoff ?? null;
            const sealed = handoff != null &&
                handoff.taskId === request.taskId &&
                handoff.fromLaneId === request.requestingLaneId &&
                normalize(handoff.toLaneId) != null;
            if (!sealed) {
                return build(false, 'blocked-unsealed-handoff', null, request.requestingLaneId, handoffCommand(request.taskId, fingerprint(handoff?.toLaneId, 'lane')), 'Handoff refused: a sealed handoff naming this task and from-lane is required. WIP ownership stays on the original lane.');
            }
            return build(true, 'handoff-transfer', null, handoff.toLaneId, reclaimCommand(request.taskId), 'Sealed handoff transfers in-scope WIP ownership to the target lane.');
        }
        case 'discard': {
            const receipt = request.destructiveReceipt ?? null;
            const validReceipt = receipt != null && receipt.taskId === request.taskId && receipt.laneId === request.requestingLaneId;
            if (requireReceipt && !validReceipt) {
                return build(false, 'blocked-discard-requires-receipt', null, request.requestingLaneId, discardCommand(request.taskId), 'Discard refused: explicit discard of dirty WIP requires a destructive-action receipt bound to this task and lane.');
            }
            return build(true, 'discard-sealed', null, null, reclaimCommand(request.taskId), 'Destructive-action receipt authorizes discard; recorded WIP ownership is cleared.');
        }
        default: {
            const exhaustive = request.kind;
