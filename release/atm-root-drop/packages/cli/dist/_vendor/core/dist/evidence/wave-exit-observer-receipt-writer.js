...
import { canonicalWaveExitReceiptPath, consumeWaveExitObserverReceipt, digestText, digestWaveExitObserverInput, digestWaveExitObserverPolicySource, WAVE_EXIT_OBSERVER_POLICY_PATH, WAVE_EXIT_OBSERVER_RECEIPT_SCHEMA_ID } from './wave-exit-observer-receipt.js';
...
export function deriveBasisActorsFromEvidenceOwners(repoRoot, owners) {
    const actors = [];
...
export function exclusiveAtomicCreate(dest, contents) {
    mkdirSync(dirname(dest), { recursive: true });
    if (existsSync(dest)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_RECEIPT_EXISTS', `Canonical wave-exit observer receipt already exists: ${dest}`, { details: { dest } });
    }
...
export function writeWaveExitObserverReceipt(input) {
    rejectCallerOverrides(input);
    if (!EXIT_ID_SHAPE.test(input.exitItemId)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_EXIT_UNMAPPED', `Exit item ${input.exitItemId} is not a sealed EXIT id.`, { diagnostics: ['exit-unmapped'], details: { exitItemId: input.exitItemId } });
    }
    const exitPolicy = input.policy.exits[input.exitItemId];
    if (!exitPolicy) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_EXIT_UNMAPPED', `Exit item ${input.exitItemId} is not mapped in the sealed observer policy.`, { diagnostics: ['exit-unmapped'], details: { exitItemId: input.exitItemId } });
    }
    ...
    if (input.claimedCommand && input.claimedCommand !== exitPolicy.command) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_UNAPPROVED_COMMAND', 'Observer command overrides are forbidden; the sealed policy owns the command.', { diagnostics: ['unapproved-command'], details: { claimedCommand: input.claimedCommand, sealedCommand: exitPolicy.command } });
    }
    if (input.observerRole !== exitPolicy.observerRole || input.policy.roles[input.observerRole]?.kind !== 'observer') {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_ROLE_MISMATCH', `Observer role ${input.observerRole} is not authorized for ${input.exitItemId}.`, { diagnostics: ['observer-role-mismatch'], details: { observerRole: input.observerRole, requiredRole: exitPolicy.observerRole } });
    }
    if (!COMMIT_SHAPE.test(input.observedHead) || !input.isAncestor(input.observedHead, input.observedHead)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_HEAD_UNREACHABLE', `observedHead ${input.observedHead} is not a reachable repository HEAD.`, { diagnostics: ['target-head-unreachable'], details: { observedHead: input.observedHead } });
    }
    const exists = input.receiptExists ?? ((path) => existsSync(path));
    const readExisting = input.readExistingReceipt ?? ((path) => {
        try {
    ...
    const baseReceipt = baseExists ? readExisting(baseAbsolutePath) : null;
    if (baseExists && (!baseReceipt || baseReceipt.observedHead === input.observedHead)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_RECEIPT_EXISTS', `Canonical wave-exit observer receipt already exists: ${baseArtifact}`, { details: { artifactPath: baseArtifact } });
    }
    ...
    if (input.claimedArtifactPath && normalizeRel(input.claimedArtifactPath) !== relativeArtifact) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_PATH_TRAVERSAL', 'Caller-supplied artifact paths are forbidden.', { diagnostics: ['artifact-path-mismatch'], details: { claimedArtifactPath: input.claimedArtifactPath, canonicalPath: relativeArtifact } });
    }
    const absolutePath = resolve(input.repoRoot, relativeArtifact);
    if (exists(absolutePath)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_RECEIPT_EXISTS', `Canonical wave-exit observer receipt already exists: ${relativeArtifact}`, { details: { artifactPath: relativeArtifact } });
    }
    const derived = input.derivedBasis;
    const uniqueBasis = [...new Set(derived.actorIds.filter(Boolean))];
    if (uniqueBasis.length !== 1) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_BASIS_UNRESOLVED', 'Basis producer actor could not be derived from evidence.', { diagnostics: ['basis-actor-unresolved'], details: { actorIds: derived.actorIds } });
    }
    const derivedBasisActor = uniqueBasis[0];
    if (input.claimedBasisActor && input.claimedBasisActor !== derivedBasisActor) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_CALLER_OVERRIDE', 'Self-filled basis actor does not match derived evidence identity.', { diagnostics: ['declared-basis-actor-mismatch'], details: { claimedBasisActor: input.claimedBasisActor, derivedBasisActor } });
    }
    if (input.observerActor === derivedBasisActor) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_ACTOR_CONFLICT', 'Observer actor is not independent of the derived basis producer.', { diagnostics: ['observer-basis-actor-conflict'], details: { observerActor: input.observerActor, derivedBasisActor } });
    }
    const currentInputDigests = {};
    for (const inputPath of exitPolicy.inputs) {
        const body = input.readObservedInput(inputPath);
        if (body == null) {
            throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_DIGEST_DRIFT', `Observed input ${inputPath} is missing at HEAD.`, { diagnostics: ['input-digest-drift'], details: { path: inputPath } });
        }
        const digest = digestWaveExitObserverInput(input.exitItemId, exitPolicy, inputPath, body);
        if (!digest) {
            throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_DIGEST_DRIFT', `Observed input ${inputPath} cannot produce its sealed digest projection.`, { diagnostics: ['input-digest-drift'], details: { path: inputPath } });
        }
    ...
    if (policySource == null) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_DIGEST_DRIFT', `Observed policy ${WAVE_EXIT_OBSERVER_POLICY_PATH} is missing at HEAD.`, { diagnostics: ['receipt-stale'], details: { path: WAVE_EXIT_OBSERVER_POLICY_PATH } });
    }
    ...
    if (input.claimedInputDigests) {
        for (const [path, digest] of Object.entries(input.claimedInputDigests)) {
            if (currentInputDigests[path] !== digest) {
                throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_DIGEST_DRIFT', 'Caller-supplied input digests do not match observed HEAD bytes.', { diagnostics: ['input-digest-drift'], details: { path, claimed: digest, observed: currentInputDigests[path] } });
            }
        }
    }
    const executed = input.executeApprovedCommand(exitPolicy.command);
    if (executed.exitCode !== 0) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_NONZERO_EXIT', `Sealed observer command exited ${executed.exitCode}.`, { diagnostics: ['nonzero-exit'], details: { exitCode: executed.exitCode, command: exitPolicy.command } });
    }
    const receipt = {
    ...
    if (hit === '--command' || input.claimedCommand) {
        if (hit === '--command' || (input.claimedCommand && input.claimedCommand.length > 0)) {
            ...
            if (!exitPolicy || input.claimedCommand !== exitPolicy.command || hit === '--command') {
                throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_UNAPPROVED_COMMAND', 'Caller-supplied observer commands are forbidden.', { diagnostics: ['unapproved-command'], details: { flag: hit ?? '--command' } });
            }
        }
    }
    if (hit && hit !== '--command') {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_CALLER_OVERRIDE', `Caller override ${hit} is forbidden for the official receipt writer.`, { details: { flag: hit } });
    }
}
function rejectForbiddenFlags(forbidden, extra) {
    for (const flag of forbidden) {
        const token = flag.split(/\s+/)[0] ?? flag;
        if (extra.includes(flag) || extra.includes(token)) {
            throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_FORBIDDEN_FLAG', `Flag ${flag} is forbidden for this sealed EXIT command.`, { diagnostics: ['unapproved-command'], details: { flag } });
        }
    }
}
function assertSafeCanonicalPath(repoRoot, policy, relativeArtifact, exitItemId) {
    const dir = normalizeRel(policy.canonicalReceiptDir);
    if (!dir || isAbsolute(dir) || dir.split('/').includes('..')) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_PATH_TRAVERSAL', 'Sealed canonical receipt directory is unsafe.', { diagnostics: ['artifact-path-mismatch'], details: { canonicalReceiptDir: policy.canonicalReceiptDir } });
    }
    ...
    if (normalizedArtifact !== baseArtifact && !successorPattern.test(normalizedArtifact)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_PATH_TRAVERSAL', 'Canonical receipt path does not match the sealed EXIT id.', { diagnostics: ['artifact-path-mismatch'], details: { relativeArtifact, baseArtifact } });
    }
    ...
    if (!isPathInside(root, dest) || !isPathInside(canonicalDir, dest)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_PATH_TRAVERSAL', 'Canonical receipt path escapes the sealed receipt directory.', { diagnostics: ['artifact-path-mismatch'], details: { dest, canonicalDir } });
    }
    const destRel = normalizeRel(relative(root, dest));
    if (FORBIDDEN_SURFACES.includes(destRel)) {
        throw new WaveExitObserverWriteError('ATM_WAVE_EXIT_OBSERVER_FORBIDDEN_SURFACE', 'Writer must not mutate certificate, completion evidence, or release verdict files.', { details: { destRel } });
    }
}
...
function writeErrorFromDiagnostics(diagnostics, details) {
