...
export function normalizeWritePath(value) {
    return value.replace(/\\/g, '/').replace(/^\.\//, '').trim();
}
export function normalizeWritePathList(values) {
    return uniqueSorted(values.map(normalizeWritePath).filter(Boolean));
}
export function computeWriteScopeDigest(allowedFiles) {
    return `sha256:${createHash('sha256').update(JSON.stringify(normalizeWritePathList(allowedFiles))).digest('hex')}`;
}
export function pathMatchesWriteScope(file, scope) {
...
export function isProtectedHistoryPath(file) {
    const normalized = normalizeWritePath(file);
    return normalized.startsWith('.atm/history/');
}
export function inspectWriteScopePolicy(input) {
...
function inspectIdentity(input) {
    ...
    const actorMismatch = Boolean((claimActorId && claimActorId !== input.actorId)
...
function isTicketStale(input) {
    if (input.ticketTaskId && input.ticketTaskId !== input.taskId)
        return true;
    if (input.ticketActorId && input.ticketActorId !== input.actorId)
        return true;
...
function buildRecoveryCommands(taskId, actorId, outOfScopeFiles) {
