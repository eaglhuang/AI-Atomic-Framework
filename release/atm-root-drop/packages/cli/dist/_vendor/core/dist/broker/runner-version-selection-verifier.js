import { createRegistryFromSnapshot, readRunnerRegistrySnapshotValue } from './runner-registry-snapshot.js';
import { selectRunnerVersion } from './runner-version-registry.js';
...
export function verifyRunnerSelection(input) {
    ...
    return {
        ...
        verdictCounts,
        metrics: {
            selectionAgeMs: summarizeAges(results.map((result) => result.selectionAgeMs).filter((value) => value !== null)),
...
export function verifyRunnerSelectionCase(verificationCase, generatedAt) {
    const findings = [];
    let rebuilt;
    try {
    ...
    if (rebuilt.outcome !== receiptSelection.outcome ||
    ...
    if (selectedCandidate && (!selectedCandidate.trusted ||
...
function classifyVerdict(input) {
    if (input.findings.some((finding) => finding.code === 'missing-contract-field'))
        return 'pending-contract';
    if (isRevalidationExpired(input.receipt, input.generatedAt))
        return 'revalidation-required';
    if (input.rebuiltOutcome === 'seal-revalidation-required' || input.rebuiltOutcome === 'no-candidate')
        return 'revalidation-required';
    if (input.findings.some((finding) => finding.code === 'qualification-gate-failed'))
        return 'revalidation-required';
    if (input.findings.some((finding) => finding.code === 'selection-mismatch' || finding.code === 'historical-snapshot-invalid'))
        return 'unqualified';
    return 'qualified';
}
function missingContractFields(receipt) {
    const missing = [];
    if (!receipt.policyVersion)
        missing.push('policyVersion');
    if (!receipt.registrySnapshotDigest)
        missing.push('registrySnapshotDigest');
    if (!receipt.selection.orderedCandidates)
        missing.push('selection.orderedCandidates');
    return missing.filter((field) => REQUIRED_RECEIPT_FIELDS.includes(field));
}
function isRevalidationExpired(receipt, generatedAt) {
    const boundary = receipt.selection.revalidationBoundaryGeneration;
    if (!boundary)
        return false;
    return String(boundary) < generatedAt;
}
function buildResult(verificationCase, verdict, selectedRunner, selectionOutcome, generatedAt, findings, coverageTags, latestVersionGap = 0) {
    return {
...
function latestVersionGap(versions, selectedSha) {
...
function countCoverage(results) {
    ...
    for (const result of results) {
        for (const tag of result.coverageTags)
            counts[tag] = (counts[tag] ?? 0) + 1;
    }
    return Object.fromEntries(Object.entries(counts).sort(([a], [b]) => a.localeCompare(b)));
}
function countPendingFieldGaps(results) {
    const counts = {};
    for (const result of results) {
        for (const finding of result.findings) {
            if (finding.code === 'missing-contract-field' && finding.field)
