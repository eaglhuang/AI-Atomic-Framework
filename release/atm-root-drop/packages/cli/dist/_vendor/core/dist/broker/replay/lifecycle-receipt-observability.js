...
export function deriveEventDerivedCorrectnessCounters(input) {
    ...
    const withoutDigest = {
        schemaId: 'atm.parallelReplayCorrectnessCounters.v1',
...
export function buildBrokerDecisionOutcomePair(input) {
    ...
    const withoutDigest = {
        schemaId: 'atm.brokerDecisionOutcomePair.v1',
...
export function classifyTelemetryCoverageState(node) {
    if (node.sealedReadBackCount > 0)
        return 'sealed-read-back';
    if (node.observedEventCount > 0)
        return 'observed';
    if (node.codeWired)
        return 'code-wired';
    return 'registered';
}
export function registryMembershipSatisfiesCoverage(node) {
    const state = classifyTelemetryCoverageState(node);
    return state === 'observed' || state === 'sealed-read-back';
}
export function evaluateTelemetryObligationSeal(input) {
    ...
    const verdict = hasSeal
        ...
        : hasUnavailable
            ...
            : 'incomplete';
    const withoutDigest = {
        schemaId: 'atm.parallelReplayTelemetryObligationSeal.v1',
        taskId: input.taskId,
        sealed,
        verdict: verdict,
 ...
 */
export function mergeAppendOnlyEvidenceWrites(existing, incoming) {
    const byId = new Map();
    ...
    if (!preserved)
        lostUpdateCount = expectedMin - preservedDigests.size;
    return {
