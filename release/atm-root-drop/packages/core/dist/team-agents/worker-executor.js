...
export function buildTeamWorkerExecutionRuntime(input) {
    ...
    const sourceAvailability = Object.fromEntries(input.manifest.tasks.map((task) => [task.taskId, reportsByTask.has(task.taskId) ? 'available' : 'unavailable']));
    const lanes = input.manifest.tasks.map((task, index) => {
        const report = reportsByTask.get(task.taskId);
        const lifecycleEvents = ['worker.start', 'worker.heartbeat'];
        if (report)
            lifecycleEvents.push('worker.report');
        if (deferredTaskIds.includes(task.taskId))
            lifecycleEvents.push('worker.defer', 'worker.retry');
        if (invalidTaskIds.has(task.taskId) || outOfScopeTaskIds.has(task.taskId))
            lifecycleEvents.push('worker.needs-review');
        return {
            taskId: task.taskId,
...
function resolveResultState(input) {
    if (input.invalidWorkerReports.length > 0 || input.outOfScopeFindings.length > 0)
        return 'needs-review';
    if (input.reportCount === 0)
        return 'executing';
    if (input.acceptedTaskIds.length === input.totalTasks && input.totalTasks > 0)
        return 'ready-for-write';
    if (input.deferredTaskIds.length > 0 || input.acceptedTaskIds.length < input.totalTasks)
        return 'serial-fallback';
    if (input.missingWorkerReports.length > 0)
        return 'executing';
    return 'executing';
}
