...
import { PARALLEL_REPLAY_LIFECYCLE_STEPS, STEP_PURPOSE_HINTS, WEAK_OR_UNRELATED_COMMAND_PATTERNS } from './lifecycle-receipt-types.js';
import { finishValidation } from './lifecycle-receipt-validation.js';
export * from './lifecycle-receipt-types.js';
export * from './lifecycle-receipt-observability.js';
export { finishValidation } from './lifecycle-receipt-validation.js';
export function isParallelReplayLifecycleStep(value) {
    return typeof value === 'string' && PARALLEL_REPLAY_LIFECYCLE_STEPS.includes(value);
}
export function buildSemanticLifecycleReceipt(input) {
    const withoutDigest = {
...
export function validateSemanticLifecycleReceipt(receipt, expected) {
    const reasons = [];
    const invariantCodes = [];
    ...
    if (typeof value.step === 'string' && typeof value.commandPurpose === 'string') {
        if (!commandPurposeMatchesStep(value.step, value.commandPurpose)) {
    ...
    if (expected?.step && value.step !== expected.step)
        reasons.push('step-does-not-match-expected');
    if (expected?.taskId && value.taskId !== expected.taskId)
        reasons.push('taskId-does-not-match-expected');
    if (expected?.actorId && value.actorId !== expected.actorId)
        reasons.push('actorId-does-not-match-expected');
...
export function validateSameFileIntentEvidence(evidence) {
    const reasons = [];
    const invariantCodes = [];
    const hasAnchor = evidence.atomOrContentAnchors.length > 0 || evidence.boundedSourceRanges.length > 0;
    if (!hasAnchor)
        reasons.push('missing-atom-or-bounded-source-anchor');
    if (!evidence.adapterIdentity || !evidence.adapterDecision)
        reasons.push('missing-adapter-identity-or-decision');
    ...
    if (!evidence.serializabilityProofDigest)
        reasons.push('missing-serializability-proof');
    if (!evidence.stewardBeforeHash || !evidence.stewardAfterHash)
        reasons.push('missing-steward-before-after-hashes');
    if (evidence.sharedCommitMemberAttribution.length === 0)
        reasons.push('missing-shared-commit-member-attribution');
...
export function validatePostComposeSemanticEvidence(evidence) {
    const reasons = [];
    const invariantCodes = [];
    ...
    if (evidence.validatorReferences.length === 0)
        reasons.push('missing-validator-references');
    if (!evidence.sealedSelectionSourceDigest)
        reasons.push('missing-sealed-selection-source');
    if (!evidence.executable)
        reasons.push('missing-executable');
    if (!evidence.cwd)
        reasons.push('missing-cwd');
    if (!evidence.runnerOrBuildDigest)
        reasons.push('missing-runner-or-build-digest');
    ...
    if (evidence.derivedResult === 'fail')
        reasons.push('validation-failed');
    if (evidence.derivedResult === 'inconclusive')
        reasons.push('validation-inconclusive');
...
export function evaluateComposeQueueResidency(input) {
    ...
    if (input.disposition === 'compose-selected') {
        if (input.waitedMs < 0)
            reasons.push('negative-waitedMs');
...
function isValidTimeWindow(value) {
...
function isUnrelatedOrWeakCommand(command) {
    const normalized = command.trim();
    if (!normalized)
        return true;
    return WEAK_OR_UNRELATED_COMMAND_PATTERNS.some((pattern) => pattern.test(normalized));
}
function commandPurposeMatchesStep(step, purpose) {
