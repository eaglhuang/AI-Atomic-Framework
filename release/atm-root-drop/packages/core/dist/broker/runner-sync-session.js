...
export function startRunnerSyncSession(request, ports) {
    ...
    return decide('build', true, null, state, [], null, `Session started for ${manifest.memberTaskIds.length} coalesced member(s); run one build for ${request.sealedSourceSha} then record it.`);
}
/**
