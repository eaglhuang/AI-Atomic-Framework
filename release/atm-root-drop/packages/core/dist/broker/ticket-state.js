...
export function assertBrokerTicketCanExecute(ticket) {
...
function buildTransition(ticket, to, actorId, reason, idempotencyKey, now) {
...
function terminalReasonFor(state) {
    if (state === 'released')
        return 'released';
    if (state === 'cancelled')
        return 'cancelled';
