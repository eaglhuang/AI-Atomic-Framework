...
import { pathToAtomMapAdapter } from '../adapters/atom-map.js';
...
function makeFile(content = makeShard(), filePath = SHARD_PATH) {
...
function testSupportsMatchingRule() {
    ...
    assert.equal(pathToAtomMapAdapter.supports(makeFile(makeShard(), SHARD_PATH.replace(/\//g, '\\'))), true);
    // The projection itself and manifest are NOT supported.
    assert.equal(pathToAtomMapAdapter.supports(makeFile(makeShard(), 'atomic_workbench/atomization-coverage/path-to-atom-map.json')), false);
    assert.equal(pathToAtomMapAdapter.supports(makeFile(makeShard(), 'atomic_workbench/atomization-coverage/path-to-atom-map-shards/manifest.json')), false);
...
function testDifferentRowsMergeable() {
    ...
    const m1 = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r1', op: 'replace', target: 'packages/core/src/a.ts::ATOM-A', value: { path_pattern: 'packages/core/src/a.ts', atom_id: 'ATOM-A', capability: 'cap-a2', coverage_status: 'covered' } }));
    const m2 = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r2', op: 'replace', target: 'packages/core/src/b.ts::ATOM-B', value: { path_pattern: 'packages/core/src/b.ts', atom_id: 'ATOM-B', capability: 'cap-b2', coverage_status: 'covered' } }));
    const decision = pathToAtomMapAdapter.canMerge([m1, m2], parsed);
    assert.equal(decision.verdict, 'mergeable');
    const merged = pathToAtomMapAdapter.merge([m1, m2], parsed);
...
function testSameRowConflict() {
    const parsed = pathToAtomMapAdapter.parse(makeFile());
    const m1 = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r1', op: 'replace', target: 'packages/core/src/a.ts::ATOM-A', value: { path_pattern: 'packages/core/src/a.ts', atom_id: 'ATOM-A', capability: 'x', coverage_status: 'covered' } }));
    const m2 = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r2', op: 'replace', target: 'packages/core/src/a.ts::ATOM-A', value: { path_pattern: 'packages/core/src/a.ts', atom_id: 'ATOM-A', capability: 'y', coverage_status: 'covered' } }));
    const decision = pathToAtomMapAdapter.canMerge([m1, m2], parsed);
...
function testMetadataChangeWidensConflictKey() {
    const parsed = pathToAtomMapAdapter.parse(makeFile());
    const rowMutation = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r1', op: 'replace', target: 'packages/core/src/a.ts::ATOM-A', value: { path_pattern: 'packages/core/src/a.ts', atom_id: 'ATOM-A', capability: 'z', coverage_status: 'covered' } }));
    const metaMutation = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r2', op: 'upsert', target: 'version', value: '2.0' }));
    ...
    const metaMutation2 = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r3', op: 'upsert', target: 'summary', value: { mapped_paths: 2 } }));
...
function testSerializeRoundTrip() {
    const parsed = pathToAtomMapAdapter.parse(makeFile());
    const upsert = pathToAtomMapAdapter.normalize(makeRequest({ requestId: 'r1', op: 'upsert', target: 'packages/core/src/c.ts::ATOM-C', value: { path_pattern: 'packages/core/src/c.ts', atom_id: 'ATOM-C', capability: 'cap-c', coverage_status: 'planned' } }));
