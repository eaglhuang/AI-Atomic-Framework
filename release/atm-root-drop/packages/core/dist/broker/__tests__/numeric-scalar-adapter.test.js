...
import { numericScalarAdapter } from '../adapters/numeric-scalar.js';
import { brokerAdapterMigration } from '../types.js';
function makeFile(content, filePath = 'metrics.scalars.json') {
...
function testTwoIncrementsCommutativeMerge() {
    ...
    assert.equal(merged.value.values.hits, 12);
...
function testIncrementDecrementNetDelta() {
    ...
    assert.equal(merged.value.values.bal, 120);
...
function testMaxMinBehavior() {
    ...
    assert.equal(merged.value.values.peak, 70);
    ...
    assert.equal(mergedMin.value.values.low, 30);
...
function testSetIfCurrentApplies() {
    ...
    assert.equal(merged.value.values.v, 99);
