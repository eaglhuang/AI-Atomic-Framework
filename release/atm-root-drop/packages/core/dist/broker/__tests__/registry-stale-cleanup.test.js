...
import { buildBlockingRegistryFindingEvidence, cleanupStaleWithEvidence, describeBlockingRegistryIntent, loadRegistry, resolveConflictBlockingIntent, saveRegistry } from '../registry.js';
...
function makeActiveIntent(overrides = {}) {
    const now = Date.now();
    return {
...
function testLoadRegistryPersistCleanupOption() {
    ...
    const rawAfterNoPersist = JSON.parse(readFileSync(registryPath, 'utf8'));
    ...
    const rawAfterPersist = JSON.parse(readFileSync(registryPath, 'utf8'));
