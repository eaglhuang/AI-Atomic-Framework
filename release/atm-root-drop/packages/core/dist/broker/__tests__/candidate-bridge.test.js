...
import { candidatesToWriteIntent, computeCandidateAtomCid } from '../candidate-bridge.js';
import { calculateBrokerDecision } from '../decision.js';
import { GOVERNANCE_BACKLOG_PROJECTION, RELEASE_MIRROR_ARTIFACT, RUNNER_SYNC_STEWARD_GENERATOR } from '../global-resource-projection.js';
...
function testParallelSafeScenario() {
    ...
    const intentB = candidatesToWriteIntent([makeCandidate({ symbol: 'RowStore', kind: 'class', filePath: 'src/row-store.ts', suggestedAtomId: 'ATM-JS-cafef00d', suggestedSourcePaths: undefined })], { taskId: 'TASK-B', actorId: 'agent-b', baseCommit: 'abc123' });
...
function testFileOverlapScenario() {
    ...
    const intentA = candidatesToWriteIntent([makeCandidate({ symbol: 'readSection', filePath: sharedFile, suggestedAtomId: 'ATM-JS-aaaa0001', suggestedSourcePaths: undefined })], baseContext);
    const intentB = candidatesToWriteIntent([makeCandidate({ symbol: 'writeSection', filePath: sharedFile, suggestedAtomId: 'ATM-JS-bbbb0002', suggestedSourcePaths: undefined })], { taskId: 'TASK-B', actorId: 'agent-b', baseCommit: 'abc123' });
    const decision = calculateBrokerDecision(intentB, registryWith([toActiveIntent(intentA, 'intent-a')]));
