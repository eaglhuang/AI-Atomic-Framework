...
import { shouldTriggerLayer2 } from '../policy.js';
function makeWriteIntent(overrides = {}) {
    return {
...
function registryWithActive(conflictRange) {
    const activeIntent = {
        intentId: 'intent-a',
        taskId: 'TASK-A',
        teamRunId: null,
...
function makeLayer2ConflictRegion(overlap) {
    return {
        leftAtom: {
            atomId: 'left',
            atomCid: 'left-cid',
...
function testShouldTriggerLayer2() {
    ...
    assert.equal(result.targetFunction.atomId, 'left');
...
function testDecisionEmitsDecompositionRequest() {
    ...
    if (!decision.decompositionRequest)
        return;
