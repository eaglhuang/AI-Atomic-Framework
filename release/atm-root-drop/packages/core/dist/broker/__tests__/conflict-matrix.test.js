...
function asActive(intent, taskId, overrides = {}) {
    return {
...
function testReadSetConflict() {
    const activeIntent = asActive(makeIntent({ taskId: 'TASK-B', actorId: 'actor-b', atomRefs: [{ atomId: 'atom-b', atomCid: 'cid-b', operation: 'modify' }] }), 'TASK-B');
    const newIntent = makeIntent({
        taskId: 'TASK-C',
        actorId: 'actor-c',
...
function testActiveReadSetConflict() {
    const activeIntent = asActive(makeIntent({
        taskId: 'TASK-B',
        actorId: 'actor-b',
...
function testTakeoverForMalformedIntent() {
    const newIntent = makeIntent({ atomRefs: [], targetFiles: [] });
    const matrix = evaluateConflictMatrix(newIntent, []);
...
function testTakeoverForStaleLease() {
    ...
    }), 'TASK-B', { expiresAt: '2000-01-01T00:00:00.000Z' });
