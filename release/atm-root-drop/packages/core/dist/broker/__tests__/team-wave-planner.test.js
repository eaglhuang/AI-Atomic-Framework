...
import { planWaves, pairBlockReasons } from '../team-wave-planner.js';
function card(over) {
    return {
        taskId: over.taskId,
...
function testSameDeliverableSplitsAsWriteWrite() {
    const reasons = pairBlockReasons(card({ taskId: 'T-A', scopePaths: ['src/shared.ts'], deliverables: ['src/shared.ts'] }), card({ taskId: 'T-B', scopePaths: ['src/shared.ts'], deliverables: ['src/shared.ts'] }), new Set());
    assert.ok(reasons.includes('same-atom-write-write'));
}
function testUnknownScopeOverlapFailsClosed() {
    const reasons = pairBlockReasons(card({ taskId: 'T-A', scopePaths: ['src/dir/'], deliverables: ['src/dir/a.ts'] }), card({ taskId: 'T-B', scopePaths: ['src/dir/b.ts'], deliverables: ['src/dir/b.ts'] }), new Set());
    assert.ok(reasons.includes('scope-overlap-unknown-range'));
}
function testClosureAuthorityMismatchBlocks() {
    const reasons = pairBlockReasons(card({ taskId: 'T-A', closureAuthority: 'target_repo' }), card({ taskId: 'T-B', closureAuthority: 'planning_repo' }), new Set());
    assert.ok(reasons.includes('closure-authority-mismatch'));
}
function testTargetRepoMismatchBlocks() {
    const reasons = pairBlockReasons(card({ taskId: 'T-A', targetRepo: 'repo-x' }), card({ taskId: 'T-B', targetRepo: 'repo-y' }), new Set());
    assert.ok(reasons.includes('target-repo-mismatch'));
}
function testGeneratedArtifactContentionBlocks() {
    const reasons = pairBlockReasons(card({ taskId: 'T-A', ownerAtomOrMap: 'atm.shared-map' }), card({ taskId: 'T-B', ownerAtomOrMap: 'atm.shared-map' }), new Set());
...
function testAppendSafeOverlapDoesNotBlock() {
    const reasons = pairBlockReasons(card({ taskId: 'T-A', scopePaths: ['map.json', 'src/a.ts'], deliverables: ['src/a.ts'] }), card({ taskId: 'T-B', scopePaths: ['map.json', 'src/b.ts'], deliverables: ['src/b.ts'] }), new Set(['map.json']));
