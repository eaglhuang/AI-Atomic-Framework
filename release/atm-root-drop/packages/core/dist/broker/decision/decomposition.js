...
export function toVirtualAtomRangesFromActiveIntent(intent) {
...
export function buildLayer2ConflictDetail(region) {
    return {
        kind: 'file-range',
        detail: `Layer2 overlap detected on '${region.filePath}' in lines [${region.lineStart}-${region.lineEnd}]`
    };
}
export function buildDecompositionRequest(targetFunction, conflictRegion, options = {}) {
    return {
        targetFunction,
        conflictRegion,
...
export function maybeBuildCidConflictDecompositionRequest(newIntent, activeIntents) {
...
export function buildSuggestedSplitAtoms(targetFunction, conflictRegion, containerRangeOverride) {
    const suggestions = [];
...
export function toSuggestedAtomCid(atomId, role, range) {
