...
export function classifyGuidanceIntent(goal, options = {}) {
...
export function loadHostIntentLexicon(repositoryRoot) {
...
export function recordGuidanceIntentPhrase(options) {
...
export function hostIntentLexiconPath(repositoryRoot) {
...
function normalizeLexiconEntry(entry) {
    ...
    if (typeof candidate.phrase !== 'string')
        return [];
    if (candidate.intent !== 'legacy-atomization' && candidate.intent !== 'legacy-candidate-ranking')
        return [];
...
function isActiveLexiconEntry(entry) {
    return entry.status === 'active-host' || entry.status === 'promoted-framework';
}
function matchTerms(normalizedGoal, terms) {
...
function confidenceFromMatches(base, matchCount) {
...
function assertPromotableFrameworkPhrase(phrase) {
    const normalizedPhrase = normalizeIntentPhrase(phrase);
