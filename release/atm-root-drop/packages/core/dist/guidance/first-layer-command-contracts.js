...
export const firstLayerRouteMatrix = Object.freeze([
    {
        intent: 'backlog',
        promptFixtures: [
...
export const firstLayerTicketStates = Object.freeze([
    {
        state: 'execute-now',
        errorCode: null,
...
export function classifyFirstLayerIntent(goal) {
    const normalized = normalizeFirstLayerText(goal);
    return firstLayerRouteMatrix.find((row) => row.promptFixtures.some((fixture) => normalizeFirstLayerText(fixture) === normalized)
        || (hasFirstLayerActionVerb(normalized, row.intent)
            && row.matchedTerms.some((term) => normalized.includes(normalizeFirstLayerText(term))))) ?? null;
}
...
function normalizeFirstLayerText(value) {
    return value
        .normalize('NFKC')
        .toLowerCase()
...
function hasFirstLayerActionVerb(normalized, intent) {
    const actionTerms = {
...
function matchesFirstLayerTerm(normalized, term) {
