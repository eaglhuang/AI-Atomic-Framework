...
export function computeMapCid(bundle) {
...
export function exportMapCapsule(bundle) {
...
export function importMapCapsule(mapCid, compressedPayload, options = {}) {
    validateMapCidFormat(mapCid);
    // L1: Hash verify
    if (!verifyMapPayloadHash(mapCid, compressedPayload)) {
        throw new MapCapsuleError('ATM_MAP_CAPSULE_HASH_MISMATCH', `map:cid hash verification failed for ${mapCid}. The payload may be corrupted.`, { mapCid });
    }
    // L2: Decompress
    let bundle;
    try {
        ...
        throw new MapCapsuleError('ATM_MAP_CAPSULE_DECOMPRESS_FAILED', `Failed to decompress map capsule for ${mapCid}.`, { mapCid, cause: String(err) });
    }
...
export function convertSpecToMapBundle(mapSpec, atomCidMap // atomId -> atomCid
) {
    const members = mapSpec.members.map((m) => {
        const atomCid = atomCidMap[m.atomId];
        if (!atomCid) {
            throw new MapCapsuleError('ATM_MAP_CAPSULE_MISSING_ATOM_CID', `No CID found for atom ${m.atomId}. Export or import the atom capsule first.`, { atomId: m.atomId });
        }
        return { atomCid, role: m.atomId };
    });
    const edges = mapSpec.edges.map((e) => ({
...
export function verifyMapPayloadHash(mapCid, compressedPayload) {
    try {
        const compressed = Buffer.from(compressedPayload, 'base64');
        const hash = createHash('sha256').update(compressed).digest('base64url');
...
export function validateMapCidFormat(mapCid) {
    if (!mapCid.startsWith(MAP_CID_PREFIX)) {
        throw new MapCapsuleError('ATM_MAP_CAPSULE_INVALID_CID', `Invalid map:cid format. Must start with "${MAP_CID_PREFIX}". Got: ${mapCid}`, { mapCid });
    }
...
export function mapCidToShortId(mapCid) {
