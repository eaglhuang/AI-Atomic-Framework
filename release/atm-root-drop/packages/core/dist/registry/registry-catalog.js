...
import { deriveRegistryCatalogCategory } from './catalog-category-deriver.js';
...
export function renderRegistryCatalogMarkdown(registryDocument, options = {}) {
    ...
    if (projection.atoms.length > 0) {
        lines.push('', '## Atoms', '', '| atomId | logicalName | function | derivedCategory | provenance | status | specPath |', '| --- | --- | --- | --- | --- | --- | --- |');
        for (const row of projection.atoms) {
            lines.push(`| \`${escapeMarkdownCell(row.entryId)}\` | \`${escapeMarkdownCell(row.logicalName || '??')}\` | ${escapeMarkdownCell(row.functionSummary)} | \`${escapeMarkdownCell(row.derivedCategory)}\` | \`${escapeMarkdownCell(row.provenance)}\` | \`${escapeMarkdownCell(row.status)}\` | \`${escapeMarkdownCell(row.specPath)}\` |`);
        }
    }
    if (projection.maps.length > 0) {
        lines.push('', '## Maps', '', '| mapId | memberCount | status | workbenchPath | notes |', '| --- | --- | --- | --- | --- |');
        for (const row of projection.maps) {
            lines.push(`| \`${escapeMarkdownCell(row.mapId)}\` | \`${escapeMarkdownCell(String(row.memberCount))}\` | \`${escapeMarkdownCell(row.status)}\` | \`${escapeMarkdownCell(row.workbenchPath)}\` | ${escapeMarkdownCell(row.notes)} |`);
        }
    }
    return `${lines.join('\n')}\n`;
}
