 ...
 */
export function describeShardUnreachability(raw, sourcePath) {
    if (normalizeGroupShard(raw, sourcePath) !== null)
        return null;
    ...
    const missing = ['groupId', 'theme', 'resourceKey'].filter((field) => !String(raw[field] ?? '').trim());
    if (missing.length > 0)
...
export function loadTestCaseGroupShards(repositoryRoot, groupsRoot) {
    ...
    if (unreachable.length > 0) {
        throw new Error(`ATM_TEST_CASE_SHARD_UNREACHABLE: ${unreachable.length} catalog shard(s) cannot be loaded and would be skipped silently. `
...
export function reportShardReachability(repositoryRoot, groupsRoot) {
    return readRawShardFiles(repositoryRoot, groupsRoot)
        ...
        let unreachableReason = 'shard could not be inspected';
        try {
...
function normalizeGroupShard(raw, sourcePath) {
    ...
    if (String(raw.schemaId ?? '').trim() !== CASE_GROUP_SHARD_SCHEMA_ID)
        return null;
    const groupId = String(raw.groupId ?? '').trim();
