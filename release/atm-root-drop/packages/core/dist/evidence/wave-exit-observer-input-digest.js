...
export function digestWaveExitObserverInput(exitItemId, exitPolicy, inputPath, source) {
    ...
    if (exitPolicy.inputDigestProjection === 'objective-audit-excluding-derived-conclusion') {
        try {
            ...
            // closeout projection.
            const { status: _derivedCertificateStatus, releasePushProvenance: _derivedReleaseProvenance, independentReview: _derivedIndependentReview, legacyAuthority: _derivedLegacyAuthority, unknownRows: _derivedUnknownRows, unresolvedRows: _derivedUnresolvedRows, supersession: _derivedSupersession, resultDigest: _derivedCertificateDigest, ...semanticAudit } = audit;
            return digestText(`${inputPath.replace(/\\/g, '/')}\n${JSON.stringify({
                ...semanticAudit,
    ...
    if (exitPolicy.inputDigestProjection !== 'completion-report-excluding-current-exit')
        return digestText(source);
    try {
        const report = JSON.parse(source);
        if (!Array.isArray(report.waveExits))
            return null;
        ...
        const authority = report.authority && typeof report.authority === 'object'
            ? { ...report.authority, targetHead: '<compiler-target-head>', originMain: '<compiler-origin-main>' }
            : report.authority;
        const validatorContracts = Array.isArray(report.validatorContracts)
            ? report.validatorContracts.map((entry) => {
                if (!entry || typeof entry !== 'object')
                    return entry;
                const { policyDigest, ...contract } = entry;
                return contract;
            })
...
export function digestWaveExitObserverInputsAtCommit(repoRoot, exitItemId, exitPolicy, commit) {
    const digests = {};
