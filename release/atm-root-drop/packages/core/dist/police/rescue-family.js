...
function checkRegistrySourceFiles(repositoryRoot) {
    ...
    try {
        ...
        if (missing.length > 0) {
            return blocker('INV-RESCUE-001', `${missing.length} atom(s) in atomic-registry.json have missing source files.`, undefined, `node atm.mjs rescue rebuild-registry --json`);
        }
...
function checkCapsuleRegistryIntegrity(repositoryRoot) {
    ...
    if (corrupted.length > 0) {
        return blocker('INV-RESCUE-002', `${corrupted.length} capsule(s) failed hash verification: ${corrupted.slice(0, 3).join(', ')}`, registryPath, 'node atm.mjs atom-capsule import --cid <cid> --payload <payload> --json');
    }
...
function checkMapRegistryIntegrity(repositoryRoot) {
    ...
    if (missing.length > 0) {
        return blocker('INV-RESCUE-003', `Map Merkle tree broken: ${missing.length} missing atom CID(s).`, mapRegPath, 'node atm.mjs map-capsule import --cid <map:cid> --payload <payload> --json');
    }
    return pass('INV-RESCUE-003', 'Map Merkle tree intact.');
}
// INV-RESCUE-004: Lineage log timestamps are strictly monotonic
function checkLineageLogMonotonicity(repositoryRoot) {
    const mapsDir = path.join(repositoryRoot, 'atomic_workbench', 'maps');
    if (!existsSync(mapsDir))
        return pass('INV-RESCUE-004', 'No maps directory found.');
    const violations = [];
    ...
    if (violations.length > 0) {
        return warn('INV-RESCUE-004', `${violations.length} lineage-log(s) have non-monotonic timestamps.`, mapsDir, 'Manually review and correct the affected lineage-log.json files.');
    }
...
function checkBindingSchemaRegistry(repositoryRoot) {
    ...
    try {
        ...
        if (invalid.length > 0) {
            return blocker('INV-RESCUE-005', `${invalid.length} binding schema(s) are invalid.`, schemaRegPath, 'Repair the affected binding schemas in binding-schema-registry.json.');
        }
...
function checkPolicyJsonSchema(repositoryRoot) {
...
function checkVendorAtomConsistency(repositoryRoot) {
    ...
    const registry = loadCapsuleRegistry(registryPath);
    const registeredShortIds = new Set(Object.keys(registry.entries).map((cid) => cid.replace('atom:cid:', '').slice(0, 16)));
    const orphanFiles = [];
    for (const filename of readdirSync(vendorDir)) {
        if (filename === 'capsule-registry.json')
            continue;
    ...
    if (orphanFiles.length > 0) {
        return warn('INV-RESCUE-007', `${orphanFiles.length} orphan file(s) in vendor/atoms/ not tracked in capsule registry.`, vendorDir, 'Run node atm.mjs rescue sync-vendor --json to reconcile.');
    }
...
function checkGuideCacheCommits(_repositoryRoot) {
    return skipped('INV-RESCUE-008', 'INV-RESCUE-008 skipped: Guide Cache (M24) is not yet implemented. This invariant will be activated when M24 is deployed.');
}
// INV-RESCUE-009: Daemon PID file points to a live ATM daemon process
// TODO (M22 dependency): Daemon mode is not yet implemented
function checkDaemonPid(_repositoryRoot) {
    return skipped('INV-RESCUE-009', 'INV-RESCUE-009 skipped: Daemon Mode (M22) is not yet implemented. This invariant will be activated when M22 is deployed.');
}
// INV-RESCUE-010: All evidence JSON files pass schema validation
function checkEvidenceJsonSchema(repositoryRoot) {
    const evidenceDirs = [
    ...
    if (invalid.length > 0) {
        return warn('INV-RESCUE-010', `${invalid.length} evidence JSON file(s) failed basic schema check.`, evidenceDirs[0], 'Inspect and repair the affected evidence files or remove invalid entries.');
    }
    return pass('INV-RESCUE-010', 'All evidence JSON files pass basic schema validation.');
}
// Helpers
function pass(invariantId, description) {
    return {
        policeFamily: 'rescue',
        invariantId,
        severity: 'info',
...
function blocker(invariantId, description, affectedFile, recoveryHint = '') {
    return {
        policeFamily: 'rescue',
        invariantId,
        severity: 'blocker',
...
function warn(invariantId, description, affectedFile, recoveryHint = '') {
    return {
        policeFamily: 'rescue',
        invariantId,
        severity: 'warning',
...
function skipped(invariantId, skippedReason) {
    return {
        policeFamily: 'rescue',
        invariantId,
        severity: 'info',
