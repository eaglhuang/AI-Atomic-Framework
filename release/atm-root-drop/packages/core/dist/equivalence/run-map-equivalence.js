...
import { resolveCanonicalMapPaths, resolveMapIntegrationTarget } from '../test-runner/map-integration.js';
...
export async function runMapEquivalence(mapId, fixturePath, options = {}) {
...
async function invokeExecutor(executor, input, context, fieldName, caseId) {
    try {
...
function normalizeFixtureCases(value) {
...
function normalizeKnownDivergences(value) {
    if (value == null) {
        return [];
    }
...
function normalizeEvidenceRefs(value, caseId) {
    if (!Array.isArray(value) || value.length === 0) {
...
function createReportId(mapId, fixtureSetId) {
...
function createMapEquivalenceError(code, message, details) {
