import type { ConflictDetail, MergePlan, PatchProposal } from './types.ts';
import {
  anchorKey,
  buildMergePlan,
...
function detectCidConflicts(proposals: readonly PatchProposal[]): ConflictDetail[] {
  const conflicts: ConflictDetail[] = [];
  const atomIdOwners = new Map<string, string>();
  const atomCidOwners = new Map<string, string>();
...
function detectMetadataMismatches(proposals: readonly PatchProposal[]): ConflictDetail[] {
  const conflicts: ConflictDetail[] = [];
...
function detectAnchorOverlaps(proposals: readonly PatchProposal[]): ConflictDetail[] {
  const conflicts: ConflictDetail[] = [];
  const groups = groupByTargetFile(proposals);
...
function detectPatchRangeOverlaps(proposals: readonly PatchProposal[]): ConflictDetail[] {
  const conflicts: ConflictDetail[] = [];
  const groups = groupByTargetFile(proposals);
...
function groupByTargetFile(proposals: readonly PatchProposal[]): Map<string, PatchProposal[]> {
...
function dedupeConflicts(conflicts: readonly ConflictDetail[]): ConflictDetail[] {
  const seen = new Set<string>();
  const unique: ConflictDetail[] = [];
