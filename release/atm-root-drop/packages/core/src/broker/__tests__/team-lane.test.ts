...
function testBoundedRegionProducesStableSyntheticAtomCid() {
  ...
  assert.equal(
    result.evidence.writeIntent.atomRefs[0]?.atomCid,
    'atm-broker-classify-explicit-mutation-request-broker-841-878'
  );
  rmSync(tempDir, { recursive: true, force: true });
...
function testRearbitrationProjectionCapturesEffectiveDecision() {
  ...
  const registry = registerIntent(
    registerIntent(
      ...
      first.evidence.writeIntent,
      first.evidence.decision.lane,
      1800,
      first.evidence.decision.admission
    ),
    second.evidence.writeIntent,
    second.evidence.decision.lane,
    1800,
    second.evidence.decision.admission
  );
