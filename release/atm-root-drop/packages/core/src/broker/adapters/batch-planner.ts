...
import { type AdapterRegistry, resolveAdapter } from './registry.ts';

/**
 * Deterministic plan id: sha256 over the sorted request ids, mirroring
 * merge-plan.ts buildDeterministicMergePlanId.
 */
export function buildDeterministicPlanId(requestIds: readonly string[]): string {
...
function fileDescriptorFor(request: MutationRequest): FileDescriptor {
  ...
  return { filePath: request.filePath, content: (request as { content?: string }).content ?? '{}' };
}
...
export function planMutationBatch(input: {
  ...
  for (const request of input.requests) {
    ...
    const file: FileDescriptor = content !== undefined
  ...
  const byFile = new Map<string, PlannedRequest[]>();
  ...
  for (const filePath of sortedFiles) {
    const entries = [...byFile.get(filePath)!].sort((left, right) =>
