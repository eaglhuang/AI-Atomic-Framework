 ...
 */
export function resolveShippedSchemaPath(moduleUrl: string, relativePath: string, fallbackRoot: string): string {
  let current = path.dirname(fileURLToPath(moduleUrl));
  while (true) {
    const candidate = path.join(current, 'schemas', relativePath);
    if (existsSync(candidate)) return candidate;
    const parent = path.dirname(current);
    if (parent === current) return path.join(fallbackRoot, 'schemas', relativePath);
    current = parent;
  }
}
