...
import type {
  AtomicSpecDocument,
  AtomicSpecPortRecord,
  FailureOptions,
  JsonReadFailure,
  JsonReadSuccess,
  NormalizedAtomicSpecModel,
  ParseAtomicSpecFailure,
  ParseAtomicSpecOptions,
  ParseAtomicSpecSuccess,
  PromptIssue
} from './parse-spec/types.ts';
...
export function resolveAtomicSpecSchemaPath(startDirectory = path.dirname(fileURLToPath(import.meta.url))): string {
  ...
  while (true) {
    ...
    if (parent === current) return fallbackPath;
    current = parent;
  }
}
