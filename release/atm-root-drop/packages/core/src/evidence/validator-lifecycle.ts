...
export const VALIDATOR_LIFECYCLE_SUMMARY_SCHEMA_ID = 'atm.validatorLifecycleSummary.v1' as const;

export type ValidatorLifecycleEventKind =
  | 'eligible'
  | 'invoked'
  | 'skipped'
  | 'cache-hit'
...
export interface ValidatorLifecycleEvent {
  readonly schemaId: 'atm.validatorLifecycleEvent.v1';
  readonly validatorId: string;
  readonly validatorVersion: string | null;
  readonly tier: 'fast' | 'default' | 'full' | 'archive-candidate';
  readonly event: ValidatorLifecycleEventKind;
  readonly durationMs: number;
  readonly cacheDecision: string;
  readonly fanOutConsumerCount: number;
  readonly blockingCount: number;
  readonly reusedFromReceipt: boolean;
  readonly usedForDecision: boolean;
}

export interface ValidatorLifecycleSummary {
  readonly schemaId: typeof VALIDATOR_LIFECYCLE_SUMMARY_SCHEMA_ID;
  readonly profile: string;
  readonly mode: string;
  readonly optimizationId: 'ATM-GOV-0200';
  readonly sourceTaskId: 'ATM-GOV-0200';
  readonly dataDrivenDecision: 'sufficient-observation' | 'insufficient-observation';
  ...
  readonly observedWindow: {
    ...
    readonly fanOutEventCount: number;
    readonly blockedCount: number;
    readonly durationMs: number;
  };
  readonly tierProposal: {
    readonly fast: readonly string[];
    readonly default: readonly string[];
    readonly full: readonly string[];
    readonly archiveCandidate: readonly string[];
    readonly insufficientObservation: readonly string[];
  };
  readonly rollbackReceipt: {
    readonly schemaId: 'atm.validatorTierRollbackReceipt.v1';
    readonly optimizationId: 'ATM-GOV-0200';
    readonly restoresConfigDigest: string;
    readonly restoresOrderingDigest: string;
    readonly invalidatesTreatmentCache: true;
    readonly parityValidator: string;
  };
  readonly consumedReceipt: {
    readonly schemaId: 'atm.validatorLifecycleConsumedReceipt.v1';
    readonly producerTaskId: 'ATM-GOV-0196' | 'ATM-GOV-0200';
    readonly historyDigest: string;
    readonly configDigest: string;
    readonly consumedBy: 'ATM-GOV-0200' | 'ATM-GOV-0202';
  };
  readonly events: readonly ValidatorLifecycleEvent[];
}
...
export function buildValidatorLifecycleSummary(input: {
  ...
  readonly usageTelemetry?: Record<string, unknown> | null;
...
function lifecycleEventsForValidator(
  validatorId: string,
  result: Record<string, unknown>,
  usage: Record<string, unknown>
): readonly ValidatorLifecycleEvent[] {
  ...
  const base = {
    schemaId: 'atm.validatorLifecycleEvent.v1' as const,
    validatorId,
  ...
  const events: ValidatorLifecycleEvent[] = [{ ...base, event: 'eligible' }];
  events.push({ ...base, event: invoked ? 'invoked' : 'skipped' });
  if (cacheDecision === 'cache-hit') events.push({ ...base, event: 'cache-hit' });
  if (cacheDecision === 'cache-miss') events.push({ ...base, event: 'cache-miss' });
  if (cacheDecision === 'cache-bypass') events.push({ ...base, event: 'cache-bypass' });
  if (reusedFromReceipt) events.push({ ...base, event: 'receipt-reuse' });
  if (fanOutConsumerCount > 0) events.push({ ...base, event: 'fan-out' });
  if (result.ok === true) events.push({ ...base, event: 'passed' });
  if (result.ok !== true) events.push({ ...base, event: 'failed' });
  if (blockingCount > 0) events.push({ ...base, event: 'blocked' });
  return events;
}

function buildTierProposal(events: readonly ValidatorLifecycleEvent[]): ValidatorLifecycleSummary['tierProposal'] {
  const byValidator = new Map<string, ValidatorLifecycleEvent[]>();
  ...
  const proposal = {
    ...
    default: [] as string[],
    full: [] as string[],
    archiveCandidate: [] as string[],
    insufficientObservation: [] as string[]
  };
  for (const [validatorId, validatorEvents] of byValidator) {
    ...
    if (hasBlock) proposal.full.push(validatorId);
    else if (hasFanOut) proposal.fast.push(validatorId);
    else if (cacheOnly) proposal.archiveCandidate.push(validatorId);
    else proposal.default.push(validatorId);
...
function normalizeLifecycleTier(result: Record<string, unknown>, usage: Record<string, unknown>): ValidatorLifecycleEvent['tier'] {
  if (String(result.cacheDecision ?? usage.cacheDecision ?? '') === 'cache-hit') return 'archive-candidate';
  if (result.slow === true || usage.tier === 'full') return 'full';
  if (usage.tier === 'fast') return 'fast';
  return 'default';
}

function countEvents(events: readonly ValidatorLifecycleEvent[], event: ValidatorLifecycleEventKind): number {
