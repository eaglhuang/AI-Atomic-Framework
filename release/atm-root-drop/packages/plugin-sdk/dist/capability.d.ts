...
export type CapabilityKind = 'project-adapter' | 'language-adapter' | 'task-store' | 'lock-store' | 'document-index' | 'shard-store' | 'artifact-store' | 'log-store' | 'state-store' | 'rule-guard' | 'evidence-store' | 'external-task-source' | 'injector-plugin' | 'version-resolver' | 'quality-metrics-comparator' | 'upgrade-proposal-adapter';
