export class ConversationTranscriptReviewError extends Error {
    code;
    transcriptId;
    constructor(code, transcriptId, message) {
        super(message);
...
export function reviewConversationTranscript(input) {
    const transcript = input.transcript;
    if (transcript.redacted !== true) {
        throw new ConversationTranscriptReviewError('unredacted-transcript', transcript.transcriptId, `conversation transcript "${transcript.transcriptId}" must be redacted before review`);
    }
    ...
    if (transcript.containsSensitiveInput && redactionReportPaths.length === 0) {
        throw new ConversationTranscriptReviewError('sensitive-without-redaction-report', transcript.transcriptId, `conversation transcript "${transcript.transcriptId}" contains sensitive input but no redaction report was provided`);
    }
    const findings = [];
    for (const turn of transcript.turns) {
        ...
        for (const rule of findingRules) {
            if (!rule.match.test(turn.content))
                continue;
...
function makePatchDraft(draftKind, summary, skillId) {
    return {
        draftKind,
        patchMode: 'dry-run',
        mutatesFiles: false,
...
function createFindingId(transcriptId, kind, turnId) {
    return `finding.transcript.${sanitizeIdentifier(transcriptId)}.${kind}.${sanitizeIdentifier(turnId)}`;
}
function createEvidenceRef(transcript, kind, turnId) {
...
function createRationale(kind, turn) {
    switch (kind) {
