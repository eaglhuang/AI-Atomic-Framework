...
export const defaultCSharpLanguageAdapterManifest = {
...
export function createCSharpLanguageAdapter(policyOverrides = {}) {
...
export function detectCSharpProjectProfile(repositoryRoot) {
    ...
    const packageManager = hasSolutionFile || hasProjectFile ? 'dotnet' : 'unknown';
    return {
        packageManager,
        hasSolutionFile,
        hasProjectFile,
...
export function validateCSharpComputeAtom(request, profile = createUnknownProfile(), basePolicy = { forbiddenSpecifiers: [] }) {
...
export function scanCSharpImports(sourceFile) {
    const records = [];
...
export function createCSharpCommandRunnerContract(profile) {
    const commands = [];
...
export function createFastCSharpStaticCheck(profile) {
...
export function createDefaultCSharpStaticCheck(profile) {
...
export function createAllCSharpStaticCheck(profile) {
    const commands = unique([profile.typecheckCommand, profile.lintCommand].filter(Boolean));
...
function listFilesByExtension(repositoryRoot, extension) {
    const results = [];
    ...
    while (stack.length > 0) {
        ...
        for (const entry of readdirSync(current, { withFileTypes: true })) {
            ...
            if (entry.isDirectory()) {
                if (entry.name === 'node_modules' || entry.name === '.git' || entry.name === '.atm')
                    continue;
