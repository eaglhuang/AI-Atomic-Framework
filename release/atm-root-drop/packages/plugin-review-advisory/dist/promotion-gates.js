 ...
 */
export function checkPromotionSafetyGates(proposal, context) {
    const findings = [];
    // ── Gate 1: baseAtomVersion mismatch ──────────────────────────────────────
    const atomVersionMismatch = typeof proposal.baseAtomVersion === 'string' &&
        typeof context.currentAtomVersion === 'string' &&
        proposal.baseAtomVersion !== context.currentAtomVersion;
    const mapVersionMismatch = typeof proposal.baseMapVersion === 'string' &&
    ...
    const hasRedactionInput = Array.isArray(proposal.inputs) &&
        proposal.inputs.some((inp) => inp.kind === 'redaction-report');
    const redactionBlocked = context.hasRedactionReport === false && !hasRedactionInput;
    if (redactionBlocked) {
        findings.push({
    ...
    const hasCharterViolations = Array.isArray(context.charterViolations) && context.charterViolations.length > 0;
    const hasCharterWaiver = proposal.charterWaiver !== undefined &&
