...
- declares mandatory adapter-native `fast` / `default` / `all` static-check plans;
...
- `fast` prefers the adapter's declared typecheck command, typically `mypy`.
- `default` combines typecheck plus lint, typically `mypy` and `ruff check`.
- `all` stays static-only and currently matches the full declared Python static set.
