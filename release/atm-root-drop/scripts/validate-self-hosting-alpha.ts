...
const requiredFiles = [
  ...
  'examples/hello-world/src/hello-world.atom.ts'
];
...
const { createSelfHostingAlphaPrompt } = await import(pathToFileURL(path.join(root, 'packages/cli/src/commands/bootstrap.ts')).href);
...
try {
  ...
  const { run } = await import(`${pathToFileURL(path.join(repoCopy, 'examples/hello-world/src/hello-world.atom.ts')).href}?selfHosting=${Date.now()}`);
