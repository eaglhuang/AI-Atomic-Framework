...
export function runGovernanceCostBench(input: {
  ...
  const ineligibleReasons = collectIneligibleReasons(reports);
  return {
    schemaId: 'atm.governanceCostBenchReport.v1',
    shadowOnly: true,
    catalogVersion: input.catalog.catalogVersion,
    generatedAt: input.generatedAt ?? new Date().toISOString(),
    measurementStatus: ineligibleReasons.length ? 'cost-measurement-incomplete' : 'complete',
...
function buildCohorts(samples: readonly GovernanceCostSampleReport[]): GovernanceCostCohortReport[] {
  ...
  return [...byKey.entries()].map(([cohortKey, entries]) => {
    const ineligibleReasons = collectIneligibleReasons(entries);
    return {
      cohortKey,
...
function collectIneligibleReasons(samples: readonly GovernanceCostSampleReport[]): string[] {
  return [...new Set(samples.flatMap((sample) =>
    sample.ineligibleReasons.map((reason) => `${sample.sampleId}:${reason}`)
