...
function assertDeepEqual(actual: any, expected: any, message: any) {
  if (!isDeepStrictEqual(actual, expected)) {
    fail(`${message}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`);
  }
}

function compactRule(rule: any) {
  const compacted: any = {
...
function ensureExpectedIssues(actualIssues: any, expectedIssues: any, label: any) {
...
assertDeepEqual(
  registryTransitionActions,
  legalFixture.rules.map((rule: any) => rule.action),
