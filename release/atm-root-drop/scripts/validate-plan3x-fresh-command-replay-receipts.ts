...
function main() {
  ...
  if (report.nonClaim !== 'These receipts prove that focused Plan 3.x validators execute against current files and that 69 rows were consumed into source replay; they do not certify the full runbook complete until Plan 4, backlog, closeout, and full validation evidence are audited.') {
  ...
  if (report.familyDisposition?.rowsConsumedIntoSourceReplay !== 69) findings.push('rowsConsumedIntoSourceReplay mismatch');
  ...
  if (commandRuns.length !== 32) findings.push(`commandRuns count mismatch: expected 32, observed ${commandRuns.length}`);
  ...
  if (verifiedRows !== 69) findings.push(`source replay verified count mismatch: expected 69, observed ${verifiedRows}`);
