...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) {
    console.error(`[hook-batch-commit-attribution] FAIL ${message}`);
    process.exit(1);
  }
}
...
const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'atm-hook-batch-commit-attribution-'));
...
try {
  ...
  writeFileSync(path.join(tempRoot, '.atm', 'history', 'evidence', 'TASK-X.json'), JSON.stringify({ taskId: 'TASK-X' }, null, 2));
  writeFileSync(path.join(tempRoot, '.atm', 'history', 'evidence', 'TASK-X.closure-packet.json'), JSON.stringify({ taskId: 'TASK-X' }, null, 2));
  writeFileSync(path.join(tempRoot, '.atm', 'history', 'evidence', 'TASK-OTHER.bundle-manifest.json'), JSON.stringify({ taskId: 'TASK-OTHER' }, null, 2));
  writeFileSync(path.join(tempRoot, '.atm', 'history', 'evidence', 'git-boundary-runs', 'example.json'), JSON.stringify({ scenario: 'pending-batch-window' }, null, 2));
  ...
  const result = runHook(['pre-commit', '--cwd', tempRoot]) as any;
  assert(result?.ok === false, 'pre-commit should still require governed git commit wrapper');
  const requiredCommand = result?.evidence?.failureEnvelope?.requiredCommand ?? '';
  assert(
    String(requiredCommand).includes('--task TASK-X '),
    'wrapper-required command must prefer the pending batch checkpoint task id'
  );

  console.log('[hook-batch-commit-attribution] ok');
} finally {
