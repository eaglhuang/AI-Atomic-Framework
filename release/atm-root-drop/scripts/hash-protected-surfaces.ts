...
import { createSourceHashSnapshot } from '../packages/core/src/hash-lock/hash-lock.ts';

export function syncProtectedSurfaceDigests(root: any) {
...
export function computeCanonicalJsonDigestFromFile(filePath: any) {
  const parsed = JSON.parse(readFileSync(filePath, 'utf8'));
  return computeCanonicalJsonDigest(parsed);
}

export function computeCanonicalJsonDigest(value: any) {
  const normalized = normalizeForHash(value);
  const content = `${stableStringify(normalized)}\n`;
  return `sha256:${createHash('sha256').update(content).digest('hex')}`;
}

function rewriteJsonDigest(filePath: any) {
...
function normalizeForHash(value: any, pathParts: string[] = []): any {
...
function listJsonFiles(directory: any): string[] {
...
function listExampleAtomSpecs(examplesRoot: any): string[] {
