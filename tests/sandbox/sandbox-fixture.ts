...
function printHelp() {
  console.log('Usage: node tests/sandbox/sandbox-fixture.ts <setup|verify> [sandbox-root] [source-root] [expected-output.json]');
}
...
function resolveSandboxRoot(inputPath: any, createIfMissing = true) {
  if (inputPath) {
...
function mkTempDir(prefix: any) {
  return createTempWorkspace(prefix);
}

function resolveSourceRoot(inputPath: any) {
  return inputPath ? path.resolve(inputPath) : repoRoot;
}

function setupSandbox(sandboxRoot: any, sourceRoot: any) {
...
function summarizeSelfHostAlpha(parsed: any) {
  return {
    ok: parsed.ok === true,
...
function renderAgentsMarkdown(sourceRoot: any, targetRoot: any) {
...
function renderTemplate(template: any, tokens: any) {
...
function ensureNodeModules(sourceRoot: any, targetRoot: any) {
...
function runGit(args: any, cwd: any) {
