...
assert.equal(report.totals.freshCommandRows, 24);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 26);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 26);
assert.equal(report.positiveRows.length, 26);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 26);
