...
assert.equal(report.totals.freshCommandRows, 25);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 25);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 25);
assert.equal(report.positiveRows.length, 25);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 25);
