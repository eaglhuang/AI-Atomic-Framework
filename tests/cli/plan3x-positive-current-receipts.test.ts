...
assert.equal(report.totals.freshCommandRows, 30);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 18);
assert.equal(report.totals.objectiveAlignedNegativeControlRowsConsumed, 6);
assert.equal(report.totals.blockedByDoctorIntegrationDrift, 2);
assert.equal(report.totals.sourceRowsMutatedByThisReport, 18);
assert.equal(report.positiveRows.length, 18);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 18);
