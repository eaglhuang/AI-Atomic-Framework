...
assert.equal(report.totals.freshCommandRows, 13);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 37);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 37);
assert.equal(report.positiveRows.length, 37);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 37);
