...
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 59);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 59);
assert.equal(report.positiveRows.length, 59);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 59);
