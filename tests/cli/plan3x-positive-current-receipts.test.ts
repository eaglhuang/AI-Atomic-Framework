...
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 65);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 65);
assert.equal(report.positiveRows.length, 65);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 65);
