...
assert.equal(report.totals.freshCommandRows, 23);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 27);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 27);
assert.equal(report.positiveRows.length, 27);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 27);
