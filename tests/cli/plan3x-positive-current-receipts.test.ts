...
assert.equal(report.totals.freshCommandRows, 26);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 24);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 24);
assert.equal(report.positiveRows.length, 24);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 24);
