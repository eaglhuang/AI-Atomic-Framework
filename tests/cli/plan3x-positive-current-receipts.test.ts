...
assert.equal(report.status, 'positive-current-receipts-partial');
assert.equal(report.totals.freshCommandRows, 45);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 8);
assert.equal(report.totals.blockedByDoctorIntegrationDrift, 2);
assert.equal(report.totals.sourceRowsMutatedByThisReport, 0);
assert.equal(report.positiveRows.length, 8);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 8);
...
assert.deepEqual(report.nextExecutionOrder.map((entry: any) => entry.id), [
  'source-replay-recompute-eight-positive-rows',
