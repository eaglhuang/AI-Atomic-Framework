...
assert.equal(report.totals.freshCommandRows, 19);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 31);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 31);
assert.equal(report.positiveRows.length, 31);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 31);
