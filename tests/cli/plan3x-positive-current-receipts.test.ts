...
assert.equal(report.totals.freshCommandRows, 16);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 34);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 34);
assert.equal(report.positiveRows.length, 34);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 34);
