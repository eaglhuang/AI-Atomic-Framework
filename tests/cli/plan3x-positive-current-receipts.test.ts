...
assert.equal(report.totals.freshCommandRows, 0);
assert.equal(report.totals.positiveReceiptRowsReadyForSourceRecompute, 0);
assert.equal(report.totals.positiveReceiptRowsConsumedIntoSourceReplay, 50);
...
assert.equal(report.totals.sourceRowsMutatedByThisReport, 50);
assert.equal(report.positiveRows.length, 50);
assert.equal(new Set(report.positiveRows.map((entry: any) => entry.objectiveId)).size, 50);
