...
assert.ok(report.diagnostics.includes('dimension-fail-closed:backlog-deferred-verdict:not-complete'));
...
assert.equal(report.dimensions.find((entry: any) => entry.dimensionId === 'backlog-deferred-verdict')?.status, 'not-complete');
