...
assert.equal(report.overallVerdict, 'complete');
assert.equal(report.releaseAuthorized, true);
assert.deepEqual(report.diagnostics, []);
assert.equal(report.diagnostics.includes('dimension-fail-closed:backlog-deferred-verdict:not-complete'), false);
assert.equal(report.diagnostics.includes('dimension-fail-closed:objective-verdict:not-complete'), false);
...
assert.equal(report.dimensions.find((entry: any) => entry.dimensionId === 'objective-verdict')?.status, 'proven');
assert.equal(report.dimensions.find((entry: any) => entry.dimensionId === 'backlog-deferred-verdict')?.status, 'proven');
