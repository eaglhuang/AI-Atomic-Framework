...
assert.equal(report.totals.sourceRowsVerified, 17);
assert.equal(report.totals.sourceRowsNotComplete, 52);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 17);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 31);
