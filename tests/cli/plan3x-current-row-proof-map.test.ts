...
assert.equal(report.totals.sourceRowsVerified, 19);
assert.equal(report.totals.sourceRowsNotComplete, 50);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 19);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 29);
