...
assert.equal(report.totals.sourceRowsVerified, 14);
assert.equal(report.totals.sourceRowsNotComplete, 55);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 14);
