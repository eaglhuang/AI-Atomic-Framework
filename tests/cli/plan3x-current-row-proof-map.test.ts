...
assert.equal(report.totals.sourceRowsVerified, 23);
assert.equal(report.totals.sourceRowsNotComplete, 46);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 23);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 27);
