...
assert.equal(report.totals.sourceRowsVerified, 46);
assert.equal(report.totals.sourceRowsNotComplete, 23);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 46);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 4);
