...
assert.equal(report.totals.sourceRowsVerified, 12);
assert.equal(report.totals.sourceRowsNotComplete, 57);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 12);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 35);
...
assert.equal(families.get('negative-control-only')?.rowCount, 1);
