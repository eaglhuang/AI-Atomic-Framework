...
assert.equal(report.totals.sourceRowsVerified, 16);
assert.equal(report.totals.sourceRowsNotComplete, 53);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 16);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 32);
