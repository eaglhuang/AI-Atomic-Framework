...
assert.equal(report.totals.sourceRowsVerified, 13);
assert.equal(report.totals.sourceRowsNotComplete, 56);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 13);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 34);
