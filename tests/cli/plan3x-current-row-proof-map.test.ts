...
assert.equal(report.totals.sourceRowsVerified, 31);
assert.equal(report.totals.sourceRowsNotComplete, 38);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 31);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 19);
