...
assert.equal(report.totals.sourceRowsVerified, 24);
assert.equal(report.totals.sourceRowsNotComplete, 45);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 24);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 26);
