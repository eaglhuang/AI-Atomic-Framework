...
assert.equal(report.totals.sourceRowsVerified, 37);
assert.equal(report.totals.sourceRowsNotComplete, 32);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 37);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 13);
