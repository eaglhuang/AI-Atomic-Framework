...
assert.equal(report.totals.sourceRowsVerified, 25);
assert.equal(report.totals.sourceRowsNotComplete, 44);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 25);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 25);
