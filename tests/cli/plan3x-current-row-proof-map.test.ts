...
assert.equal(report.totals.sourceRowsVerified, 22);
assert.equal(report.totals.sourceRowsNotComplete, 47);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 22);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 28);
