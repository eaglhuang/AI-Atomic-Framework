...
assert.equal(report.totals.sourceRowsVerified, 27);
assert.equal(report.totals.sourceRowsNotComplete, 42);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 27);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 23);
