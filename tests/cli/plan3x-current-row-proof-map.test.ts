...
assert.equal(report.totals.sourceRowsVerified, 21);
assert.equal(report.totals.sourceRowsNotComplete, 48);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 21);
...
assert.equal(families.get('governed-state-replay-needed')?.rowCount, 18);
