...
assert.equal(report.totals.sourceRowsVerified, 50);
assert.equal(report.totals.sourceRowsNotComplete, 19);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 50);
