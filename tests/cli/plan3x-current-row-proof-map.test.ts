...
assert.equal(report.totals.sourceRowsVerified, 18);
assert.equal(report.totals.sourceRowsNotComplete, 51);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 18);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 30);
