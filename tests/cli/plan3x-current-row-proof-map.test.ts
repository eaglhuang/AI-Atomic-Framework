...
assert.equal(report.totals.sourceRowsVerified, 26);
assert.equal(report.totals.sourceRowsNotComplete, 43);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 26);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 24);
