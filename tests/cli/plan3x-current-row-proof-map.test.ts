...
assert.equal(report.totals.sourceRowsVerified, 34);
assert.equal(report.totals.sourceRowsNotComplete, 35);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 34);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 16);
assert.equal(families.get('governed-state-replay-needed')?.rowCount, 17);
