...
assert.equal(report.totals.sourceRowsVerified, 20);
assert.equal(report.totals.sourceRowsNotComplete, 49);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 20);
