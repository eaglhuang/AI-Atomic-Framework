...
assert.equal(report.totals.sourceRowsVerified, 40);
assert.equal(report.totals.sourceRowsNotComplete, 29);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 40);
assert.equal(families.get('verified-current-receipt')?.status, 'verified');
assert.equal(families.get('fresh-command-replay-needed')?.rowCount, 10);
