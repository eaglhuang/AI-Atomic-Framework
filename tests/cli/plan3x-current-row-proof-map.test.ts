...
assert.equal(report.totals.sourceRowsVerified, 69);
assert.equal(report.totals.sourceRowsNotComplete, 0);
...
assert.equal(families.get('verified-current-receipt')?.rowCount, 69);
...
assert.equal(families.get('governed-state-replay-needed')?.rowCount, 0);
