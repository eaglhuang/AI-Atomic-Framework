...
assert.deepEqual(classifyBacklogDisposition({ status: 'Open', followUp: 'Owned by ATM-GOV-0340.' }), { disposition: 'owned-open', ownerRefs: ['ATM-GOV-0340'] });
...
assert.equal(classifyBacklogDisposition({ status: 'Open', followUp: 'Investigate later.' }).disposition, 'unclassified');
