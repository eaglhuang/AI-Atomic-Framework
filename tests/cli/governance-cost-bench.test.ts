...
function testPairedCostAndTimeReport(): void {
  ...
  assert.equal(first.baseCost.incrementalCashCost, 0.0105);
  assert.equal(first.outcomeCost.incrementalCashCost, 0.00615);
  assert.equal(first.outcomeCost.fullyLoadedCashCost, 0.05615);
  assert.equal(first.ratios.incrementalCashCostRatio, 0.585714);
