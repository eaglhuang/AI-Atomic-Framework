...
assert.equal(evaluateBurnIn(scheduled, { minCompletedRuns: 2, minCalendarDays: 0 }).semanticVerdict, 'accept');
...
assert.equal(evaluateBurnIn([run(2, 'push', '2026-09-13T00:00:00Z', 'failure'), run(1, 'workflow_dispatch', '2026-09-12T00:00:00Z')], { minCompletedRuns: 2, minCalendarDays: 0 }).semanticVerdict, 'reject');
...
const missingLifecycleReceipt = {
  schemaId: 'atm.ciLifecycleEvidence.v1',
  sourceDigest: 'sha256:' + 'a'.repeat(64),
  receiptDigest: 'sha256:' + 'b'.repeat(64),
  runs: [{ ...run(3), lifecycle: undefined }],
};
assert.equal(evaluateBurnIn(missingLifecycleReceipt, { minCompletedRuns: 1, minCalendarDays: 0 }).claimStatus, 'invalid-input');
assert.equal(evaluateBurnIn(missingLifecycleReceipt, { minCompletedRuns: 1, minCalendarDays: 0 }).semanticVerdict, 'reject');
...
const candidateValidatorSource = readFileSync(new URL('../../scripts/validate-candidate-npm-install.ts', import.meta.url), 'utf8');
assert.match(candidateValidatorSource, /candidateSmokeCommandNames/);
assert.match(candidateValidatorSource, /requiredSuccessCommandFailures/);
