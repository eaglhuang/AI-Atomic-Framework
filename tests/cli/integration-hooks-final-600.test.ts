...
const maxLines = 600;
const integrationHooksDir = path.join(root, 'packages/cli/src/commands/integration-hooks');
const targets = [
  'packages/cli/src/commands/integration-hooks.ts',
  ...collectTypeScriptFiles(integrationHooksDir)
...
if (failures.length > 0) {
  throw new Error(`integration-hooks final-600 check failed:\n${failures.join('\n')}`);
}

process.stdout.write(`[integration-hooks-final-600] ok (${targets.length} files)\n`);
