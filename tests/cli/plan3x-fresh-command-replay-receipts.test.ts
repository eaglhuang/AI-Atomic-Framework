...
assert.equal(report.familyDisposition.sourceRowCount, 19);
assert.equal(report.familyDisposition.focusedCommandsExecuted, 24);
assert.equal(report.familyDisposition.commandsGreen, 24);
assert.equal(report.familyDisposition.rowsConsumedIntoSourceReplay, 31);
assert.equal(report.familyDisposition.rowsCertifiedCompleteByTheseReceipts, 0);
assert.equal(report.familyDisposition.remainingRowsInFamily, 19);
assert.equal(report.commandRuns.length, 24);
