...
assert.equal(report.familyDisposition.sourceRowCount, 31);
assert.equal(report.familyDisposition.focusedCommandsExecuted, 13);
assert.equal(report.familyDisposition.commandsGreen, 13);
assert.equal(report.familyDisposition.rowsConsumedIntoSourceReplay, 17);
assert.equal(report.familyDisposition.rowsCertifiedCompleteByTheseReceipts, 0);
assert.equal(report.familyDisposition.remainingRowsInFamily, 31);
assert.equal(report.commandRuns.length, 13);
