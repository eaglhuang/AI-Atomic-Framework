...
assert.equal(report.familyDisposition.sourceRowCount, 32);
assert.equal(report.familyDisposition.focusedCommandsExecuted, 12);
assert.equal(report.familyDisposition.commandsGreen, 12);
assert.equal(report.familyDisposition.rowsConsumedIntoSourceReplay, 16);
assert.equal(report.familyDisposition.rowsCertifiedCompleteByTheseReceipts, 0);
assert.equal(report.familyDisposition.remainingRowsInFamily, 32);
assert.equal(report.commandRuns.length, 12);
