...
const maxLines = 600;
const batchDir = path.join(root, 'packages/cli/src/commands/batch');
const targets = [
  'packages/cli/src/commands/batch.ts',
  ...collectTypeScriptFiles(batchDir)
...
if (failures.length > 0) {
  throw new Error(`batch final-600 check failed:\n${failures.join('\n')}`);
}

process.stdout.write(`[batch-final-600] ok (${targets.length} files)\n`);
