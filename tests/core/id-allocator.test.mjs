...
import { allocateAtomId, AtomIdAllocationError, normalizeAtomBucket, parseAtomId } from '../../packages/core/src/manager/id-allocator.mjs';
...
try {
  ...
  assert.throws(() => allocateAtomId('BAD-BUCKET', { repositoryRoot: tempRoot }), (error) => error.code === 'ATM_BUCKET_INVALID');
...
function writeRegistry(repositoryRoot, entries) {
