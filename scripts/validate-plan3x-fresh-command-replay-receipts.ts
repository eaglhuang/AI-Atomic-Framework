...
function main() {
  ...
  if (report.nonClaim !== 'These receipts prove that focused Plan 3.x validators execute against current files and that 15 rows were consumed into source replay; they do not certify any plan complete while source replay verdicts remain not-complete.') {
  ...
  if (report.familyDisposition?.rowsConsumedIntoSourceReplay !== 15) findings.push('rowsConsumedIntoSourceReplay mismatch');
  ...
  if (commandRuns.length !== 11) findings.push(`commandRuns count mismatch: expected 11, observed ${commandRuns.length}`);
  ...
  if (verifiedRows !== 15) findings.push(`source replay verified count mismatch: expected 15, observed ${verifiedRows}`);
