...
function main() {
  ...
  if (report.nonClaim !== 'These receipts prove that focused Plan 3.x validators execute against current files and that 37 rows were consumed into source replay; they do not certify any plan complete while source replay verdicts remain not-complete.') {
  ...
  if (report.familyDisposition?.rowsConsumedIntoSourceReplay !== 37) findings.push('rowsConsumedIntoSourceReplay mismatch');
  ...
  if (commandRuns.length !== 26) findings.push(`commandRuns count mismatch: expected 26, observed ${commandRuns.length}`);
  ...
  if (verifiedRows !== 37) findings.push(`source replay verified count mismatch: expected 37, observed ${verifiedRows}`);
