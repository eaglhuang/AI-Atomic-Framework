...
function main() {
  ...
  if (report.nonClaim !== 'These receipts prove that focused Plan 3.x validators execute against current files and that 20 rows were consumed into source replay; they do not certify any plan complete while source replay verdicts remain not-complete.') {
  ...
  if (report.familyDisposition?.rowsConsumedIntoSourceReplay !== 20) findings.push('rowsConsumedIntoSourceReplay mismatch');
  ...
  if (commandRuns.length !== 16) findings.push(`commandRuns count mismatch: expected 16, observed ${commandRuns.length}`);
  ...
  if (verifiedRows !== 20) findings.push(`source replay verified count mismatch: expected 20, observed ${verifiedRows}`);
