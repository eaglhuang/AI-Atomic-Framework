...
function main() {
  ...
  if (report.nonClaim !== 'These receipts prove that focused Plan 3.x validators execute against current files and that 26 rows were consumed into source replay; they do not certify any plan complete while source replay verdicts remain not-complete.') {
  ...
  if (report.familyDisposition?.rowsConsumedIntoSourceReplay !== 26) findings.push('rowsConsumedIntoSourceReplay mismatch');
  ...
  if (commandRuns.length !== 22) findings.push(`commandRuns count mismatch: expected 22, observed ${commandRuns.length}`);
  ...
  if (verifiedRows !== 26) findings.push(`source replay verified count mismatch: expected 26, observed ${verifiedRows}`);
