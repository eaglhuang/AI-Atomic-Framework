...
function main() {
  ...
  if (report.nonClaim !== 'These receipts prove that focused Plan 3.x validators execute against current files and that 40 rows were consumed into source replay; they do not certify any plan complete while source replay verdicts remain not-complete.') {
  ...
  if (report.familyDisposition?.rowsConsumedIntoSourceReplay !== 40) findings.push('rowsConsumedIntoSourceReplay mismatch');
  ...
  if (commandRuns.length !== 27) findings.push(`commandRuns count mismatch: expected 27, observed ${commandRuns.length}`);
  ...
  if (verifiedRows !== 40) findings.push(`source replay verified count mismatch: expected 40, observed ${verifiedRows}`);
