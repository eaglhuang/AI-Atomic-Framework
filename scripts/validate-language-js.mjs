...
function assertMessage(report, code) {
  assert(report.messages.some((message) => message.code === code), `expected message code ${code}`);
}
...
function validateNoDownstreamTerms() {
  const protectedFiles = [
    ...
    'packages/language-js/src/language-js-adapter.mjs',
    'packages/language-js/README.md',
    'scripts/validate-language-js.mjs',
...
try {
  ...
  assert(validReport.imports.some((importRecord) => importRecord.specifier === 'node:path'), 'valid compute atom must scan node:path import');
  assert(validReport.commandRunnerContract.commands.some((command) => command.commandKind === 'test' && command.command === 'npm run test'), 'command contract must include delegated test command');
  ...
  assert(requireImports.some((importRecord) => importRecord.statementKind === 'require' && importRecord.specifier === 'node:child_process'), 'scanImports must detect require calls');
  assert(requireImports.some((importRecord) => importRecord.statementKind === 'dynamic-import' && importRecord.specifier === 'node:path'), 'scanImports must detect dynamic imports');
