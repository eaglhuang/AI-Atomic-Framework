...
function main() {
  ...
  if (positiveRefs.length !== 14) findings.push(`positive row count mismatch: expected 14, observed ${positiveRefs.length}`);
  ...
  if (report.totals?.objectiveAlignedNegativeControlRowsConsumed !== 3) findings.push('objectiveAlignedNegativeControlRowsConsumed mismatch');
