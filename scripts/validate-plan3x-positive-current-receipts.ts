...
function main() {
  ...
  if (positiveRefs.length !== 16) findings.push(`positive row count mismatch: expected 16, observed ${positiveRefs.length}`);
  ...
  if (report.totals?.objectiveAlignedNegativeControlRowsConsumed !== 5) findings.push('objectiveAlignedNegativeControlRowsConsumed mismatch');
