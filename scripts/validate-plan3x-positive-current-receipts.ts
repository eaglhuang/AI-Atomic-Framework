...
function main() {
  ...
  if (positiveRefs.length !== 31) findings.push(`positive row count mismatch: expected 31, observed ${positiveRefs.length}`);
  ...
  if (report.totals?.objectiveAlignedNegativeControlRowsConsumed !== 8) findings.push('objectiveAlignedNegativeControlRowsConsumed mismatch');
