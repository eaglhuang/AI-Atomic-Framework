...
const generatedRegistryPath = path.join(root, 'packages', 'core', 'src', 'error-code-registry.generated.ts');
...
const registryDigest = `sha256:${createHash('sha256').update(registryJson).digest('hex')}`;
...
const markdown = [
  ...
  '',
  `Canonical registry digest: \`${registryDigest}\`.`,
  '',
  '## Operator Registry',
  '',
...
const generatedRegistry = [
  '// Generated by `npm run generate:error-codes`; edit docs/governance/error-code-registry.json instead.',
  '',
  `export const ATM_ERROR_CODE_REGISTRY_DIGEST = '${registryDigest}' as const;`,
  '',
  `export const ATM_ERROR_CODE_REGISTRY = ${registryJson} as const;`,
  ''
].join('\n');
writeGenerated(generatedRegistryPath, generatedRegistry);
if (checkMode) {
  if (stalePaths.length > 0) {
    console.error(`[generate-error-code-index] stale generated file(s): ${stalePaths.map((filePath) => path.relative(root, filePath)).join(', ')}`);
    process.exitCode = 1;
  } else {
    console.log(`[generate-error-code-index] check passed (${occurrences.size} codes; ${registryDigest})`);
  }
} else {
  console.log(`[generate-error-code-index] wrote ${path.relative(root, outputPath)} and ${path.relative(root, generatedRegistryPath)} (${occurrences.size} codes; ${registryDigest})`);
}

function writeGenerated(filePath: string, content: string): void {
  if (checkMode) {
    if (!existsSync(filePath) || readFileSync(filePath, 'utf8') !== content) stalePaths.push(filePath);
    return;
  }
