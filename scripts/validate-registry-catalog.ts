...
import { createAtomicRegistryEntry, createRegistryDocument, validateRegistryDocumentFile, writeRegistryArtifacts } from '../packages/core/src/registry/registry.ts';
import { renderRegistryCatalogMarkdown, writeRegistryCatalogFile } from '../packages/core/src/registry/registry-catalog.ts';
...
function normalizeComparableMarkdown(value: any) {
  return normalizeNewlines(value).trimEnd();
}

function assertNoBom(filePath: any) {
...
try {
  ...
  const mapEntries = (fixture.mapFixtures ?? []).map((mapFixture: any) => {
    const workbenchPath = `atomic_workbench/maps/${mapFixture.mapId}`;
    const specPath = `${workbenchPath}/map.spec.json`;
    const testPath = `${workbenchPath}/map.integration.test.ts`;
    ...
    return createAtomicMapRegistryEntry(specDocument as any, {
