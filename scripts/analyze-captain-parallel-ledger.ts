...
function renderMarkdown(result: CaptainParallelLedgerAnalysis): string {
  ...
  return [
    ...
    '',
    '## Auto-Batch Pipeline', '',
    `- Broker tickets: ${result.autoBatchPipeline.evidenceSources.brokerTickets}; wave tickets: ${result.autoBatchPipeline.evidenceSources.waveTickets}; waitedMs p50/p95: ${formatNullable(result.autoBatchPipeline.metrics.waitedMs.p50)} / ${formatNullable(result.autoBatchPipeline.metrics.waitedMs.p95)}; batchRate: ${result.autoBatchPipeline.metrics.batchRate === null ? 'n/a' : formatPct(result.autoBatchPipeline.metrics.batchRate)}; build/projection/commit signals: ${result.autoBatchPipeline.metrics.buildsPerWave}/${result.autoBatchPipeline.metrics.projectionsPerWave}/${result.autoBatchPipeline.metrics.commitsPerWave}.`,
