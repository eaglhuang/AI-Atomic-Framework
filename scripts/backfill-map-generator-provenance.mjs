...
import { validateRegistryDocument, validateRegistryDocumentFile, writeRegistryArtifacts } from '../packages/core/src/registry/registry.mjs';
...
if (writeMode && !dryRun) {
  ...
  const updatedRegistry = {
    ...registryDocument,
    generatedAt: now,
    entries: registryDocument.entries.map((entry) => {
      if (entry?.mapId !== result.mapId) {
        return entry;
      }
      const evidence = uniqueStrings([
        ...(entry.evidence ?? []).filter((value) => value !== 'generator-provenance:generated'),
...
function resolveLegacySourcePaths(sourceId) {
...
function resolveOwnerVersion(registryDocument, ownerAtom) {
  const ownerEntry = (registryDocument.entries ?? []).find((entry) => entry?.atomId === ownerAtom);
  return String(ownerEntry?.currentVersion || ownerEntry?.atomVersion || '0.1.0').trim();
}

function createBackfillRequest(legacyMap, legacyReport, ownerVersion) {
  const failedCaseCount = Array.isArray(legacyReport.cases)
    ? legacyReport.cases.filter((entry) => entry?.status === 'fail').length
    : 0;
  return {
    mapVersion: '0.1.0',
    members: [
...
function createLineageLog(input) {
  return {
    ...
    legacyMemberIds: Array.isArray(input.legacyMap.members)
      ? input.legacyMap.members.map((member) => member.atomId)
      : [],
    legacyCaseNames: Array.isArray(input.legacyReport.cases)
      ? input.legacyReport.cases.map((entry) => entry.name)
      : [],
    notes: [
