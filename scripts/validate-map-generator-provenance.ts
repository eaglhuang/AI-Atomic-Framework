...
if (writeReport) {
  ...
  fail('map generator provenance audit report is missing; run node --strip-types scripts/validate-map-generator-provenance.ts --write');
} else {
  const currentReportText = readFileSync(reportPath, 'utf8');
  if (currentReportText !== reportText) {
    fail('map generator provenance audit report is stale; run node --strip-types scripts/validate-map-generator-provenance.ts --write');
  }
}
