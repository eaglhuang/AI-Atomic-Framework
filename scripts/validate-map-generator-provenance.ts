...
import { validateRegistryDocumentFile } from '../packages/core/src/registry/registry.ts';
...
if (writeReport) {
  ...
  fail('map generator provenance audit report is missing; run node --experimental-strip-types scripts/validate-map-generator-provenance.ts --write');
} else {
  const currentReportText = readFileSync(reportPath, 'utf8');
  if (currentReportText !== reportText) {
    fail('map generator provenance audit report is stale; run node --experimental-strip-types scripts/validate-map-generator-provenance.ts --write');
  }
}
...
function buildAuditReport(registryDocument: any) {
  const entries = (Array.isArray(registryDocument.entries) ? registryDocument.entries : [])
    ...
    .map((entry: any) => auditMapEntry(entry));
  ...
  return {
    ...
    summary: {
      ...
      generatorBorn: entries.filter((entry: any) => entry.provenanceClass === 'generator-born').length,
      backfilledLegacy: entries.filter((entry: any) => entry.provenanceClass === 'backfilled-legacy').length,
      missingProvenance: entries.filter((entry: any) => entry.provenanceClass === 'missing-provenance').length
    },
    entries
  };
}

function auditMapEntry(entry: any) {
  ...
  const testPaths = Array.isArray(entry.location?.testPaths) && entry.location.testPaths.length > 0
    ...
    : [`${workbenchPath}/map.integration.test.ts`];
