...
function validateWorkflow(): void {
  ...
  for (const requiredStep of ['npm ci', 'npm run typecheck', 'npx eslint scripts/validate-ci-product-lane.ts tests/cli/ci-product-lane-contract.test.ts', 'ci-product-lane-contract.test.ts', 'run-cli-test-sweep.ts', 'validate-package-skeleton.ts', 'npm run validate:package-install', 'npm pack --workspaces --dry-run', 'npm ci --ignore-scripts']) {
