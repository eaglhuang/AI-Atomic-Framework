...
import { createLazyInstantiationContract, propagateTemplateUpgrade } from '../packages/core/src/polymorph/template.ts';
import { detectPolymorphicDimensions } from '../packages/core/src/polymorph/dimension-detector.ts';
...
for (const relativePath of [
  ...
  'packages/core/src/polymorph/dimension-detector.ts',
...
check(propagation.propagatedInstances.every((instance: any) => instance.inheritedBy === 'behavior.evolve'), 'propagation must mark inheritedBy=behavior.evolve');
check(propagation.propagatedInstances.every((instance: any) => instance.needsRegistryWrite === false), 'propagation must not write lazy instances into registry');
