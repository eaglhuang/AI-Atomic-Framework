...
import { createAtomicRegistryEntry, createRegistryDocument, evaluateRegistryEntryDrift, validateRegistryDocument } from '../packages/core/src/registry/registry.ts';
...
function collectDeclaredWorkbenchFolders(registryDocument: any) {
...
function assertNoWorkbenchAliasDrift(repositoryRoot: any, registryDocument: any, label: any) {
...
function mutateFile(tempRoot: any, relativePath: any, transform: any) {
...
function assertProtectedFilesStayNeutral() {
  const protectedFiles = [
    ...
    'scripts/validate-registry-core.ts',
...
try {
  ...
  let restore = mutateFile(tempRoot, fixture.specPath, (content: any) => `${content}\n `);
  ...
  restore = mutateFile(tempRoot, fixture.codePath, (content: any) => `${content}\nexport const driftProbe = true;\n`);
  ...
  restore = mutateFile(tempRoot, fixture.testPath, (content: any) => `${content}\n// drift probe\n`);
  drift = evaluateRegistryEntryDrift(entry, { repositoryRoot: tempRoot });
  ...
  const upgradedMinimalMapEntry = createAtomicMapRegistryEntry({
    ...mapFixtures.minimal,
    members: mapFixtures.minimal.members.map((member: any, index: any) => index === 0
