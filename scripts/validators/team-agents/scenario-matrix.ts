...
import { type ScopeLeaseRegistryEntry } from '../../../packages/core/src/governance/scope-lock.ts';
import { assertCoordinatorOnly, type WaveRole } from '../../../packages/cli/src/commands/team-wave.ts';
import { fail } from './assertions.ts';

export function waveCard(over: Partial<WaveCandidateCard> & { taskId: string }): WaveCandidateCard {
  return {
    taskId: over.taskId,
...
export function scopeLease(over: Partial<ScopeLeaseRegistryEntry> & { leaseId: string }): ScopeLeaseRegistryEntry {
  return {
