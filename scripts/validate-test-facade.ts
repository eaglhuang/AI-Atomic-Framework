...
check(filtered.parsed.validators.every((entry: any) => Array.isArray(entry.tags) && entry.tags.some((tag: any) => String(tag).toLowerCase() === 'cli')), 'filtered run must keep only tag:cli validators');
