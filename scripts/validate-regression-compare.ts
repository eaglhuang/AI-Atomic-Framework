 ...
 * 2. regression-compare.ts 對 positive fixtures 產出正確結果
 * 3. regression-compare.ts 對 negative fixtures 正確判定 fail
...
function run() {
  ...
  const modulePath = path.join(repoRoot, 'packages/core/src/police/regression-compare.ts');
  ...
  // --- 3. Core module 存在 ---
  assert(existsSync(modulePath), 'regression-compare.ts not found');
