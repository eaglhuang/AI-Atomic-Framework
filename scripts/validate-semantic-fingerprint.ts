...
const schemaPaths: Record<string, string> = {
  ...
  registryV1: 'schemas/registry.schema.json',
...
check(
  historyEntry.versions.every((versionRecord: any) => typeof versionRecord.semanticFingerprint === 'string'),
  'registry-v1 version history must store semanticFingerprint per version'
);
check(
  historyEntry.versions.every((versionRecord: any) => normalizeSemanticFingerprint(versionRecord.semanticFingerprint) === createAtomicSpecSemanticFingerprint(helloWorldAtom)),
...
for (const [logicalName, row] of Object.entries(versionIndex as Record<string, any>)) {
  check(
...
function validateDocument(label: any, validate: any, document: any, relativePath: any) {
