...
function normalizeRelative(relativePath: any) {
  return relativePath.replace(/\\/g, '/');
}

function listRelativeFiles(relativeDir: any) {
...
function pathMatchesScope(relativePath: any, scope: any) {
  ...
  if (Array.isArray(scope.extensions) && scope.extensions.length > 0 && !scope.extensions.some((extension: any) => normalizedPath.endsWith(extension))) {
    return false;
  }
...
function scopeEnumerationRoot(scope: any) {
  if (scope.path) {
    return scope.path;
  }
  if (scope.pathPrefix) {
