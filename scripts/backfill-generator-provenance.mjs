...
import { createSourceHashSnapshot, normalizeSourcePathList } from '../packages/core/src/hash-lock/hash-lock.mjs';
...
import { validateRegistryDocument, writeRegistryArtifacts } from '../packages/core/src/registry/registry.mjs';
...
const updatedEntries = registryDocument.entries.map((entry) => updateEntry(entry));
...
function writeWorkbenchMirror(entry, options) {
...
function renderBackfillTest(entry, sourceSpecPath) {
  return [
