 ...
 * 2. core diff.mjs 對 positive fixtures 產出正確結果
 * 3. core diff.mjs 對 negative fixtures 正確拋錯
...
function run() {
  ...
  const diffModulePath = path.join(repoRoot, 'packages/core/src/registry/diff.mjs');
  const cliModulePath = path.join(repoRoot, 'packages/cli/src/atm.mjs');
  ...
  // --- 3. Core module 存在 ---
  assert(existsSync(diffModulePath), 'packages/core/src/registry/diff.mjs not found');

  // --- 4. CLI module 存在且已註冊 ---
  assert(existsSync(cliModulePath), 'packages/cli/src/atm.mjs not found');
