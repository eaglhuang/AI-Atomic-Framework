...
function writeHostFiles(hostRoot: any, files: any) {
  for (const [relativePath, content] of Object.entries(files as Record<string, string>)) {
...
try {
  ...
  assert(stores.taskStore.listTasks().some((task: any) => task.workItemId === fixture.workItem.workItemId), 'task store must list the stored task');
