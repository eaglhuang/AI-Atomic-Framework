...
const validatorMap = new Map(config.validators.map((entry) => [entry.name, entry]));
...
const selectedValidators = selectedNames.map((name) => {
...
const results = parsedCli.parallel && !parsedCli.legacy
  ? await Promise.all(selectedValidators.map((validator) => runValidator(validator, profileConfig.mode, { json: parsedCli.json })))
...
function parseCliArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--filter') {
      ...
      options.filters.push(...value.split(',').map((entry) => entry.trim()).filter(Boolean));
      index += 1;
      continue;
    }
    if (arg === '--parallel') {
...
function resolveProfileValidatorNames(profileName) {
  const profile = resolveProfileConfig(profileName);
  const inherited = profile.extends ? resolveProfileValidatorNames(profile.extends) : [];
  return [...new Set([...inherited, ...(profile.validators ?? [])])];
}

function applyFilters(validatorNames, filters) {
  if (!filters || filters.length === 0) {
    return validatorNames;
  }
  return validatorNames.filter((name) => {
    const entry = validatorMap.get(name);
    if (!entry) {
      return false;
    }
    return filters.some((filter) => {
      if (filter.startsWith('tag:')) {
        ...
        return (entry.tags ?? []).some((candidate) => String(candidate).toLowerCase() === tag);
      }
      const needle = filter.toLowerCase();
      const byName = name.toLowerCase().includes(needle);
      const byTag = (entry.tags ?? []).some((candidate) => String(candidate).toLowerCase().includes(needle));
      return byName || byTag;
    });
  });
}

async function runValidatorsSequential(validators, mode, options) {
  const results = [];
...
function runValidator(validator, mode, options) {
  return new Promise((resolve) => {
    const startedAt = Date.now();
    const child = spawn(process.execPath, [path.join(root, validator.entry), '--mode', mode], {
      cwd: root,
...
function createSummary({ profile, mode, filters, parallel, legacy, startedAt, results }) {
...
function emitSummary(summary, jsonMode) {
  if (jsonMode) {
